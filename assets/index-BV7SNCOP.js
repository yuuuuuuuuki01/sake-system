...
cd sake-system\\relay`,success:["エクスプローラーで sake-system\\\\relay フォルダが見える状態になっていれば成功です。","cd を実行したあと、コマンドプロンプトの行頭が relay フォルダを指していれば問題ありません。"],errors:["git コマンドが使えない場合は Git for Windows が未導入です。先に Git をインストールしてください。","アクセス拒否が出る場合は C:\\\\Program Files 配下ではなく C:\\\\sake-relay\\\\ などに配置してください。"]})}
  ...
  `}function kn(e,t,s="all",n="",i=[],r=[],c="",p="",u=null,y="all",f="",v=[],b=[],w=[],P=null,o=[],l=[],d="amount",m="calendar"){const g=t==="products"?"商品別集計":t==="customers"?"得意先別集計":"担当別集計",$=t==="products"?e.productTotals:t==="customers"?e.customerTotals:e.staffTotals,A=s!=="all"&&i.length>0&&t!=="staff"?i:$,S=Ye(A,w,dl),C={all:"月別",yearly:"月別推移",monthly:"日別推移",weekly:"日別推移",daily:"当日"},I=ts[d],L=B=>d==="quantity"?B.quantity:d==="volume"?B.volumeMl:B.amount,T=B=>d==="quantity"?`${B.toLocaleString()}本`:d==="volume"?Tt(B):Ue(B);let R,j=[],K,se,oe;if(P&&P.monthlySales.length>0)R=P.monthlySales.slice(-24).map(B=>({month:B.month,amount:L(B)})),K=`${P.name} の月別${I}`,se=`${P.tab==="customers"?"得意先":"商品"}: ${P.code}`,oe="#0968e5";else if(o.length>0&&s!=="all"){R=o.map(le=>({month:le.month,amount:L(le)})),j=l.map(le=>({month:le.month,amount:L(le)}));const B=R.reduce((le,Q)=>le+Q.amount,0),$e=j.reduce((le,Q)=>le+Q.amount,0),ct=$e>0?(B-$e)/$e*100:0,Te=ct>0?"+":"";K=`${C[s]} ${I}（${n}）`,se=`${T(B)}${$e>0?` / 前年比 ${Te}${ct.toFixed(1)}%`:""}`,oe="#2f855a"}else{R=pl(e.monthlySales,d,m).curr,j=[];const $e=R.reduce((Te,le)=>Te+le.amount,0);K=`${m==="fiscal"?"決算年度別":"暦年別"}${I}`,se=`累計 ${T($e)}（${R.length}${m==="fiscal"?"期":"年"}）`,oe="#0F5B8D"}const ve=["amount","quantity","volume"].map(B=>`<button class="tab-button ${B===d?"active":""}" data-chart-metric="${B}">${ts[B]}</button>`).join(""),ie=["all","yearly","monthly","weekly","daily"].map(B=>`<button class="button ${B===s?"primary":"secondary"} small" type="button" data-analytics-period="${B}">${as[B]}</button>`).join(""),M=m==="fiscal"&&s==="yearly"?ml(e.monthlySales):r,X=m==="fiscal"&&s==="yearly"&&!M.includes(n)?M[0]??"":n,Qe=s!=="all"&&M.length>0&&t!=="staff"?`<select id="analytics-period-select" style="padding:6px 10px;border:1px solid var(--border);border-radius:6px;font-size:13px;">
        ...
        </select>`:"",Te=(b.length>0?b:e.staffTotals).filter(Q=>!c||Q.name.includes(c)||Q.code.includes(c)),le=y!=="all"&&f?` (${f})`:"";if(qe=`
...
${w?.stack??""}</div></section>`}return`
  ...
  `}const it={純米大吟醸:"#7c3aed",大吟醸:"#a855f7",純米吟醸:"#2563eb",純米:"#059669",本醸造:"#d97706",普通酒:"#6b7280",リキュール:"#e11d48",その他:"#9ca3af"},Nt=["純米大吟醸","大吟醸","純米吟醸","純米","本醸造","普通酒","リキュール","その他"];function ze(e){return e.toLocaleString("ja-JP")}function Be(e){return(e/1e3).toLocaleString("ja-JP",{maximumFractionDigits:1})}function Ta(e){return e<2?"#ef4444":e<=4?"#eab308":"#22c55e"}function Sc(e){return e<2?"要醸造":e<=4?"注意":"余裕あり"}function kc(e){if(e.length===0)return'<div class="chart-empty">出荷データなし</div>';const t=[...new Set(e.map(l=>l.month))].sort(),s=Nt.filter(l=>e.some(d=>d.brewCategory===l)),n={};for(const l of e)n[l.month]||(n[l.month]={}),n[l.month][l.brewCategory]=l.shipmentMl;const i=820,r=300,c={top:20,right:20,bottom:50,left:70},p=i-c.left-c.right,u=r-c.top-c.bottom,y=t.map(l=>s.reduce((d,m)=>d+(n[l]?.[m]??0),0)),f=Math.max(...y,1),v=p/t.length,b=Math.max(v-8,14),w=[0,.25,.5,.75,1].map(l=>{const d=c.top+u-u*l,m=f*l/1e3;return`
    ...
    `}).join(""),P=t.map((l,d)=>{let m=c.top+u;const g=c.left+d*v+(v-b)/2,$=s.map(L=>{const T=n[l]?.[L]??0,R=T/f*u;return m-=R,R>0?`<rect x="${g}" y="${m}" width="${b}" height="${R}" fill="${it[L]??"#9ca3af"}" opacity="0.85" rx="1"><title>${L}: ${Be(T)}L</title></rect>`:""}).join(""),[x,A]=l.split("-"),S=parseInt(A),C=S===10||d%2===0,I=S===10?`${x}年度`:`${S}月`;return`<g>${$}${C?`<text x="${g+b/2}" y="${r-12}" class="chart-axis centered-axis" style="font-size:10px;">${I}</text>`:""}</g>`}).join(""),o=s.map(l=>`<span style="display:inline-flex;align-items:center;gap:4px;margin-right:12px;font-size:11px;">
