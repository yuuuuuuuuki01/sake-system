...
`,"<br />")}function Ja(e){const o=[...Object.values(Le),{id:"custom",season:"カスタム",subject:"",body:""}].map(n=>`
  ...
  `}function ie(e){return e.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}function Xe(e){return e.trim().toLowerCase()}function za(e,a){const o=Xe(a),t=e.filter(i=>o?[i.code,i.name,i.name].map(Xe).some(r=>r.includes(o)):!0).slice(0,50),n=t.length?`
        <div class="table-wrap">
          <table>
            <thead>
              <tr>
...
`);return`
  ...
  `}function H(e){return e.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}function at(e){return e.trim().toLowerCase()}function ds(e,a){const o=at(a),t=e.filter(i=>o?[i.code,i.name,i.janCode].map(at).some(r=>r.includes(o)):!0),n=t.length?`
    ...
    `).join(""),c=e.reduce((p,y)=>p+y.amount,0),d=a.reduce((p,y)=>p+y.balance,0),u=a.filter(p=>p.status!=="paid").length;return`
    <section class="page-head">
      <div>
  ...
  `}function W(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function ps(e,a){const o={holding:"保有中",due:"期日到来",cleared:"決済済"},t={holding:"neutral",due:"warning",cleared:"success"},n=e.map(u=>`
      <tr>
  ...
  `}function hs(e,a,o){const t={success:"正常",warning:"注意",error:"異常",running:"実行中"};return`
    <section class="page-head">
      <div>
...
cd sake-system\\relay`,success:["エクスプローラーで sake-system\\\\relay フォルダが見える状態になっていれば成功です。","cd を実行したあと、コマンドプロンプトの行頭が relay フォルダを指していれば問題ありません。"],errors:["git コマンドが使えない場合は Git for Windows が未導入です。先に Git をインストールしてください。","アクセス拒否が出る場合は C:\\\\Program Files 配下ではなく C:\\\\sake-relay\\\\ などに配置してください。"]})}
  ...
  `}function vs(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function bs(e){return e.replace("-","/")}function gs(e){if(e.length===0)return'<div class="chart-empty">データなし</div>';const a=760,o=280,t={top:16,right:24,bottom:36,left:64},n=a-t.left-t.right,i=o-t.top-t.bottom,r=Math.max(...e.map(u=>u.amount),1),l=n/e.length,c=[0,.25,.5,.75,1].map(u=>{const p=t.top+i-i*u,y=`${Math.round(r*u/1e4).toLocaleString("ja-JP")}万円`;return`
        <g>
          <line x1="${t.left}" y1="${p}" x2="${a-t.right}" y2="${p}" class="chart-grid" />
          <text x="8" y="${p+4}" class="chart-axis">${y}</text>
        </g>
      `}).join(""),d=e.map((u,p)=>{const y=u.amount/r*i,h=Math.max(l-18,24),P=t.left+p*l+(l-h)/2,R=t.top+i-y;return`
        <g>
          <rect x="${P}" y="${R}" width="${h}" height="${y}" rx="6" class="analytics-bar" />
          <text x="${P+h/2}" y="${o-10}" class="chart-axis centered-axis">${bs(u.month)}</text>
        </g>
      `}).join("");return`
  ...
  `}function X(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function ks(e){const a=Math.max(...e.salesByProduct.flatMap(i=>i.values),1),o=e.salesByProduct.map(i=>{const r=i.values.map((l,c)=>`
          <div class="bar-col">
            <div class="bar" style="height:${Math.round(l/a*120)}px" title="${e.months[c]}: ${X(l)}"></div>
      ...
      <tr>
        ...
        <td>${i.productName}</td>
        <td class="numeric">${X(i.costPrice)}</td>
        <td class="numeric">${X(i.sellPrice)}</td>
        <td class="numeric">${X(i.margin)}</td>
        <td class="numeric">
  ...
  `}function re(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function Ds(e,a,o,t){const n={cash:"現金",card:"カード",paypay:"PayPay",other:"その他"},i={new:"新規",processing:"処理中",shipped:"発送済",delivered:"配達済"},r={new:"warning",processing:"neutral",shipped:"success",delivered:"success"},l=e.map(p=>`
    ...
    `).join(""),d=e.reduce((p,y)=>p+y.amount,0),u=a.filter(p=>p.status==="new").length;return`
    <section class="page-head">
      <div>
  ...
  `}function qs(e,a,o,t){let n="";switch(e){case"chain_store":n=Ts(t,o,a);break;case"quotation":n=Rs(t,o,a);break;case"invoice_monthly":n=Es(t,o,a);break}const i=Object.keys(nt).map(c=>`<button class="tab-button ${e===c?"active":""}" data-print-template="${c}">${nt[c]}</button>`).join(""),r=t.lines.map((c,d)=>`
      <tr>
      ...
      </tr>`).join(""),l=[{key:"showSeal",label:"印影"},{key:"showRegistrationNo",label:"登録番号"},{key:"showBankInfo",label:"振込先"},{key:"showJanCode",label:"JAN"},{key:"showRemarks",label:"備考"}].map(c=>`<label style="font-size:12px;"><input type="checkbox" data-print-opt="${c.key}" ${a[c.key]?"checked":""} /> ${c.label}</label>`).join(" ");return`
    <section class="page-head no-print">
      <div>
      ...
      <div class="print-settings">
          ...
          <div style="display:flex;flex-wrap:wrap;gap:8px;">
            ...
            <label class="field" style="flex:1 1 100px;">
              ...
              <select data-print-field="taxRate">
                ...
                <option value="0.08" ${t.taxRate===.08?"selected":""}>8%</option>
              </select>
            </label>
            ${e==="invoice_monthly"?`
                <label class="field" style="flex:1 1 100px;">
                  <span>お支払期限</span>
                  <input type="date" data-print-field="dueDate" value="${t.dueDate??""}" />
                </label>
                <label class="field" style="flex:1 1 100px;">
                  <span>前回請求額</span>
                  <input type="number" data-print-field="previousBalance" value="${t.previousBalance??0}" />
                </label>`:""}
            ${e==="chain_store"?`
                <label class="field" style="flex:1 1 100px;">
                  <span>柱店CD</span>
                  <input type="text" data-print-field="chainStoreCode" value="${t.chainStoreCode??""}" />
                </label>
                <label class="field" style="flex:1 1 100px;">
                  <span>伝票区分</span>
                  <input type="text" data-print-field="slipTypeCode" value="${t.slipTypeCode??""}" />
                </label>`:""}
          </div>
        </div>
        ...
        <div class="panel">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
            <h3 class="panel-title">明細 (${t.lines.length}行)</h3>
        ...
        <details class="panel">
          ...
          <div style="margin-top:12px;">
            ...
            <div style="display:flex;flex-wrap:wrap;gap:12px;align-items:center;">
              <label style="font-size:12px;"><input type="checkbox" data-print-opt="showReferenceOverlay" ${a.showReferenceOverlay?"checked":""} /> 参考画像表示</label>
              <label style="display:flex;align-items:center;gap:4px;font-size:12px;">
                濃さ <input type="range" min="0" max="1" step="0.05" value="${a.overlayOpacity}" data-print-opt="overlayOpacity" style="width:80px;" />
              </label>
            </div>
          </div>
        </details>`:""}
