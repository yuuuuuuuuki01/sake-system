    ...
    <section class="panel" style="margin-top:12px;">
      <div class="panel-header" style="padding-bottom:4px;">
        <div>
          ...
          <p class="panel-caption">印刷瓶・ラベル不要品はチェックを外してください${i.size>0?`（${i.size}品除外中 = ${J(Math.round(v))}本）`:""}</p>
        </div>
      </div>
      <div style="display:flex;flex-wrap:wrap;gap:4px;padding:4px;max-height:300px;overflow-y:auto;">
        ${e.map(E=>{const q=E.plannedQty>0?E.plannedQty:Math.max(0,E.demandForecast+E.safetyStockTarget-E.openingStock);if(q<=0)return"";const R=i.has(E.productCode);return`
            <label style="display:flex;align-items:center;gap:4px;padding:4px 8px;border:1px solid var(--border);border-radius:6px;font-size:11px;cursor:pointer;
              background:${R?"var(--surface-alt)":"var(--surface)"};${R?"opacity:0.5;":""}white-space:nowrap;">
              <input type="checkbox" data-action="cal-label-toggle" data-code="${E.productCode}"
                ${R?"":"checked"} style="margin:0;" />
              <span style="overflow:hidden;text-overflow:ellipsis;max-width:140px;" title="${E.productName}">${E.productName}</span>
              <span style="color:var(--text-secondary);">${J(Math.round(q))}</span>
            </label>
          `}).join("")}
      </div>
    </section>
