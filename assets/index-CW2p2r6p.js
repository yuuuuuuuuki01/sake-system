...
`}function jl(e){const t=Rs(e,.15),n=Gt(e,.88),s=Gt(e,.96);return`
...
<div class="q-doc">
  ...
  ${v}
</div>`}function Bs(e,t,n,s,i,l,d){const p=e.lines.reduce((A,T)=>A+T.amount,0),u=Math.round(p*e.taxRate/100),h=p+u,v=e.templateType==="sake",g=s.length>=1?t.filter(A=>A.name.includes(s)||A.code.includes(s)).slice(0,8):[],x=i.length>=1?n.filter(A=>A.name.includes(i)||A.code.includes(i)).slice(0,8):[];if(e.previewMode){const A=d.accentColor||"#0968e5";return`
          ...
          <div class="filter-grid filter-grid--wide">
            ...
            <label class="field" style="flex:1 1 160px;">
              ...
              <select id="mat-type">
                ...
                ${Ss.map(s=>`<option ${n?.materialType===s?"selected":""}>${s}</option>`).join("")}
              </select>
            </label>
...
金井酒造店`;function td(e,t){const n=t?e.find(i=>i.id===t):null,s=t==="__new__";return`
  ...
  `}function xd(e,t,n){const{subtotal:s,taxAmount:i,total:l}=Js(e.lines,e.taxRate),d=e.previousBalance??0,p=e.paymentAmount??0,u=d-p+l,h=e.lines.map(g=>`
      <tr>
  ...
  `}function $d(e,t,n){const{subtotal:s,taxAmount:i,total:l}=Js(e.lines,e.taxRate),d=e.lines.map(u=>`
      <tr>
...
${T?.stack??""}</div></section>`}return`
  ...
  `}const Xd={純米大吟醸:"#7c3aed",大吟醸:"#a855f7",純米吟醸:"#2563eb",純米:"#059669",本醸造:"#d97706",普通酒:"#6b7280",リキュール:"#e11d48",その他:"#9ca3af"},Zs={planned:"計画中",active:"進行中",completed:"完了"},Zd={planned:"neutral",active:"warning",completed:"success"},eo={未着手:"#e5e7eb",進行中:"#3b82f6",完了:"#22c55e"},$e=8;function to(e){return e.toLocaleString("ja-JP")}function kt(e){return Xd[e]??"#6366f1"}function Xe(e,t){return Math.round((new Date(t).getTime()-new Date(e).getTime())/864e5)}function ao(e,t){const n=new Date(e);return n.setDate(n.getDate()+t),n.toISOString().slice(0,10)}function Re(e){return e?e.slice(5).replace("-","/"):"―"}function ep(e,t){const n=e.filter(A=>A.status!=="completed"&&A.startDate&&A.targetEndDate);if(n.length===0)return"";const s=n.flatMap(A=>[A.startDate,A.targetEndDate]);s.sort();const i=s[0],l=s[s.length-1],d=Math.min(Xe(i,l)+1,150),p=d*$e,u=[];let h="";for(let A=0;A<d;A++){const T=ao(i,A),E=T.slice(0,7);E!==h&&(u.push(`<span style="position:absolute;left:${A*$e}px;font-size:9px;color:#9ca3af;white-space:nowrap;border-left:1px solid #d1d5db;padding-left:2px;">${parseInt(T.slice(5,7))}月</span>`),h=E)}const v=new Date().toISOString().slice(0,10),g=Xe(i,v),x=g>=0&&g<d?`<div style="position:absolute;left:${g*$e}px;top:0;width:2px;height:100%;background:#ef4444;z-index:5;opacity:0.6;"></div>`:"",C=n.map(A=>{const T=(t[A.id]??[]).sort((m,y)=>m.stepOrder-y.stepOrder),E=kt(A.brewCategory),o=T.find(m=>m.status==="進行中"),r=T.length>0?Math.round(T.filter(m=>m.status==="完了").length/T.length*100):0,c=T.map(m=>{const y=Math.max(Xe(i,m.plannedStart),0),b=Math.min(Xe(i,m.plannedEnd),d-1),w=Math.max((b-y+1)*$e,$e),_=eo[m.status],$=m.status==="未着手"?"#666":"#fff";return`<div style="position:absolute;left:${y*$e}px;top:3px;width:${w}px;height:18px;background:${_};border-radius:2px;font-size:7px;line-height:18px;padding:0 2px;color:${$};overflow:hidden;white-space:nowrap;" title="${m.stepName} ${Re(m.plannedStart)}〜${Re(m.plannedEnd)}">${m.stepOrder<=3?"":m.stepName.slice(0,3)}</div>`}).join("");return`
    ...
    <article class="panel" style="border-left:4px solid ${u};margin-bottom:8px">
      ...
      ${n?ap(e,s):""}
    </article>`}function ap(e,t){if(t.length===0)return'<div style="padding:12px 0;color:#9ca3af;font-size:0.82rem">工程未登録</div>';const n=t.flatMap(g=>[g.plannedStart,g.plannedEnd].filter(Boolean));if(n.length===0)return'<div style="padding:12px 0;color:#9ca3af;font-size:0.82rem">日程未設定</div>';n.sort();const s=n[0],i=n[n.length-1],l=Math.min(Xe(s,i)+1,100),d=l*$e,p=[];let u="";for(let g=0;g<l;g++){const x=ao(s,g),C=x.slice(0,7);C!==u&&(p.push(`<span style="position:absolute;left:${g*$e}px;font-size:8px;color:#9ca3af;white-space:nowrap">${parseInt(x.slice(5,7))}月</span>`),u=C)}const h=t.map(g=>{const x=Math.max(Xe(s,g.plannedStart),0),C=Math.min(Xe(s,g.plannedEnd),l-1),A=x*$e,T=Math.max((C-x+1)*$e,$e),E=eo[g.status],o=g.status==="未着手"?"#374151":"#fff";return`
    ...
    >
      ...
      ${s}
    </select>`}function lp(e,t){const n={"at-risk":{label:"離反リスク",cls:"danger"},dormant:{label:"休眠",cls:"warning"},declining:{label:"下落中",cls:"info"}}[e.status],s=e.status==="declining"&&e.amountLastYearSameMonth>0?`<td class="numeric" style="color:var(--color-danger);font-weight:700;">${Math.round(e.yoyRatio*100)}%</td>`:e.status==="dormant"?`<td class="numeric" style="color:var(--color-warning);">${e.daysSinceLastOrder}日</td>`:'<td class="numeric" style="color:var(--color-danger);">今月未注文</td>',i=!!t?.actionedAt,l=i?'style="opacity:0.45;"':"",d=t?.reason?`<br><span class="status-pill info" style="font-size:0.72rem;">${no(t.reason)}</span>`:"";return`
...
<div class="panel">
  ...
  ${fp(d)}
      ...
      <span class="status-pill" style="background:${je[i]}15;color:${je[i]};padding:2px 8px;border-radius:9px;font-size:0.75rem">${Ca[i]}</span>
    </td></tr>`);for(const d of l){const p=ze.map((u,h)=>{const v=d.peakMonths.includes(h),g=so(d,h),x=h===t?"outline:2px solid #0F5B8D;outline-offset:-1px;":"";let C="transparent";v?C=je[d.seasonType]:g&&(C=je[d.seasonType]+"40");const A=v||g?`background:${C};border-radius:3px;height:18px;width:100%;`:"";return`<td style="padding:2px;text-align:center;${x}"><div style="${A}" title="${v?"ピーク":g?"提案期間":""}"></div></td>`}).join("");n.push(`<tr style="border-bottom:1px solid #f0f0f0">
        ...
        ${p}
      </tr>`)}}}return n.join("")}function so(e,t){if(e.peakMonths.length===0||e.peakMonths.includes(t))return!1;const n=e.proposalStartMonth,s=e.peakMonths[0];return n<=s?t>=n&&t<s:t>=n||t<s}function gp(e,t){const s=["year-round","seasonal","year-end"].map(i=>{const l=e[i];if(l.length===0)return"";const d=l.filter(u=>u.peakMonths.includes(t)||so(u,t));if(d.length===0)return"";const p=d.map(u=>{const v=u.peakMonths.includes(t)?'<span class="status-pill" style="background:#c0562115;color:#c05621">ピーク月</span>':'<span class="status-pill" style="background:#2f855a15;color:#2f855a">提案開始</span>',g=u.monthlyQuantity.reduce((x,C)=>x+C,0);return`<tr>
      ...
      <tr>
        ...
        <td>
          <select class="input-cell" data-ded-row="${v}" data-ded-field="categoryCode">
            ${fs.map(g=>`<option value="${g.code}" ${g.code===h.categoryCode?"selected":""}>${g.code}:${g.name}</option>`).join("")}
          </select>
        </td>
        <td class="numeric">
