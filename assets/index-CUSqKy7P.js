...
`}function Ls(e){const t=String(e);return/[,"\n]/.test(t)?`"${t.replaceAll('"','""')}"`:t}function Ds(e){const n=["酒類コード","区分名","アルコール度数","製造数量","前月繰越","当月調整","輸出控除","見本等控除","課税数量","税率(円/L)","税額(円)"].join(","),l=e.rows.map(c=>[c.taxCategory,c.taxCategoryName,c.alcoholDegree,c.productionVolume,c.previousBalance,c.currentAdjustment,c.exportDeduction,c.sampleDeduction,c.taxableVolume,c.taxRate,c.taxAmount].map(Ls).join(",")),r=`,合計,,${e.rows.reduce((c,a)=>c+a.productionVolume,0)},,,${e.rows.reduce((c,a)=>c+a.exportDeduction,0)},${e.rows.reduce((c,a)=>c+a.sampleDeduction,0)},${e.totalVolume},,${e.totalTax}`;return"\uFEFF"+[n,...l,r].join(`
`)+`
`}function Ps(e){const t=e.rows.map(r=>{const c=Math.max(0,r.productionVolume+r.previousBalance+r.currentAdjustment-r.exportDeduction-r.sampleDeduction),a=Math.round(c*r.taxRate);return{...r,taxableVolume:c,volume:c,taxAmount:a}}),n=t.reduce((r,c)=>r+c.taxableVolume,0),l=t.reduce((r,c)=>r+c.taxAmount,0);return{...e,rows:t,totalVolume:n,totalTax:l}}async function qs(e){const{supabaseInsert:t}=await b(async()=>{const{supabaseInsert:n}=await Promise.resolve().then(()=>q);return{supabaseInsert:n}},void 0);await t("tax_declarations",{target_year:e.targetYear,target_month:e.targetMonth,company_name:e.companyName,company_no:e.companyNo,company_address:e.companyAddress,company_representative:e.companyRepresentative,tax_office:e.taxOffice,total_taxable_volume:e.totalVolume,total_tax_amount:e.totalTax,status:e.status,xml_data:ja(e),submitted_at:e.submittedAt})}async function wt(e){const t=await x("store_sales",{sale_date:`eq.${e}`,order:"sale_time.asc"});return t.length>0?t.map(n=>({id:m(n,["id"],""),saleDate:m(n,["sale_date"],e),saleTime:m(n,["sale_time"],""),productCode:m(n,["product_code"],""),productName:m(n,["product_name"],""),quantity:A(n,["quantity"],0),unitPrice:A(n,["unit_price"],0),amount:A(n,["amount"],0),paymentMethod:m(n,["payment_method"],"cash")})):[]}async function Oa(){const e=await x("store_orders",{order:"order_date.desc",limit:"100"});return e.length>0?e.map(t=>({id:m(t,["id"],""),orderNo:m(t,["order_no"],""),orderDate:V(t,["order_date"],""),customerName:m(t,["customer_name"],""),postalCode:m(t,["postal_code"],""),address:m(t,["shipping_address"],""),items:[],totalAmount:A(t,["total_amount"],0),status:m(t,["status"],"new"),shippingDate:V(t,["shipping_date"],"")})):[]}async function Oe(e){const t=await Ue("email_campaigns",{subject:e.subject,body:e.body,template_id:e.templateId,audience_mode:e.audienceMode,audience_filter:e.audienceFilter,recipient_count:e.recipientCount,sent_count:e.status==="sent"?e.recipientCount:0,status:e.status,sent_at:e.status==="sent"?new Date().toISOString():null});return{id:t?.id??`local-email-${Date.now()}`,subject:t?.subject??e.subject,body:t?.body??e.body,templateId:t?.template_id??e.templateId,audienceMode:t?.audience_mode??e.audienceMode,audienceFilter:t?.audience_filter??e.audienceFilter,recipientCount:t?.recipient_count??e.recipientCount,status:t?.status??e.status,createdAt:t?.created_at??new Date().toISOString(),updatedAt:t?.updated_at??new Date().toISOString()}}async function Ma(e,t){throw new Error("VITE_RESEND_API_KEY is not configured")}async function Ts(e){const t={order:"updated_at.desc"};return e&&(t.template_key=`eq.${e}`),(await x("print_layouts",t)).map(l=>({id:m(l,["id"],""),name:m(l,["name"],""),templateKey:m(l,["template_key"],""),positions:l.positions??{},isDefault:z(l,["is_default"],!1),note:m(l,["note"],""),updatedAt:m(l,["updated_at"],"")}))}async function Ns(e){const{supabaseInsert:t}=await b(async()=>{const{supabaseInsert:r}=await Promise.resolve().then(()=>q);return{supabaseInsert:r}},void 0),n={id:e.id,name:e.name,template_key:e.templateKey,positions:e.positions,is_default:e.isDefault??!1,note:e.note??"",updated_at:new Date().toISOString()},l=await t("print_layouts",n);return l?{id:m(l,["id"],e.id),name:m(l,["name"],e.name),templateKey:m(l,["template_key"],e.templateKey),positions:l.positions??e.positions,isDefault:z(l,["is_default"],!1),note:m(l,["note"],""),updatedAt:m(l,["updated_at"],"")}:null}async function Rs(e){const t=new URL("/rest/v1/print_layouts","https://loarwnuyvfxiscjjsmiz.supabase.co");t.searchParams.set("id",`eq.${e}`);const n="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxvYXJ3bnV5dmZ4aXNjampzbWl6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzYyMTM0MDYsImV4cCI6MjA5MTc4OTQwNn0.7XofVlIiTjlzA9Oz-3lTX1ghLL1qqCcDRadf_a__ys8 ";try{return(await fetch(t.toString(),{method:"DELETE",headers:{apikey:n,Authorization:`Bearer ${n}`}})).ok}catch{return!1}}async function js(){return(await x("mail_senders",{order:"is_default.desc,name.asc"})).map(t=>({id:m(t,["id"],""),name:m(t,["name"],""),email:m(t,["email"],""),displayName:m(t,["display_name"],""),signature:m(t,["signature"],""),replyTo:m(t,["reply_to"],""),isDefault:z(t,["is_default"],!1),isVerified:z(t,["is_verified"],!1),note:m(t,["note"],"")}))}async function Os(e){const{supabaseInsert:t}=await b(async()=>{const{supabaseInsert:l}=await Promise.resolve().then(()=>q);return{supabaseInsert:l}},void 0),n=await t("mail_senders",{id:e.id,name:e.name,email:e.email,display_name:e.displayName??"",signature:e.signature??"",reply_to:e.replyTo??"",is_default:e.isDefault??!1,is_verified:e.isVerified??!1,note:e.note??"",updated_at:new Date().toISOString()});return n?{id:m(n,["id"],e.id),name:m(n,["name"],e.name),email:m(n,["email"],e.email),displayName:m(n,["display_name"],""),signature:m(n,["signature"],""),replyTo:m(n,["reply_to"],""),isDefault:z(n,["is_default"],!1),isVerified:z(n,["is_verified"],!1)}:null}async function Ms(e){const t="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxvYXJ3bnV5dmZ4aXNjampzbWl6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzYyMTM0MDYsImV4cCI6MjA5MTc4OTQwNn0.7XofVlIiTjlzA9Oz-3lTX1ghLL1qqCcDRadf_a__ys8 ";try{const n=new URL("/rest/v1/mail_senders","https://loarwnuyvfxiscjjsmiz.supabase.co");return n.searchParams.set("id",`eq.${e}`),(await fetch(n.toString(),{method:"DELETE",headers:{apikey:t,Authorization:`Bearer ${t}`}})).ok}catch{return!1}}const kt={delivery:"🚚 納品",tour:"🏭 蔵見学",meeting:"📋 商談",brewing:"🍶 仕込",general:"📌 その他"},St={delivery:"#9C27B0",tour:"#FF9800",meeting:"#2196F3",brewing:"#4CAF50",general:"#0F5B8D"};async function Fs(e){const t=`${e}-01T00:00:00Z`,[n,l]=e.split("-").map(i=>parseInt(i,10)),r=new Date(n,l,0).getDate(),c=`${e}-${String(r).padStart(2,"0")}T23:59:59Z`;return(await x("calendar_events",{starts_at:`gte.${t}`,and:`(starts_at.lte.${c})`,order:"starts_at.asc"})).map(i=>({id:m(i,["id"],""),title:m(i,["title"],""),description:m(i,["description"],""),category:m(i,["category"],"general")||"general",startsAt:m(i,["starts_at"],new Date().toISOString()),endsAt:m(i,["ends_at"],""),isAllDay:z(i,["is_all_day"],!1),location:m(i,["location"],""),attendees:i.attendees??[],relatedCustomerCode:m(i,["related_customer_code"],""),relatedOrderId:m(i,["related_order_id"],""),color:m(i,["color"],""),googleEventId:m(i,["google_event_id"],"")}))}async function Bs(e){const{supabaseInsert:t}=await b(async()=>{const{supabaseInsert:l}=await Promise.resolve().then(()=>q);return{supabaseInsert:l}},void 0);return await t("calendar_events",{id:e.id,title:e.title,description:e.description??"",category:e.category,starts_at:e.startsAt,ends_at:e.endsAt||null,is_all_day:e.isAllDay??!1,location:e.location??"",attendees:e.attendees??[],related_customer_code:e.relatedCustomerCode??null,related_order_id:e.relatedOrderId??null,color:e.color??St[e.category],updated_at:new Date().toISOString()})?e:null}async function zs(e){const t="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxvYXJ3bnV5dmZ4aXNjampzbWl6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzYyMTM0MDYsImV4cCI6MjA5MTc4OTQwNn0.7XofVlIiTjlzA9Oz-3lTX1ghLL1qqCcDRadf_a__ys8 ";try{const n=new URL("/rest/v1/calendar_events","https://loarwnuyvfxiscjjsmiz.supabase.co");return n.searchParams.set("id",`eq.${e}`),(await fetch(n.toString(),{method:"DELETE",headers:{apikey:t,Authorization:`Bearer ${t}`}})).ok}catch{return!1}}async function Fa(){return(await x("integration_settings",{order:"name.asc"})).map(t=>({id:m(t,["id"],""),name:m(t,["name"],""),provider:m(t,["provider"],""),config:t.config??{},isEnabled:z(t,["is_enabled"],!1),lastSyncAt:m(t,["last_sync_at"],""),lastStatus:m(t,["last_status"],"")}))}async function Ae(e){const{supabaseInsert:t}=await b(async()=>{const{supabaseInsert:l}=await Promise.resolve().then(()=>q);return{supabaseInsert:l}},void 0);return await t("integration_settings",{id:e.id,name:e.name,provider:e.provider,config:e.config,is_enabled:e.isEnabled,last_sync_at:e.lastSyncAt||null,last_status:e.lastStatus||null,updated_at:new Date().toISOString()})?e:null}async function Vs(e){const t=e.config.shop_domain,n=e.config.admin_token;if(!t||!n)return{count:0,error:"shop_domain と admin_token を設定してください"};try{const l=`https://${t}/admin/api/2024-01/orders.json?status=any&limit=50`,r=await fetch(l,{headers:{"X-Shopify-Access-Token":n,"Content-Type":"application/json"}});if(!r.ok)return{count:0,error:`HTTP ${r.status}`};const c=await r.json(),{supabaseInsert:a}=await b(async()=>{const{supabaseInsert:o}=await Promise.resolve().then(()=>q);return{supabaseInsert:o}},void 0);let i=0;for(const o of c.orders){const d=`shopify_${o.id}`;await a("shopify_orders",{id:d,shopify_order_id:String(o.id),order_number:String(o.order_number??""),order_date:String(o.created_at??new Date().toISOString()),customer_name:String(o.customer?.first_name??"")+" "+String(o.customer?.last_name??""),customer_email:String(o.customer?.email??""),total_amount:Math.round(parseFloat(String(o.total_price??"0"))),financial_status:String(o.financial_status??""),fulfillment_status:String(o.fulfillment_status??"unfulfilled"),line_items:o.line_items??[],shipping_address:o.shipping_address??null,raw_payload:o}),i++}return await Ae({...e,lastSyncAt:new Date().toISOString(),lastStatus:`${i}件取得成功`}),{count:i}}catch(l){return{count:0,error:l instanceof Error?l.message:String(l)}}}async function Js(){return(await x("shopify_orders",{order:"order_date.desc",limit:"50"})).map(t=>({id:m(t,["id"],""),shopifyOrderId:m(t,["shopify_order_id"],""),orderNumber:m(t,["order_number"],""),orderDate:m(t,["order_date"],""),customerName:m(t,["customer_name"],""),customerEmail:m(t,["customer_email"],""),totalAmount:C(t.total_amount),financialStatus:m(t,["financial_status"],""),fulfillmentStatus:m(t,["fulfillment_status"],""),lineItems:t.line_items??[]}))}async function Us(e){const t=e.config.refresh_token,n=e.config.client_id,l=e.config.client_secret;if(!t||!n||!l)return{token:"",error:"refresh_token / client_id / client_secret が未設定です"};const r=await fetch("https://oauth2.googleapis.com/token",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:new URLSearchParams({grant_type:"refresh_token",refresh_token:t,client_id:n,client_secret:l})});if(!r.ok)return{token:"",error:`トークンリフレッシュ失敗: HTTP ${r.status}`};const a=(await r.json()).access_token;return await Ae({...e,config:{...e.config,oauth_token:a}}),e.config.oauth_token=a,{token:a}}async function Ys(e){let t=e.config.oauth_token;const n=e.config.calendar_id||"primary";if(!t&&!e.config.refresh_token)return{count:0,error:"oauth_token または refresh_token を設定してください"};try{const l=new Date().toISOString(),r=new Date(Date.now()+30*86400*1e3).toISOString(),c=`https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(n)}/events?timeMin=${l}&timeMax=${r}&singleEvents=true&orderBy=startTime`;let a=await fetch(c,{headers:{Authorization:`Bearer ${t}`}});if(a.status===401){const u=await Us(e);if(u.error)return{count:0,error:u.error};t=u.token,a=await fetch(c,{headers:{Authorization:`Bearer ${t}`}})}if(!a.ok)return{count:0,error:`HTTP ${a.status}`};const i=await a.json(),{supabaseInsert:o}=await b(async()=>{const{supabaseInsert:u}=await Promise.resolve().then(()=>q);return{supabaseInsert:u}},void 0);let d=0;for(const u of i.items){const p=`gcal_${u.id}`,h=u.start?.dateTime??u.start?.date??"",v=u.end?.dateTime??u.end?.date??"";await o("calendar_events",{id:p,title:String(u.summary??"(無題)"),description:String(u.description??""),category:"general",starts_at:String(h),ends_at:String(v),location:String(u.location??""),google_event_id:String(u.id??""),updated_at:new Date().toISOString()}),d++}return await Ae({...e,lastSyncAt:new Date().toISOString(),lastStatus:`${d}件取得`}),{count:d}}catch(l){return{count:0,error:l instanceof Error?l.message:String(l)}}}async function Hs(){return(await x("fax_inbox",{order:"received_at.desc",limit:"50"})).map(t=>({id:m(t,["id"],""),receivedAt:m(t,["received_at"],""),senderPhone:m(t,["sender_phone"],""),senderName:m(t,["sender_name"],""),imageUrl:m(t,["image_url"],""),ocrStatus:m(t,["ocr_status"],"pending")||"pending",ocrText:m(t,["ocr_text"],""),extractedData:t.extracted_data??{},linkedInvoiceId:m(t,["linked_invoice_id"],"")}))}async function Xs(e,t){const n=e.config.api_key;if(!n)return{text:"",error:"Cloud Vision API key 未設定"};try{const l=`https://vision.googleapis.com/v1/images:annotate?key=${n}`,r=await fetch(l,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({requests:[{image:{content:t.replace(/^data:image\/\w+;base64,/,"")},features:[{type:"DOCUMENT_TEXT_DETECTION"}],imageContext:{languageHints:["ja"]}}]})});return r.ok?{text:(await r.json()).responses?.[0]?.fullTextAnnotation?.text??""}:{text:"",error:`HTTP ${r.status}`}}catch(l){return{text:"",error:l instanceof Error?l.message:String(l)}}}async function Gs(e){const{supabaseInsert:t}=await b(async()=>{const{supabaseInsert:l}=await Promise.resolve().then(()=>q);return{supabaseInsert:l}},void 0);return await t("fax_inbox",{id:e.id,received_at:e.receivedAt,sender_phone:e.senderPhone||null,sender_name:e.senderName||null,image_url:e.imageUrl||null,ocr_status:e.ocrStatus,ocr_text:e.ocrText||null,extracted_data:e.extractedData||null,linked_invoice_id:e.linkedInvoiceId||null})?e:null}const Be={admin:"👑 管理者",manager:"📋 マネージャー",staff:"👤 スタッフ"},ze={all:"全体",sales:"営業",brewery:"蔵人",management:"管理"};async function Qs(){return(await x("user_profiles",{order:"display_name.asc"})).map(t=>({id:m(t,["id"],""),email:m(t,["email"],""),displayName:m(t,["display_name"],""),staffCode:m(t,["staff_code"],""),department:m(t,["department"],"all")||"all",role:m(t,["role"],"staff")||"staff",defaultMailSenderId:m(t,["default_mail_sender_id"],""),phone:m(t,["phone"],""),avatarUrl:m(t,["avatar_url"],""),isActive:z(t,["is_active"],!0),lastSignInAt:m(t,["last_sign_in_at"],""),createdAt:m(t,["created_at"],"")}))}async function Ws(e){if(!e)return null;const t=await x("user_profiles",{email:`eq.${e}`});if(t.length===0)return null;const n=t[0];return{id:m(n,["id"],""),email:m(n,["email"],""),displayName:m(n,["display_name"],""),staffCode:m(n,["staff_code"],""),department:m(n,["department"],"all")||"all",role:m(n,["role"],"staff")||"staff",defaultMailSenderId:m(n,["default_mail_sender_id"],""),phone:m(n,["phone"],""),avatarUrl:m(n,["avatar_url"],""),isActive:z(n,["is_active"],!0),lastSignInAt:m(n,["last_sign_in_at"],"")}}async function Ks(e){const{supabaseInsert:t}=await b(async()=>{const{supabaseInsert:l}=await Promise.resolve().then(()=>q);return{supabaseInsert:l}},void 0);return await t("user_profiles",{id:e.id,email:e.email,display_name:e.displayName,staff_code:e.staffCode||null,department:e.department,role:e.role,default_mail_sender_id:e.defaultMailSenderId||null,phone:e.phone||null,avatar_url:e.avatarUrl||null,is_active:e.isActive,updated_at:new Date().toISOString()})?e:null}async function Zs(e){const t="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxvYXJ3bnV5dmZ4aXNjampzbWl6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzYyMTM0MDYsImV4cCI6MjA5MTc4OTQwNn0.7XofVlIiTjlzA9Oz-3lTX1ghLL1qqCcDRadf_a__ys8 ";try{const n=new URL("/rest/v1/user_profiles","https://loarwnuyvfxiscjjsmiz.supabase.co");return n.searchParams.set("id",`eq.${e}`),(await fetch(n.toString(),{method:"DELETE",headers:{apikey:t,Authorization:`Bearer ${t}`}})).ok}catch{return!1}}async function en(e){const{supabaseInsert:t}=await b(async()=>{const{supabaseInsert:n}=await Promise.resolve().then(()=>q);return{supabaseInsert:n}},void 0);await t("audit_logs",{action:e.action,entity_type:e.entityType??null,entity_id:e.entityId??null,user_email:e.userEmail??null,changes:e.changes??null})}async function tn(e=100){return(await x("audit_logs",{order:"created_at.desc",limit:String(e)})).map(n=>({id:m(n,["id"],""),action:m(n,["action"],""),entityType:m(n,["entity_type"],""),entityId:m(n,["entity_id"],""),userEmail:m(n,["user_email"],""),changes:n.changes??{},createdAt:m(n,["created_at"],"")}))}const Ve={new_order:"🛒 新規受注",payment_overdue:"⚠️ 入金遅延",low_stock:"📦 低在庫",fax_received:"📠 FAX受信",tour_inquiry:"🏭 見学問合せ",new_prospect:"🎯 新規見込客"};async function Ba(){return(await x("slack_notifications",{order:"event_type.asc"})).map(t=>({id:m(t,["id"],""),eventType:m(t,["event_type"],"new_order"),enabled:z(t,["enabled"],!0),channel:m(t,["channel"],""),condition:t.condition??{},lastTriggeredAt:m(t,["last_triggered_at"],"")}))}async function an(e){const{supabaseInsert:t}=await b(async()=>{const{supabaseInsert:l}=await Promise.resolve().then(()=>q);return{supabaseInsert:l}},void 0);return await t("slack_notifications",{id:e.id,event_type:e.eventType,enabled:e.enabled,channel:e.channel,condition:e.condition})?e:null}async function sn(e=50){return(await x("slack_notification_logs",{order:"sent_at.desc",limit:String(e)})).map(n=>({id:m(n,["id"],""),eventType:m(n,["event_type"],""),channel:m(n,["channel"],""),message:m(n,["message"],""),status:m(n,["status"],"sent"),error:m(n,["error"],""),sentAt:m(n,["sent_at"],"")}))}async function nn(e,t,n){const r=(await Fa()).find(d=>d.provider==="slack");if(!r||!r.isEnabled)return{ok:!1,error:"Slack連携が無効です"};const c=r.config.webhook_url;if(!c)return{ok:!1,error:"Webhook URL未設定"};const i=(await Ba()).find(d=>d.eventType===e&&d.enabled);if(!i)return{ok:!1,error:"通知ルールが無効"};const o=n??i.channel??r.config.default_channel??"#general";try{const d=await fetch(c,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:`${Ve[e]} ${t}`,channel:o})}),u=d.ok,{supabaseInsert:p}=await b(async()=>{const{supabaseInsert:h}=await Promise.resolve().then(()=>q);return{supabaseInsert:h}},void 0);return await p("slack_notification_logs",{id:`slack_${Date.now()}`,event_type:e,channel:o,message:t,status:u?"sent":"failed",error:u?null:`HTTP ${d.status}`}),u?{ok:!0}:{ok:!1,error:`HTTP ${d.status}`}}catch(d){return{ok:!1,error:d instanceof Error?d.message:String(d)}}}const Ee={cold:"❄️ 未接触",warm:"🌡️ 関心あり",hot:"🔥 見込み高",contacted:"📞 アプローチ中",negotiating:"💬 商談中",won:"🎉 受注",lost:"💔 失注"},Ge={cold:"#90A4AE",warm:"#FFA726",hot:"#EF5350",contacted:"#42A5F5",negotiating:"#AB47BC",won:"#66BB6A",lost:"#757575"};async function ln(){return(await x("prospects",{order:"updated_at.desc"})).map(t=>({id:m(t,["id"],""),companyName:m(t,["company_name"],""),contactName:m(t,["contact_name"],""),email:m(t,["email"],""),phone:m(t,["phone"],""),address:m(t,["address"],""),website:m(t,["website"],""),businessType:m(t,["business_type"],""),stage:m(t,["stage"],"cold"),source:m(t,["source"],""),expectedAmount:C(t.expected_amount),probability:C(t.probability),assignedStaffCode:m(t,["assigned_staff_code"],""),nextActionDate:m(t,["next_action_date"],""),nextAction:m(t,["next_action"],""),note:m(t,["note"],""),lastContactAt:m(t,["last_contact_at"],""),wonAt:m(t,["won_at"],""),lostAt:m(t,["lost_at"],""),lostReason:m(t,["lost_reason"],""),convertedCustomerCode:m(t,["converted_customer_code"],""),createdAt:m(t,["created_at"],"")}))}async function za(e){const{supabaseInsert:t}=await b(async()=>{const{supabaseInsert:l}=await Promise.resolve().then(()=>q);return{supabaseInsert:l}},void 0);return await t("prospects",{id:e.id,company_name:e.companyName,contact_name:e.contactName||null,email:e.email||null,phone:e.phone||null,address:e.address||null,website:e.website||null,business_type:e.businessType||null,stage:e.stage,source:e.source||null,expected_amount:e.expectedAmount,probability:e.probability,assigned_staff_code:e.assignedStaffCode||null,next_action_date:e.nextActionDate||null,next_action:e.nextAction||null,note:e.note||null,updated_at:new Date().toISOString()})?e:null}async function on(e){const t="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxvYXJ3bnV5dmZ4aXNjampzbWl6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzYyMTM0MDYsImV4cCI6MjA5MTc4OTQwNn0.7XofVlIiTjlzA9Oz-3lTX1ghLL1qqCcDRadf_a__ys8 ";try{const n=new URL("/rest/v1/prospects","https://loarwnuyvfxiscjjsmiz.supabase.co");return n.searchParams.set("id",`eq.${e}`),(await fetch(n.toString(),{method:"DELETE",headers:{apikey:t,Authorization:`Bearer ${t}`}})).ok}catch{return!1}}async function rn(e){return(await x("prospect_activities",{prospect_id:`eq.${e}`,order:"activity_date.desc"})).map(n=>({id:m(n,["id"],""),prospectId:m(n,["prospect_id"],""),activityType:m(n,["activity_type"],"call"),title:m(n,["title"],""),description:m(n,["description"],""),activityDate:m(n,["activity_date"],""),result:m(n,["result"],""),staffCode:m(n,["staff_code"],"")}))}async function cn(e){const{supabaseInsert:t}=await b(async()=>{const{supabaseInsert:l}=await Promise.resolve().then(()=>q);return{supabaseInsert:l}},void 0);return await t("prospect_activities",{id:e.id,prospect_id:e.prospectId,activity_type:e.activityType,title:e.title||null,description:e.description||null,activity_date:e.activityDate,result:e.result||null,staff_code:e.staffCode||null})?e:null}const Va=["瓶 (720ml)","瓶 (1.8L)","瓶 (300ml)","瓶 (500ml)","キャップ・栓","ラベル(表)","ラベル(裏)","首掛け","化粧箱","ダンボール","包装紙","熨斗・水引","和紙","リボン","その他"];async function dn(e){const{supabaseInsert:t}=await b(async()=>{const{supabaseInsert:l}=await Promise.resolve().then(()=>q);return{supabaseInsert:l}},void 0);return await t("materials",{id:e.id,legacy_material_code:e.code,material_code:e.code,name:e.name,unit:e.unit,material_type:e.materialType||null,current_stock:e.currentStock,minimum_stock:e.minimumStock,unit_cost:e.unitCost,last_purchase_date:e.lastUpdated,is_active:!0,updated_at:new Date().toISOString()})?e:null}async function un(e){const t="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxvYXJ3bnV5dmZ4aXNjampzbWl6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzYyMTM0MDYsImV4cCI6MjA5MTc4OTQwNn0.7XofVlIiTjlzA9Oz-3lTX1ghLL1qqCcDRadf_a__ys8 ";try{const n=new URL("/rest/v1/materials","https://loarwnuyvfxiscjjsmiz.supabase.co");return n.searchParams.set("id",`eq.${e}`),(await fetch(n.toString(),{method:"DELETE",headers:{apikey:t,Authorization:`Bearer ${t}`}})).ok}catch{return!1}}async function pn(){return(await x("delivery_locations",{order:"name.asc"})).map(t=>({id:m(t,["id"],""),customerCode:m(t,["customer_code"],""),name:m(t,["name"],""),postalCode:m(t,["postal_code"],""),address:m(t,["address"],""),lat:t.lat?Number(t.lat):void 0,lng:t.lng?Number(t.lng):void 0,contactName:m(t,["contact_name"],""),phone:m(t,["phone"],""),deliveryNote:m(t,["delivery_note"],""),isActive:z(t,["is_active"],!0)}))}async function mn(e){const{supabaseInsert:t}=await b(async()=>{const{supabaseInsert:l}=await Promise.resolve().then(()=>q);return{supabaseInsert:l}},void 0);return await t("delivery_locations",{id:e.id,customer_code:e.customerCode||null,name:e.name,postal_code:e.postalCode||null,address:e.address||null,lat:e.lat??null,lng:e.lng??null,contact_name:e.contactName||null,phone:e.phone||null,delivery_note:e.deliveryNote||null,is_active:e.isActive})?e:null}async function hn(e=50){return(await x("call_logs",{order:"started_at.desc",limit:String(e)})).map(n=>({id:m(n,["id"],""),callDirection:m(n,["call_direction"],"inbound"),fromNumber:m(n,["from_number"],""),toNumber:m(n,["to_number"],""),matchedCustomerCode:m(n,["matched_customer_code"],""),matchedProspectId:m(n,["matched_prospect_id"],""),durationSeconds:C(n.duration_seconds),callStatus:m(n,["call_status"],"answered"),recordingUrl:m(n,["recording_url"],""),transcript:m(n,["transcript"],""),ivryCallId:m(n,["ivry_call_id"],""),startedAt:m(n,["started_at"],""),endedAt:m(n,["ended_at"],""),notes:m(n,["notes"],"")}))}async function Ja(e){const{supabaseInsert:t}=await b(async()=>{const{supabaseInsert:l}=await Promise.resolve().then(()=>q);return{supabaseInsert:l}},void 0);return await t("call_logs",{id:e.id,call_direction:e.callDirection,from_number:e.fromNumber||null,to_number:e.toNumber||null,matched_customer_code:e.matchedCustomerCode||null,matched_prospect_id:e.matchedProspectId||null,duration_seconds:e.durationSeconds??0,call_status:e.callStatus??"answered",started_at:e.startedAt||null,ended_at:e.endedAt||null,notes:e.notes||null,ivry_call_id:e.ivryCallId||null})?e:null}async function yn(e){const t=e.config.api_key,n=e.config.team_id;if(!t||!n)return{count:0,error:"IVRy API key または team_id 未設定"};try{const l=`https://api.ivry.jp/v1/teams/${n}/calls?limit=100`,r=await fetch(l,{headers:{Authorization:`Bearer ${t}`,"Content-Type":"application/json"}});if(!r.ok)return{count:0,error:`HTTP ${r.status}`};const a=(await r.json()).calls??[];let i=0;for(const o of a)await Ja({id:`ivry_${o.id}`,callDirection:String(o.direction??"inbound"),fromNumber:String(o.from??""),toNumber:String(o.to??""),durationSeconds:Number(o.duration??0),callStatus:String(o.status??"answered"),recordingUrl:String(o.recording_url??""),startedAt:String(o.started_at??""),endedAt:String(o.ended_at??""),ivryCallId:String(o.id??"")}),i++;return await Ae({...e,lastSyncAt:new Date().toISOString(),lastStatus:`${i}件取得`}),{count:i}}catch(l){return{count:0,error:l instanceof Error?l.message:String(l)}}}async function vn(e,t){const n=e.config.api_key,l=e.config.team_id;if(!n||!l)return{synced:0,error:"IVRy API key または team_id 未設定"};try{let r=0;for(const c of t){if(!c.phone)continue;(await fetch(`https://api.ivry.jp/v1/teams/${l}/contacts`,{method:"POST",headers:{Authorization:`Bearer ${n}`,"Content-Type":"application/json"},body:JSON.stringify({name:c.name,phone_number:c.phone,external_id:c.customerCode??"",note:c.note??""})})).ok&&r++}return{synced:r}}catch(r){return{synced:0,error:r instanceof Error?r.message:String(r)}}}async function fn(){return(await x("lead_lists",{order:"created_at.desc"})).map(t=>({id:m(t,["id"],""),name:m(t,["name"],""),query:m(t,["query"],""),area:m(t,["area"],""),businessType:m(t,["business_type"],""),totalCount:C(t.total_count),source:m(t,["source"],"manual"),createdAt:m(t,["created_at"],"")}))}async function bn(e){return(await x("lead_items",{list_id:`eq.${e}`,order:"rating.desc.nullslast"})).map(n=>({id:m(n,["id"],""),listId:m(n,["list_id"],""),companyName:m(n,["company_name"],""),address:m(n,["address"],""),phone:m(n,["phone"],""),website:m(n,["website"],""),email:m(n,["email"],""),businessType:m(n,["business_type"],""),rating:n.rating?Number(n.rating):void 0,reviewCount:C(n.review_count),lat:n.lat?Number(n.lat):void 0,lng:n.lng?Number(n.lng):void 0,placeId:m(n,["place_id"],""),status:m(n,["status"],"new"),convertedProspectId:m(n,["converted_prospect_id"],""),note:m(n,["note"],"")}))}async function gn(e){const{supabaseInsert:t}=await b(async()=>{const{supabaseInsert:l}=await Promise.resolve().then(()=>q);return{supabaseInsert:l}},void 0);return await t("lead_lists",{id:e.id,name:e.name,query:e.query||null,area:e.area||null,business_type:e.businessType||null,total_count:e.totalCount,source:e.source})?e:null}async function Ua(e){const{supabaseInsert:t}=await b(async()=>{const{supabaseInsert:l}=await Promise.resolve().then(()=>q);return{supabaseInsert:l}},void 0);return await t("lead_items",{id:e.id,list_id:e.listId,company_name:e.companyName,address:e.address||null,phone:e.phone||null,website:e.website||null,email:e.email||null,business_type:e.businessType||null,rating:e.rating??null,review_count:e.reviewCount??null,lat:e.lat??null,lng:e.lng??null,place_id:e.placeId||null,status:e.status,converted_prospect_id:e.convertedProspectId||null,note:e.note||null})?e:null}async function $n(e,t,n){const l=e.config.api_key;if(!l)return{results:[],error:"Google Maps API key 未設定"};const r=`${t} ${n}`.trim(),c=`https://maps.googleapis.com/maps/api/place/textsearch/json?query=${encodeURIComponent(r)}&language=ja&key=${l}`;try{const a=await fetch(c);if(!a.ok)return{results:[],error:`HTTP ${a.status}`};const i=await a.json();return i.status!=="OK"&&i.status!=="ZERO_RESULTS"?{results:[],error:`API status: ${i.status}`}:{results:i.results.map(d=>{const u=d.geometry?.location;return{id:`place_${d.place_id}`,listId:"",companyName:String(d.name??""),address:String(d.formatted_address??""),rating:d.rating?Number(d.rating):void 0,reviewCount:d.user_ratings_total?Number(d.user_ratings_total):void 0,lat:u?.lat,lng:u?.lng,placeId:String(d.place_id??""),status:"new"}})}}catch(a){return{results:[],error:a instanceof Error?a.message:String(a)}}}async function _n(e){const t={id:`p_${Date.now()}_${Math.random().toString(36).slice(2,8)}`,companyName:e.companyName,phone:e.phone,address:e.address,lat:e.lat,lng:e.lng,businessType:e.businessType,stage:"cold",source:"リスト",expectedAmount:1e5,probability:10,note:e.note??(e.rating?`Google評価: ⭐${e.rating} (${e.reviewCount}件)`:"")},n=await za(t);return n&&await Ua({...e,status:"imported",convertedProspectId:t.id}),n}async function wn(){return(await x("workflow_orders",{order:"order_date.desc"})).map(t=>({id:m(t,["id"],""),orderNo:m(t,["order_no"],""),customerName:m(t,["customer_name"],""),customerCode:m(t,["customer_code"],""),orderDate:m(t,["order_date"],""),deliveryDate:m(t,["delivery_date"],""),stage:m(t,["stage"],"new"),totalAmount:C(t.total_amount),itemCount:C(t.item_count),priority:m(t,["priority"],"normal"),staffName:m(t,["staff_name"],""),notes:m(t,["notes"],"")}))}async function kn(e){const{supabaseInsert:t}=await b(async()=>{const{supabaseInsert:l}=await Promise.resolve().then(()=>q);return{supabaseInsert:l}},void 0);return await t("workflow_orders",{id:e.id,order_no:e.orderNo,customer_name:e.customerName,customer_code:e.customerCode||null,order_date:e.orderDate,delivery_date:e.deliveryDate||null,stage:e.stage,total_amount:e.totalAmount,item_count:e.itemCount,priority:e.priority,staff_name:e.staffName||null,notes:e.notes||null,updated_at:new Date().toISOString()})?e:null}async function Sn(){return(await x("tour_inquiries",{order:"created_at.desc"})).map(t=>({id:m(t,["id"],""),name:m(t,["name"],""),email:m(t,["email"],""),phone:m(t,["phone"],""),visitDate:m(t,["visit_date"],""),partySize:C(t.party_size)||1,language:m(t,["language"],"ja"),purpose:m(t,["purpose"],""),message:m(t,["message"],""),status:m(t,["status"],"new"),repliedAt:m(t,["replied_at"],""),confirmedTime:m(t,["confirmed_time"],""),createdAt:m(t,["created_at"],new Date().toISOString())}))}async function xn(e){const{supabaseInsert:t}=await b(async()=>{const{supabaseInsert:l}=await Promise.resolve().then(()=>q);return{supabaseInsert:l}},void 0);return await t("tour_inquiries",{id:e.id,name:e.name,email:e.email,phone:e.phone||null,visit_date:e.visitDate||null,party_size:e.partySize,language:e.language,purpose:e.purpose||null,message:e.message||null,status:e.status,replied_at:e.repliedAt||null,confirmed_time:e.confirmedTime||null})?e:null}const An=[{table:"sake_sales_document_lines",display:"売上伝票明細"},{table:"sake_purchase_document_lines",display:"仕入伝票明細"},{table:"sake_sales_document_headers",display:"売上伝票ヘッダ"},{table:"sake_purchase_document_headers",display:"仕入伝票ヘッダ"},{table:"sake_inventory_movements_sk",display:"在庫移動(SK)"},{table:"sake_current_stock_sh",display:"在庫(SH)"},{table:"sake_inventory_movements_k5",display:"在庫移動(K5)"},{table:"sake_current_stock_h5",display:"在庫(H5)"},{table:"sake_special_prices_sh",display:"特価(SH)"},{table:"sake_products_sh",display:"商品(SH)"},{table:"sake_special_prices_h5",display:"特価(H5)"},{table:"sake_products_sk",display:"商品(SK)"},{table:"sake_products_k5",display:"商品(K5)"},{table:"sake_products_h5",display:"商品(H5)"},{table:"sake_customers",display:"得意先"},{table:"sake_suppliers",display:"仕入先"},{table:"sake_delivery_destinations",display:"納品先"},{table:"sake_trading_partners",display:"取引先"},{table:"sake_current_stock_sk",display:"在庫(SK)"}];async function Ya(){return(await Promise.all(An.map(async t=>{const[n,l]=await Promise.all([mt(t.table),x(t.table,{select:"_synced_at",order:"_synced_at.desc",limit:"1"})]);return{tableName:t.table,displayName:t.display,rowCount:n,lastSyncAt:l[0]?._synced_at??null}}))).sort((t,n)=>n.rowCount-t.rowCount)}async function Me(e,t,n=100){const l=(t-1)*n,[r,c]=await Promise.all([x(e,{select:"_source_file,_record_index,_record_size,_raw_b64,_source_path,_source_file_mtime,_synced_at",order:"_record_index.asc",limit:String(n),offset:String(l)}),mt(e)]);return{records:r,total:c}}async function rt(e){const t=await x("customers",{select:"memo",or:`legacy_customer_code.eq.${e},customer_code.eq.${e}`,limit:"1"});if(t.length===0)return"";const n=t[0].memo;if(typeof n=="string"&&n)try{const l=JSON.parse(n);return String(l.price_group??"")}catch{return""}return""}async function Ha(e,t){if(e){const l=await x("customer_product_prices",{select:"special_price",price_group:`eq.${e}`,legacy_product_code:`eq.${t}`,limit:"1"});if(l.length>0&&l[0].special_price)return C(l[0].special_price)}const n=await x("products",{select:"default_sale_price",or:`legacy_product_code.eq.${t},product_code.eq.${t}`,limit:"1"});return n.length>0&&n[0].default_sale_price?C(n[0].default_sale_price):0}const w=Object.freeze(Object.defineProperty({__proto__:null,CALENDAR_CATEGORY_COLORS:St,CALENDAR_CATEGORY_LABELS:kt,DEPT_LABELS:ze,INVOICE_TYPE_LABELS:it,JIKOMI_STATUS_LABELS:Ca,MATERIAL_CATEGORIES:Va,PROSPECT_STAGE_COLORS:Ge,PROSPECT_STAGE_LABELS:Ee,ROLE_LABELS:Be,SEASONAL_TEMPLATES:yt,SLACK_EVENT_LABELS:Ve,TAX_DEDUCTION_LABELS:ot,TAX_RATE_CATEGORIES:Ra,convertLeadToProspect:_n,deleteCalendarEvent:zs,deleteMailSender:Ms,deleteMaterial:un,deletePrintLayout:Rs,deleteProspect:on,deleteUserProfile:Zs,fetchAuditLogs:tn,fetchBillList:Ta,fetchBillingSummary:$t,fetchCalendarEvents:Fs,fetchCallLogs:hn,fetchCustomerAnalysis:Aa,fetchCustomerLedger:bt,fetchCustomerPriceGroup:rt,fetchCustomerPricing:ka,fetchDeliveryLocations:pn,fetchDeliveryNote:gt,fetchFaxInbox:Hs,fetchIntegrationSettings:Fa,fetchInvoices:Fe,fetchJikomiList:Ia,fetchKenteiList:Da,fetchLeadItems:bn,fetchLeadLists:fn,fetchMailSenders:js,fetchMasterStats:va,fetchMaterialList:lt,fetchMyProfile:Ws,fetchPayableList:qa,fetchPaymentStatus:ya,fetchPipelineMeta:fa,fetchPrintLayouts:Ts,fetchProductABC:Ea,fetchProductPrice:Ha,fetchProspectActivities:rn,fetchProspects:ln,fetchPurchaseList:Pa,fetchRawMaterialStock:Na,fetchRawRecords:Me,fetchRawTableList:Ya,fetchSalesAnalytics:xe,fetchSalesReport:Xe,fetchSalesSummary:ha,fetchShopifyOrders:Js,fetchSlackLogs:sn,fetchSlackRules:Ba,fetchStoreOrders:Oa,fetchStoreSales:wt,fetchSyncDashboard:ba,fetchTankList:La,fetchTaxDeclaration:_t,fetchTourInquiriesFromDb:Sn,fetchUserProfiles:Qs,fetchWorkflowOrdersFromDb:wn,generateTaxCSV:Ds,generateTaxXML:ja,ocrFaxImage:Xs,recalculateTaxDeclaration:Ps,recordAudit:en,resolveProductPrice:Sa,saveCalendarEvent:Bs,saveCallLog:Ja,saveDeliveryLocation:mn,saveEmailCampaign:Oe,saveFaxRecord:Gs,saveIntegrationSetting:Ae,saveInvoice:ga,saveLeadItem:Ua,saveLeadList:gn,saveMailSender:Os,saveMaterial:dn,savePrintLayout:Ns,saveProspect:za,saveProspectActivity:cn,saveSlackRule:an,saveTaxDeclaration:qs,saveTourInquiry:xn,saveUserProfile:Ks,saveWorkflowOrder:kn,searchPlaces:$n,sendEmailCampaign:Ma,sendSlackNotification:nn,submitFeatureRequest:$a,syncGoogleCalendar:Ys,syncIvryCallLogs:yn,syncPhoneBookToIvry:vn,syncShopifyOrders:Vs,updateCustomer:_a,updateProduct:wa},Symbol.toStringTag,{value:"Module"}));function le(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}const En={open:"未締め",closed:"締め済"};function Cn(e,t){const n=e.customers.map(l=>`
      <tr>
        <td>
  ...
  `}const Tn={forecasts:[],deliveries:[],calendarMonth:new Date().toISOString().slice(0,7),selectedSegment:"all"},Ot={monthly:"月次生産","made-to-order":"受注生産","november-only":"11月限定","annual-batch":"年間一括"},me={monthly:"#0F5B8D","made-to-order":"#6b46c1","november-only":"#c05621","annual-batch":"#2f855a"};function Nn(e){const t=e.filter(r=>r>0).length,n=e.reduce((r,c)=>r+c,0),l=n>0?e[10]/n:0;return n>0&&e[11]/n,l>.6?"november-only":t<=3?"made-to-order":t<=4&&l<.3?"annual-batch":"monthly"}function Ga(e,t){if(t==="monthly"){const r=e.filter((a,i)=>i!==11).filter(a=>a>0);if(r.length===0)return 0;const c=[...r].sort((a,i)=>a-i);if(c.length>2){const a=c.slice(0,-1);return Math.round(a.reduce((i,o)=>i+o,0)/a.length)}return Math.round(r.reduce((a,i)=>a+i,0)/r.length)}if(t==="november-only")return e[10];if(t==="annual-batch")return e.reduce((l,r)=>l+r,0);const n=e.filter(l=>l>0);return n.length>0?Math.round(n.reduce((l,r)=>l+r,0)/n.length):0}function Rn(e,t){const n=(new Date().getMonth()+1)%12;if(t==="november-only")return n===10?e[10]:0;if(t==="annual-batch"){const l=e.indexOf(Math.max(...e));return n===l?e.reduce((r,c)=>r+c,0):0}return t==="made-to-order"?0:n===11?e[11]:Ga(e,t)}function jn(e,t,n){const l=new Array(12).fill(0);t.forEach(c=>{const a=parseInt(c.month.slice(5,7))-1;l[a]+=c.amount});const r=l.reduce((c,a)=>c+a,0);return e.slice(0,30).map(c=>{const a=l.map(v=>r===0?0:Math.round(c.quantity*(v/r))),i=Nn(a),o=Math.round(c.quantity/12),d=Ga(a,i),u=Rn(a,i),p=i==="monthly"?d*11+a[11]:c.quantity,h=i==="monthly"?Math.round(d*1.5):i==="november-only"?Math.round(a[10]*.1):0;return{code:c.code,name:c.name,segment:i,monthlyShipments:a,avgMonthly:o,adjustedAvg:d,nextMonthForecast:u,annualForecast:p,safetyStock:h}})}function On(e){const[t,n]=e.split("-").map(Number);return new Date(t,n,0).getDate()}function Mn(e){const[t,n]=e.split("-").map(Number);return new Date(t,n-1,1).getDay()}function Qa(e,t){const n=On(t),l=Mn(t),[r,c]=t.split("-").map(Number),a=new Map;e.forEach(S=>{if(S.date.slice(0,7)===t){const T=S.date.slice(0,10);a.has(T)||a.set(T,[]),a.get(T).push(S)}});const i=new Date().toISOString().slice(0,10),o=["日","月","火","水","木","金","土"].map(S=>`<th class="dcal-header">${S}</th>`).join("");let d="",u=1;for(let S=0;S<6&&!(u>n&&S>0);S++){d+="<tr>";for(let T=0;T<7;T++)if(S===0&&T<l||u>n)d+='<td class="dcal-cell dcal-empty"></td>';else{const N=`${r}-${String(c).padStart(2,"0")}-${String(u).padStart(2,"0")}`,E=a.get(N)||[],k=N===i,j=E.length>0?E.some(M=>M.status==="scheduled")?'<span class="dcal-dot scheduled"></span>':E.some(M=>M.status==="dispatched")?'<span class="dcal-dot dispatched"></span>':'<span class="dcal-dot delivered"></span>':"";d+=`
          <td class="dcal-cell ${k?"dcal-today":""}">
            ...
            ${j}
          </td>`,u++}d+="</tr>"}const[p,h]=c===1?[r-1,12]:[r,c-1],[v,f]=c===12?[r+1,1]:[r,c+1],_=`${p}-${String(h).padStart(2,"0")}`,g=`${v}-${String(f).padStart(2,"0")}`;return`
    <section class="panel">
      <div class="panel-header">
        <div>
          <h2>納品カレンダー</h2>
        ...
        <table class="forecast-table">
          <thead>
            <tr>
              ...
              <th>商品名</th>
              <th>区分</th>
              ${Mt.map((i,o)=>`<th class="forecast-month-th ${o===c?"current-month":""}">${i}</th>`).join("")}
  ...
  `}function Yn(e,t,n,l,r="month",c){const a={success:"正常",warning:"注意",error:"異常",running:"実行中"},i=Vn(e.allDailySales,r,c),o=i.reduce((E,k)=>E+k.amount,0),d=i.reduce((E,k)=>E+k.bottles,0),u=i.reduce((E,k)=>E+k.volumeMl,0),p=i.length,h=d>0?Math.round(o/d):0,v=u>0?Math.round(o/(u/1e3)):0,_=(i.length>0?e.allDailySales.filter(E=>{const k=i[0]?.date??"",j=i[i.length-1]?.date??"",M=Ft(k,-1),pe=Ft(j,-1);return E.date>=M&&E.date<=pe}):[]).reduce((E,k)=>E+k.amount,0),g=_>0?(o-_)/_*100:0,S=g>0?"+":"",T=e.salesRecords.slice(0,10).map(E=>`
            <tr>
    ...
    <section class="kpi-grid">
      ...
      <article class="panel kpi-card">
        ...
        <p class="kpi-value" style="color:${g>=0?"#2f855a":"#c53d3d"}">${_>0?`${S}${g.toFixed(1)}%`:"―"}</p>
        <p class="kpi-sub">前年同期 ${_>0?Y(_):"データなし"}</p>
      </article>
    ...
    <section class="panel">
      ...
      <div class="table-wrap">
        <table>
          ...
          <tbody>${i.slice().reverse().slice(0,31).map(E=>`
            <tr>
  ...
  `}function Hn(e){const t=e.prospects.reduce((a,i)=>a+i.expectedAmount*i.probability/100,0),n=e.prospects.filter(a=>a.stage==="hot"||a.stage==="negotiating").length,l=new Date().toISOString().slice(0,10),r=e.upcomingEvents.filter(a=>a.startsAt.slice(0,10)>=l).slice(0,5),c=e.tourInquiries.filter(a=>a.status==="new").length;return`
    <section class="kpi-grid compact">
      <article class="panel kpi-card">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(100px, 1fr));gap:8px;">
          ${["cold","warm","hot","contacted","negotiating","won"].map(a=>{const i=e.prospects.filter(d=>d.stage===a).length,o=e.prospects.filter(d=>d.stage===a).reduce((d,u)=>d+u.expectedAmount,0);return`
              <div style="background:${Ge[a]};color:white;padding:12px;border-radius:6px;text-align:center;">
                <div style="font-size:11px;">${Ee[a]}</div>
    ...
    ${e.deliveries&&e.deliveries.length>0?zn(e.deliveries,e.deliveryCalendarMonth||new Date().toISOString().slice(0,7)):""}
  `}function Xn(e){return e?new Intl.DateTimeFormat("ja-JP",{year:"numeric",month:"long",day:"numeric"}).format(new Date(e)):""}function oe(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function Gn(e,t){const n=e.lines.length?e.lines.map((r,c)=>`
          <tr>
      ...
      <div class="delivery-header">
        ...
        <div class="delivery-meta">
          <table class="delivery-meta-table">
            ...
            <tr><th>納品日</th><td>${Xn(e.invoiceDate)}</td></tr>
  ...
  `}function X(e){return e.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}function Qn(e){return X(e).replaceAll(`
`,"<br />")}function Wn(e){const n=[...Object.values(yt),{id:"custom",season:"カスタム",subject:"",body:""}].map(r=>`
        <button
  ...
  `}function Kn(e,t){const n=[De("得意先",t.customers.map(r=>`
          <button
  ...
  `}function Pe(e){return e.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}function Bt(e){return e.trim().toLowerCase()}function Zn(e,t){const n=Bt(t),l=e.filter(c=>n?[c.code,c.name,c.name].map(Bt).some(a=>a.includes(n)):!0).slice(0,50),r=l.length?`
        <div class="table-wrap">
          <table>
            <thead>
              <tr>
      ...
      `:"";return Wa({title:"得意先検索",searchQuery:t,placeholder:"コード・名前で検索",resultsHtml:r,emptyMessage:"該当する得意先が見つかりません。"})}function ei(e){return e.toISOString().slice(0,10)}function ie(e){return e.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}function ee(e,t){return e[t]?`<div class="field-error">${ie(e[t])}</div>`:""}function re(e,t,n=""){return[n,e[t]?"has-error":""].filter(Boolean).join(" ")}function ti(e,t,n,l){const r=Object.keys(it).map(o=>`<option value="${o}" ${e.invoiceType===o?"selected":""}>${it[o]}</option>`).join(""),c=e.lines.map((o,d)=>`
      <tr>
        <td>
          <div class="input-group">
            <input class="${re(l,`lines.${d}.productCode`,"input-cell")}" type="text" data-line="${d}" data-field="productCode" value="${ie(o.productCode)}" placeholder="P00001" />
        ...
        <td>
          <input class="${re(l,`lines.${d}.productName`,"input-cell")}" type="text" data-line="${d}" data-field="productName" value="${ie(o.productName)}" placeholder="商品名" />
          ${ee(l,`lines.${d}.productName`)}
        </td>
        <td>
          <input class="${re(l,`lines.${d}.quantity`,"input-cell numeric")}" type="number" data-line="${d}" data-field="quantity" value="${o.quantity}" min="0" />
          ${ee(l,`lines.${d}.quantity`)}
        </td>
        <td><input class="input-cell" type="text" data-line="${d}" data-field="unit" value="${o.unit}" placeholder="本" /></td>
        <td>
          <input class="${re(l,`lines.${d}.unitPrice`,"input-cell numeric")}" type="number" data-line="${d}" data-field="unitPrice" value="${o.unitPrice}" min="0" />
    ...
    <section class="panel">
      ...
      <div class="filter-grid filter-grid--wide">
        ...
        <label class="field">
          <span>伝票日付</span>
          <input class="${re(l,"invoiceDate")}" id="inv-date" type="date" value="${e.invoiceDate||ei(new Date)}" />
          ${ee(l,"invoiceDate")}
        </label>
        <label class="field">
          <span>得意先コード</span>
  ...
  `}function ii(e,t,n,l,r,c){const a=e.lines.reduce((h,v)=>h+v.amount,0),i=Math.round(a*e.taxRate/100),o=a+i,d=l.length>=1?t.filter(h=>h.name.includes(l)||h.code.includes(l)).slice(0,8):[],u=r.length>=1?n.filter(h=>h.name.includes(r)||h.code.includes(r)).slice(0,8):[],p=e.fieldConfig;return e.previewMode?`
      <section class="page-head">
        <div>
    ...
    <section class="panel">
      ...
      ${u.length>0?`
        <div class="search-results">
          ${u.map(h=>{const v=c?Sa(h,c):{price:h.salePrice||0,label:"標準価格"},f=v.label!=="標準価格";return`
    ...
    ${si(p)}
    ${ni(e.sealSettings)}
  `}function li(e){const t=Za(e),n=window.open("","_blank","width=800,height=1100");if(!n){alert("ポップアップがブロックされました。許可してください。");return}n.document.write(`<!DOCTYPE html>
<html lang="ja">
<head>
<meta charset="UTF-8" />
...
<\/script>
...
</html>`),n.document.close()}function oi(e){return new Intl.DateTimeFormat("ja-JP",{year:"numeric",month:"2-digit",day:"2-digit"}).format(new Date(e))}function ri(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function ci(e,t){const n=e.length?e.map(l=>`
            <tr>
              ...
              <td class="numeric">${ri(l.amount)}</td>
            </tr>
  ...
  `}function di(e){return new Date(e.getFullYear(),e.getMonth(),1)}function ui(e,t){return new Date(e.getFullYear(),e.getMonth()+t,1)}function es(e,t){const n=new Date(e);return n.setDate(n.getDate()+t),n}function ts(e){const t=new Date(e),n=t.getDay();return t.setDate(t.getDate()-n),t.setHours(0,0,0,0),t}function zt(e){const t=es(ts(e),6);return t.setHours(23,59,59,999),t}function Vt(e){return new Date(`${e}T00:00:00`)}function Jt(e){return`${e.getMonth()+1}/${e.getDate()}`}function pi(e){return e.replaceAll("&","&amp;").replaceAll('"',"&quot;").replaceAll("<","&lt;")}function mi(){const e=new Date,t=ts(ui(di(e),-3)),n=zt(new Date(e.getFullYear(),e.getMonth()+4,0)),l=[];let r=new Date(t);for(;r<=n;){const c=zt(r);l.push({start:new Date(r),end:c,label:`${Jt(r)} - ${Jt(c)}`}),r=es(r,7)}return l}function hi(e){const t=mi(),n=`160px repeat(${t.length}, minmax(56px, 1fr))`,l=t.map(c=>`
        <div class="gantt-week">
          <span>${c.label}</span>
        </div>
      `).join(""),r=e.length?e.map(c=>{const a=Vt(c.startDate),i=Vt(c.expectedDoneDate),o=Math.max(0,t.findIndex(p=>p.end>=a)),d=Math.max(o,t.reduce((p,h,v)=>h.start<=i?v:p,o)),u=[`仕込番号: ${c.jikomiNo}`,`銘柄: ${c.productName}`,`期間: ${c.startDate} - ${c.expectedDoneDate}`,`タンク: ${c.tankNo}`,`備考: ${c.note||"なし"}`].join(`
`);return`
  ...
  `}function Ut(e,t){const n={planned:"neutral",active:"warning",done:"success"},l=e.map(i=>`
      <tr>
        ...
        <td>
          <span class="status-pill ${n[i.status]}">${Ca[i.status]}</span>
        </td>
        <td>${i.note||"―"}</td>
      </tr>
  ...
  `}function yi(e){const t={pending:"未実施",submitted:"申請中",approved:"承認済"},n={pending:"neutral",submitted:"warning",approved:"success"},l=e.map(o=>`
      <tr>
  ...
  `}function vi(e){return e.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}function fi(e,t){return`
  ...
  `}function _i(e,t){const n=[...new Set(e.map(r=>r.businessType).filter(Boolean))].sort(),l=[...new Set(e.map(r=>r.areaCode).filter(Boolean))].sort();return`
  ...
  `}function as(e,t){return!e||e.length<=t?e||"":e.slice(0,t)+"…"}function wi(e){return e.map(t=>`
        <tr>
      ...
      `).join("")}function ki(e){return e.map(t=>`
        <tr>
          <td class="mono">${t.code}</td>
          <td>${as(t.name,25)}</td>
          <td>${t.category}</td>
      ...
      `).join("")}function Si(e,t,n=xt){const{filtered:l,paged:r,totalPages:c}=$i(e.customers,n);return`
    <section class="page-head">
      <div>
      ...
      `:`
        <div class="table-wrap">
          <table>
            ...
            <tbody>${ki(e.products)}</tbody>
          </table>
        </div>
      `}
    </section>
  `}function We(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function xi(e,t){if(!e&&!t)return"";const n=e;return`
  ...
  `}function Ai(e){const t=e.map(r=>{const a=(r.minimumStock>0?r.currentStock/r.minimumStock:1/0)<1.5;return`
        <tr>
  ...
  `}function Ei(e){return e?new Intl.DateTimeFormat("ja-JP",{year:"numeric",month:"2-digit",day:"2-digit"}).format(new Date(e)):"-"}function Ke(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}const Ci={unpaid:"未入金",partial:"一部入金",paid:"入金済"};function Ii(e){return`
    <section class="page-head">
      <div>
    ...
    <section class="panel">
      ...
      <div class="table-wrap">
        ...
        <tr>
          ...
          <td>${Ei(n.lastPaymentDate)}</td>
          <td><span class="status-pill ${n.status==="paid"?"success":n.status==="partial"?"warning":"danger"}">${Ci[n.status]}</span></td>
        </tr>
      `).join("")}</tbody>
        </table>
      </div>
    </section>
  `}function ce(e){return e.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}function Ht(e){return e.trim().toLowerCase()}function Li(e,t){const n=Ht(t),l=e.filter(c=>n?[c.code,c.name,c.janCode].map(Ht).some(a=>a.includes(n)):!0),r=l.length?`
        <div class="table-wrap">
          <table>
            <thead>
              <tr>
  ...
  `}function ve(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function Pi(e,t){const n={holding:"保有中",due:"期日到来",cleared:"決済済"},l={holding:"neutral",due:"warning",cleared:"success"},r=e.map(u=>`
      <tr>
  ...
  `}function Ne(e){if(!e)return"error";const n=(Date.now()-new Date(e).getTime())/(1e3*60*60);return n<1?"success":n<24?"warning":"error"}function Xt(e){if(!e)return"未同期";const n=(Date.now()-new Date(e).getTime())/(1e3*60*60);return n<1?"正常":n<24?"注意":"要確認"}function Ti(e){return`
    <section class="kpi-grid compact">
      <article class="panel kpi-card">
  ...
  `}function Ni(e,t,n,l){const r={success:"正常",warning:"注意",error:"異常",running:"実行中"};return`
    <section class="page-head">
      <div>
...
cd sake-system\\relay`,success:["エクスプローラーで sake-system\\\\relay フォルダが見える状態になっていれば成功です。","cd を実行したあと、コマンドプロンプトの行頭が relay フォルダを指していれば問題ありません。"],errors:["git コマンドが使えない場合は Git for Windows が未導入です。先に Git をインストールしてください。","アクセス拒否が出る場合は C:\\\\Program Files 配下ではなく C:\\\\sake-relay\\\\ などに配置してください。"]})}
  ...
  `}function Oi(e){if(e.monthlyByCustomer.length===0)return'<p class="empty-row">データなし</p>';const t=e.months.map(l=>`<th class="numeric">${l}</th>`).join(""),n=e.monthlyByCustomer.map(l=>{const r=l.values.reduce((a,i)=>a+i,0),c=l.values.map(a=>`<td class="numeric">${a>0?(a/1e4).toFixed(0)+"万":"—"}</td>`).join("");return`
        <tr>
          <td>${l.label}</td>
          ${c}
  ...
  `}function Mi(e){e.ranking.reduce((o,d)=>o+d.amount,0);const t=e.ranking.filter(o=>o.abcRank==="A").length,n=e.ranking.filter(o=>o.abcRank==="B").length,l=e.ranking.filter(o=>o.abcRank==="C").length,r=e.ranking.filter(o=>o.abcRank==="A").reduce((o,d)=>o+d.amount,0),c=e.ranking.filter(o=>o.abcRank==="B").reduce((o,d)=>o+d.amount,0),a=e.ranking.filter(o=>o.abcRank==="C").reduce((o,d)=>o+d.amount,0),i=e.ranking.map(o=>`
        <tr>
          ...
          <td><span class="status-pill ${Ri(o.abcRank)}">${o.abcRank}</span></td>
        </tr>
      `).join("");return`
    <section class="page-head">
      <div>
  ...
  `}function Gt(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function Fi(e){switch(e){case"A":return"success";case"B":return"warning";case"C":return"neutral"}}function Bi(e){if(e.length===0)return'<div class="chart-empty">データなし</div>';const t=e.slice(0,20),n=760,l=320,r={top:24,right:56,bottom:60,left:72},c=n-r.left-r.right,a=l-r.top-r.bottom,i=Math.max(...t.map(f=>f.amount),1),o=c/t.length,d=[0,.25,.5,.75,1].map(f=>{const _=r.top+a-a*f,g=`${Math.round(i*f/1e4)}万`;return`
        <g>
          <line x1="${r.left}" y1="${_}" x2="${n-r.right}" y2="${_}" class="chart-grid" />
          <text x="4" y="${_+4}" class="chart-axis">${g}</text>
        </g>
      `}).join(""),u=[0,25,50,70,90,100].map(f=>{const _=r.top+a-a*f/100,g=f===70||f===90;return`
        <g>
          <text x="${n-4}" y="${_+4}" class="chart-axis" text-anchor="end">${f}%</text>
          ${g?`<line x1="${r.left}" y1="${_}" x2="${n-r.right}" y2="${_}" stroke="${f===70?"#2F855A":"#B7791F"}" stroke-dasharray="6 3" stroke-width="1.5" opacity="0.6" />`:""}
        </g>
      `}).join(""),p=t.map((f,_)=>{const g=f.amount/i*a,S=Math.max(o-10,16),T=r.left+_*o+(o-S)/2,N=r.top+a-g,E=f.abcRank==="A"?"#2F855A":f.abcRank==="B"?"#B7791F":"#718096",k=f.name.length>6?f.name.slice(0,6)+"…":f.name;return`
        <g>
          <rect x="${T}" y="${N}" width="${S}" height="${g}" rx="4" fill="${E}" opacity="0.8" />
          <text x="${T+S/2}" y="${l-8}" class="chart-axis centered-axis pareto-label" transform="rotate(-35 ${T+S/2} ${l-16})">${k}</text>
        </g>
      `}).join(""),h=t.map((f,_)=>{const g=r.left+_*o+o/2,S=r.top+a-a*f.cumRatio/100;return`${g},${S}`}).join(" "),v=t.map((f,_)=>{const g=r.left+_*o+o/2,S=r.top+a-a*f.cumRatio/100;return`<circle cx="${g}" cy="${S}" r="3.5" fill="#C53D3D" />`}).join("");return`
  ...
  `}function zi(e){const t=e.ranking.filter(i=>i.abcRank==="A").length,n=e.ranking.filter(i=>i.abcRank==="B").length,l=e.ranking.filter(i=>i.abcRank==="C").length,r=e.ranking.filter(i=>i.abcRank==="A").reduce((i,o)=>i+o.amount,0),c=e.ranking.map(i=>`
        <tr>
          <td class="mono">${i.code}</td>
          <td>${i.name}</td>
          ...
          <td><span class="status-pill ${Fi(i.abcRank)}">${i.abcRank}</span></td>
        </tr>
      `).join(""),a=e.monthlyByProduct.length>0?(()=>{const i=e.months.map(d=>`<th class="numeric">${d}</th>`).join(""),o=e.monthlyByProduct.map(d=>{const u=d.values.reduce((h,v)=>h+v,0),p=d.values.map(h=>`<td class="numeric">${h>0?(h/1e4).toFixed(0)+"万":"—"}</td>`).join("");return`<tr><td>${d.label}</td>${p}<td class="numeric"><strong>${Gt(u)}</strong></td></tr>`}).join("");return`
          <div class="table-wrap">
            <table>
  ...
  `}function Vi(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function Ji(e){return e.replace("-","/")}function Ui(e){if(e.length===0)return'<div class="chart-empty">データなし</div>';const t=760,n=280,l={top:16,right:24,bottom:36,left:64},r=t-l.left-l.right,c=n-l.top-l.bottom,a=Math.max(...e.map(u=>u.amount),1),i=r/e.length,o=[0,.25,.5,.75,1].map(u=>{const p=l.top+c-c*u,h=`${Math.round(a*u/1e4).toLocaleString("ja-JP")}万円`;return`
        <g>
          <line x1="${l.left}" y1="${p}" x2="${t-l.right}" y2="${p}" class="chart-grid" />
          <text x="8" y="${p+4}" class="chart-axis">${h}</text>
        </g>
      `}).join(""),d=e.map((u,p)=>{const h=u.amount/a*c,v=Math.max(i-18,24),f=l.left+p*i+(i-v)/2,_=l.top+c-h;return`
        <g>
          <rect x="${f}" y="${_}" width="${v}" height="${h}" rx="6" class="analytics-bar" />
          <text x="${f+v/2}" y="${n-10}" class="chart-axis centered-axis">${Ji(u.month)}</text>
        </g>
      `}).join("");return`
  ...
  `}function Yi(e){return e.length===0?'<tr><td colspan="5" class="empty-row">データなし</td></tr>':e.map(t=>`
        <tr>
          <td class="mono">${t.code}</td>
          <td>${t.name}</td>
          <td class="numeric">${Vi(t.amount)}</td>
          <td class="numeric">${t.quantity.toLocaleString("ja-JP")}</td>
          <td class="numeric">${t.documents.toLocaleString("ja-JP")}</td>
        </tr>
      `).join("")}function Hi(e,t){const n=t==="products"?"商品別集計":"得意先別集計",l=t==="products"?e.productTotals:e.customerTotals;return`
    <section class="page-head">
      <div>
  ...
  `}function fe(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function Xi(e){const t=Math.max(...e.salesByProduct.flatMap(c=>c.values),1),n=e.salesByProduct.map(c=>{const a=c.values.map((i,o)=>`
          <div class="bar-col">
  ...
  `}function Gi(e){return new Intl.DateTimeFormat("ja-JP",{year:"numeric",month:"2-digit",day:"2-digit"}).format(new Date(e))}function Qi(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function Qt(e){return e.toISOString().slice(0,10)}function Wi(e,t,n){const l=e.length?e.map(r=>`
            <tr>
              ...
              <td class="numeric">${Qi(r.amount)}</td>
            </tr>
  ...
  `}function Re(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function Ki(e,t,n,l){const r={cash:"現金",card:"カード",paypay:"PayPay",other:"その他"},c={new:"新規",processing:"処理中",shipped:"発送済",delivered:"配達済"},a={new:"warning",processing:"neutral",shipped:"success",delivered:"success"},i=e.map(p=>`
      <tr>
        <td>${p.saleTime}</td>
  ...
  `}const Ze={customers:"得意先マスタ",products:"商品マスタ",suppliers:"仕入先マスタ",staff:"担当者マスタ"},Zi={customers:{required:["legacy_customer_code","name"],optional:["kana_name","phone","postal_code","address1","address2","closing_day","payment_day","email"]},products:{required:["legacy_product_code","name"],optional:["kana_name","jan_code","category_code","volume_ml","alcohol_degree","list_price","default_sale_price"]},suppliers:{required:["legacy_supplier_code","name"],optional:["kana_name","phone","postal_code","address1","closing_day","payment_day","email"]},staff:{required:["legacy_staff_code","name"],optional:["kana_name","department"]}};function el(e,t,n,l){const r=Zi[e],c=Object.keys(Ze).map(i=>`
  ...
  `}const nl={new:{label:"新規受注",color:"#2196F3",icon:"📥"},picking:{label:"ピッキング中",color:"#FF9800",icon:"🔍"},packed:{label:"梱包完了",color:"#4CAF50",icon:"📦"},shipped:{label:"発送済",color:"#9C27B0",icon:"🚚"},delivered:{label:"配達完了",color:"#4CAF50",icon:"✅"}},il=["new","picking","packed","shipped","delivered"];function ll(e){const t={new:[],picking:[],packed:[],shipped:[],delivered:[]};e.forEach(c=>t[c.stage].push(c));const n=il.map(c=>{const a=nl[c],i=t[c];return`
  ...
  `}function ol(e,t,n){const l=e.cart.reduce((c,a)=>c+a.amount,0);return`
  ...
  `}function rl(e,t,n){if(e.step==="customer"){const l=e.customerQuery.toLowerCase(),r=l?t.filter(c=>c.name.toLowerCase().includes(l)||c.code.toLowerCase().includes(l)):t.slice(0,20);return`
      <section class="panel">
  ...
  `}const Wt={new:"新規",replied:"返信済",confirmed:"確定",completed:"来訪済",cancelled:"キャンセル"},Kt={new:"warning",replied:"neutral",confirmed:"success",completed:"success",cancelled:"neutral"},Zt={ja:"🇯🇵 日本語",en:"🇬🇧 English",zh:"🇨🇳 中文",ko:"🇰🇷 한국어"};function cl(e,t){const n=e.find(c=>c.id===t)??e[0],l=e.filter(c=>c.status==="new").length,r=e.filter(c=>c.status==="confirmed").length;return`
    <section class="page-head">
      <div>
          ...
          ${e.map(c=>`
            <button class="tour-item ${n?.id===c.id?"active":""}" data-tour-id="${c.id}">
              <div class="tour-item-head">
                ...
                <span class="status-pill ${Kt[c.status]}">${Wt[c.status]}</span>
              </div>
              <div class="tour-item-sub">
                ${Zt[c.language]} · 👥 ${c.partySize}名
              </div>
...
金井酒造店`;function pl(e,t){const n=t?e.find(r=>r.id===t):null,l=t==="__new__";return`
    <section class="page-head">
      <div>
    ...
    ${n||l?`
      ...
      `:""}
  `}function ml(e,t,n,l){const[r,c]=t.split("-").map(k=>parseInt(k,10)),a=new Date(r,c-1,1),i=new Date(r,c,0),o=a.getDay(),d=i.getDate(),u=[];for(let k=0;k<o;k++)u.push({isOutside:!0});for(let k=1;k<=d;k++)u.push({date:new Date(r,c-1,k)});for(;u.length%7!==0;)u.push({isOutside:!0});const p=n?e.filter(k=>k.category===n):e,h={};p.forEach(k=>{const j=k.startsAt.slice(0,10);h[j]??=[],h[j].push(k)});const v=new Date().toISOString().slice(0,10),f=u.map(k=>{if(k.isOutside)return'<div class="cal-cell cal-outside"></div>';const j=k.date,M=`${j.getFullYear()}-${String(j.getMonth()+1).padStart(2,"0")}-${String(j.getDate()).padStart(2,"0")}`,pe=h[M]??[],ms=M===v,Lt=j.getDay();return`
        <div class="cal-cell ${ms?"cal-today":""} ${Lt===0?"cal-sun":Lt===6?"cal-sat":""}"
             data-cal-date="${M}">
      ...
      `}).join(""),_=l?.isOpen?hl(l):"",g=new Date(r,c-2,1),S=new Date(r,c,1),T=`${g.getFullYear()}-${String(g.getMonth()+1).padStart(2,"0")}`,N=`${S.getFullYear()}-${String(S.getMonth()+1).padStart(2,"0")}`,E=(()=>{const k=new Date;return`${k.getFullYear()}-${String(k.getMonth()+1).padStart(2,"0")}`})();return`
    <section class="page-head">
      <div>
  ...
  `}function vl(e,t){const n=e.reduce((c,a)=>c+a.totalAmount,0),l=e.filter(c=>c.financialStatus==="paid").length,r=e.filter(c=>c.fulfillmentStatus!=="fulfilled"&&c.fulfillmentStatus!=="shipped").length;return`
    <section class="page-head">
      <div>
  ...
  `}function bl(e,t,n){const l=t==="__new__"?null:e.find(a=>a.id===t),r=t==="__new__";return n?.role==="admin"?`
    <section class="page-head">
      <div>
  ...
  `:`
    ...
    `}function $l(e){const t={};return e.forEach(n=>{const l=n.userEmail??"(anonymous)";t[l]=(t[l]??0)+1}),`
    <section class="page-head">
      <div>
  ...
  `}function _l(e){const t=e.prospects.reduce((c,a)=>c+a.expectedAmount,0),n=e.prospects.reduce((c,a)=>c+a.expectedAmount*a.probability/100,0),l=e.prospects.filter(c=>c.stage==="won").length,r=e.prospects.filter(c=>c.stage==="hot"||c.stage==="negotiating").length;return`
    <section class="page-head">
      <div>
    ...
    ${e.viewMode==="kanban"?wl(e.prospects):kl(e.prospects)}
  ...
  `}function xl(e,t,n){const l=e?.config.webhook_url??"",r=e?.config.default_channel??"#general";return`
    <section class="page-head">
      <div>
  ...
  `}const El=["飲食店","居酒屋","寿司屋","和食","焼肉","フレンチ","イタリアン","バー","酒販店","ワインショップ","百貨店","スーパー","ホテル","旅館","ブライダル","セレクトショップ"];function Cl(e){const t=e.activeListId?e.lists.find(c=>c.id===e.activeListId):null,n=e.items.filter(c=>c.status==="new").length,l=e.items.filter(c=>c.status==="imported").length,r=e.items.filter(c=>c.status==="excluded").length;return`
    <section class="page-head">
      <div>
        <p class="eyebrow">新規営業</p>
        <h1>リスト取得ツール</h1>
  ...
  `}function Pl(e,t,n){const{subtotal:l,taxAmount:r,total:c}=ss(e.lines,e.taxRate),a=e.previousBalance??0,i=e.paymentAmount??0,o=a-i+c,d=e.lines.map(p=>`
      <tr>
  ...
  `}function ql(e,t,n){const{subtotal:l,taxAmount:r,total:c}=ss(e.lines,e.taxRate),a=e.lines.map(o=>`
      <tr>
  ...
  `}function Tl(e,t,n,l){let r="";switch(e){case"chain_store":r=Dl(l,n,t);break;case"quotation":r=ql(l,n,t);break;case"invoice_monthly":r=Pl(l,n,t);break}const c=Object.keys(ta).map(o=>`<button class="tab-button ${e===o?"active":""}" data-print-template="${o}">${ta[o]}</button>`).join(""),a=l.lines.map((o,d)=>`
      <tr>
...
`}async function Fl(e,t){const{supabaseInsert:n}=await b(async()=>{const{supabaseInsert:i}=await Promise.resolve().then(()=>q);return{supabaseInsert:i}},void 0);let l=0,r=0;const a={customers:"customers",products:"products",suppliers:"suppliers",staff:"staff"}[e];for(const i of t){if(!i._valid)continue;const{_valid:o,_error:d,...u}=i,p={...u};if(!p.id){const h=e==="customers"?"legacy_customer_code":e==="products"?"legacy_product_code":e==="suppliers"?"legacy_supplier_code":"legacy_staff_code";p.id=String(u[h]??`${e}-${Date.now()}-${l+r}`)}for(const h of["volume_ml","closing_day","payment_day","list_price","default_sale_price"])if(typeof p[h]=="string"&&p[h]!==""){const v=Number(p[h]);Number.isFinite(v)&&(p[h]=v)}try{await n(a,p)!==null?l++:r++}catch{r++}}return{inserted:l,failed:r}}function at(e){return new Intl.DateTimeFormat("ja-JP",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"}).format(new Date(e))}function Bl(e,t=40){return!e||e.length<=t?e||"":e.slice(0,t)+"…"}function zl(e,t,n,l,r){const c=n.reduce((d,u)=>d+u.rowCount,0),a=n.map(d=>d.lastSyncAt).filter(d=>d!==null).sort().reverse()[0]??null,i=100,o=Math.max(1,Math.ceil(r/i));return`
        ...
        <table>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td class="mono" style="font-size:11px;max-width:300px;overflow:hidden;text-overflow:ellipsis;" title="${d._raw_b64?d._raw_b64.slice(0,200):""}">${Bl(d._raw_b64)}</td>
            </tr>
            `).join("")}
          </tbody>
        </table>
    ...
    `}
  `}function Vl(e){const t={empty:"空",in_use:"使用中",aging:"熟成中"},n={empty:"neutral",in_use:"warning",aging:"success"},l=e.map(d=>{const u=d.capacity>0?Math.round(d.currentVolume/d.capacity*100):0;return`
        <tr>
  ...
  `}function st(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function Jl(e,t,n){const l=e.rows.map((d,u)=>`
      <tr>
  ...
  `}const Ul={success:'<svg width="18" height="18" viewBox="0 0 18 18" fill="none"><circle cx="9" cy="9" r="8.5" stroke="currentColor"/><path d="M5.5 9.5L7.5 11.5L12.5 6.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>',error:'<svg width="18" height="18" viewBox="0 0 18 18" fill="none"><circle cx="9" cy="9" r="8.5" stroke="currentColor"/><path d="M6.5 6.5L11.5 11.5M11.5 6.5L6.5 11.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>',warning:'<svg width="18" height="18" viewBox="0 0 18 18" fill="none"><path d="M9 2L16.5 15H1.5L9 2Z" stroke="currentColor" stroke-width="1.2" stroke-linejoin="round"/><path d="M9 7V10.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/><circle cx="9" cy="13" r="0.8" fill="currentColor"/></svg>',info:'<svg width="18" height="18" viewBox="0 0 18 18" fill="none"><circle cx="9" cy="9" r="8.5" stroke="currentColor"/><path d="M9 8V13" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/><circle cx="9" cy="5.5" r="0.8" fill="currentColor"/></svg>'};let ae=null,Yl=0;const ut=[];function Hl(){return ae&&document.body.contains(ae)||(ae=document.createElement("div"),ae.className="toast-container",document.body.appendChild(ae)),ae}function $(e,t="success",n){const l=Hl(),r=++Yl,c=t==="error"?5e3:t==="warning"?4e3:3e3,a=document.createElement("div");a.className=`toast toast-${t}`,a.setAttribute("role","status"),a.setAttribute("aria-live","polite"),a.innerHTML=`
    <span class="toast-icon">${Ul[t]}</span>
    <span class="toast-msg">${Gl(e)}</span>
    <button class="toast-dismiss" aria-label="閉じる">✕</button>
  `;const i={id:r,message:e,type:t,el:a};ut.push(i),l.appendChild(a),requestAnimationFrame(()=>{a.classList.add("toast-enter")});const o=()=>Xl(i);a.querySelector(".toast-dismiss").addEventListener("click",o),setTimeout(()=>{a.classList.add("toast-exit"),a.addEventListener("animationend",o,{once:!0})},c)}function Xl(e){const t=ut.indexOf(e);t!==-1&&(ut.splice(t,1),e.el.remove())}function Gl(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function W(e,t={}){const{title:n="確認",confirmLabel:l="OK",cancelLabel:r="キャンセル",variant:c="primary"}=t;return new Promise(a=>{const i=document.createElement("div");i.className="modal-backdrop confirm-backdrop",i.setAttribute("role","dialog"),i.setAttribute("aria-modal","true"),i.innerHTML=`
    ...
    `;const o=u=>{i.classList.add("confirm-exit"),i.addEventListener("animationend",()=>{i.remove()},{once:!0}),a(u)};i.querySelector(".confirm-cancel").addEventListener("click",()=>o(!1)),i.querySelector(".confirm-ok").addEventListener("click",()=>o(!0)),i.addEventListener("click",u=>{u.target===i&&o(!1)});const d=u=>{u.key==="Escape"&&(document.removeEventListener("keydown",d),o(!1))};document.addEventListener("keydown",d),document.body.appendChild(i),requestAnimationFrame(()=>{i.querySelector(".confirm-ok")?.focus()})})}const Ql=`<svg width="28" height="28" viewBox="0 0 28 28" fill="none">
...
`)}`,a=new Blob([c],{type:"text/csv;charset=utf-8;"}),i=URL.createObjectURL(a),o=document.createElement("a");o.href=i,o.download=e,document.body.append(o),o.click(),o.remove(),window.setTimeout(()=>URL.revokeObjectURL(i),0)}const Zl=["/","/cat/sales","/cat/brewery","/cat/purchase","/cat/more","/sales","/payment","/master","/invoice","/ledger","/analytics","/customer-analysis","/product-abc","/invoice-entry","/delivery","/billing","/report","/jikomi","/tanks","/kentei","/materials","/purchase","/raw-material","/tax","/store","/setup","/email","/import","/print","/form-designer","/map","/workflow","/mobile-order","/tour","/mail-senders","/calendar","/integrations","/shopify","/fax","/users","/profile","/audit","/prospects","/slack","/calls","/list-builder","/raw-browser","/demand-forecast"];let ue=[];async function eo(){const{supabaseQueryAll:e}=await b(async()=>{const{supabaseQueryAll:n}=await Promise.resolve().then(()=>q);return{supabaseQueryAll:n}},void 0);ue=(await e("customers",{select:"name,email,delivery_area_code",email:"neq.",is_active:"eq.true"})).filter(n=>typeof n.email=="string"&&n.email.length>0).map(n=>({name:String(n.name??""),email:String(n.email??""),area:String(n.delivery_area_code??""),historySegment:"seasonal"}))}const sa=[{path:"/sales",title:"売上一覧"},{path:"/payment",title:"入金状況"},{path:"/master",title:"マスタ"},{path:"/invoice",title:"伝票照会"},{path:"/ledger",title:"得意先台帳"},{path:"/analytics",title:"売上分析"},{path:"/invoice-entry",title:"伝票入力"},{path:"/delivery",title:"納品書"},{path:"/billing",title:"月次請求"},{path:"/report",title:"集計帳票"},{path:"/customer-analysis",title:"得意先分析"},{path:"/product-abc",title:"商品ABC分析"},{path:"/jikomi",title:"仕込管理"},{path:"/tanks",title:"タンク管理"},{path:"/kentei",title:"検定管理"},{path:"/materials",title:"資材管理"},{path:"/purchase",title:"仕入・買掛"},{path:"/raw-material",title:"手形・原料"},{path:"/tax",title:"酒税申告"},{path:"/store",title:"店舗・直売所"},{path:"/setup",title:"連動設定"},{path:"/import",title:"CSV/Excelインポート"},{path:"/print",title:"印刷センター"},{path:"/form-designer",title:"帳票デザイナー"},{path:"/map",title:"取引先マップ"},{path:"/workflow",title:"受注ワークフロー"},{path:"/mobile-order",title:"モバイル受注"},{path:"/tour",title:"酒蔵見学"},{path:"/mail-senders",title:"メール送信元管理"},{path:"/calendar",title:"カレンダー"},{path:"/integrations",title:"外部連携設定"},{path:"/shopify",title:"Shopify注文"},{path:"/fax",title:"FAX OCR"},{path:"/users",title:"ユーザー管理"},{path:"/profile",title:"プロフィール"},{path:"/audit",title:"操作ログ"},{path:"/prospects",title:"新規営業"},{path:"/slack",title:"Slack通知"},{path:"/calls",title:"通話履歴(IVRy)"},{path:"/list-builder",title:"リスト取得ツール"},{path:"/raw-browser",title:"データブラウザ"},{path:"/demand-forecast",title:"需要予測・納品カレンダー"}];function ns(e){const t=yt[e];return t?{subject:t.subject,body:t.body}:{subject:"",body:""}}function At(){return{invoiceType:"sales",invoiceDate:new Date().toISOString().slice(0,10),customerCode:"",customerName:"",staffCode:"",lines:[],note:""}}function to(){const e=ns("spring");return{mode:"all",region:"all",historySegment:"seasonal",templateId:"spring",subject:e.subject,body:e.body,saveMessage:null}}const Qe=new Date,ao=Qe.toISOString().slice(0,7),so=Qe.getFullYear(),no=Qe.getMonth()+1,io=Qe.toISOString().slice(0,10),lo="C0011",se=to();function is(e){const t="/".endsWith("/")?"/".slice(0,-1):"/",n=e.startsWith(t)?e.slice(t.length)||"/":e;return Zl.includes(n)?n:"/"}function Et(e){switch(e){case"/invoice-entry":case"/quote":case"/delivery":case"/billing":case"/invoice":case"/ledger":return"sales";case"/analytics":case"/customer-analysis":case"/product-abc":case"/report":case"/demand-forecast":return"analytics";case"/prospects":case"/map":case"/list-builder":case"/calls":case"/email":case"/mail-senders":case"/workflow":case"/mobile-order":case"/shopify":case"/fax":return"crm";case"/purchase":case"/raw-material":return"orders";case"/jikomi":case"/tanks":case"/kentei":case"/materials":case"/tax":return"brewery";case"/master":case"/calendar":case"/store":case"/tour":case"/print":case"/form-designer":return"master";case"/setup":case"/integrations":case"/slack":case"/import":case"/raw-browser":case"/users":case"/profile":case"/audit":return"settings";default:return"dashboard"}}const na=is(location.pathname),s={salesSummary:null,paymentStatus:null,masterStats:null,pipelineMeta:null,syncDashboard:null,rawTableList:[],rawRecords:[],rawSelectedTable:null,rawPage:1,rawTotalCount:0,invoiceRecords:[],customerLedger:null,salesAnalytics:null,customerAnalysis:null,productABC:null,invoiceForm:At(),invoiceSaving:!1,invoiceSavedDocNo:null,invoicePriceGroup:"",pickerMode:null,pickerQuery:"",pickerTargetLine:null,invoiceErrors:{},deliveryNote:null,deliverySearchDocNo:"",billingSummary:null,billingYearMonth:ao,salesReport:null,jikomiList:[],jikomiView:"list",tankList:[],kenteiList:[],materialList:[],purchaseList:[],payableList:[],billList:[],rawStockList:[],taxDeclaration:null,taxYear:so,taxMonth:no,storeSales:[],storeOrders:[],storeTab:"pos",importEntity:"customers",importPreview:null,importing:!1,importResult:null,fdDesignMode:!0,fdSavedPositions:null,fdActiveFieldId:null,mapRegionFilter:"",workflowOrders:[],mobileOrder:{step:"customer",selectedCustomer:null,cart:[],customerQuery:"",productQuery:"",memo:"",submittedDocNo:null},tourInquiries:[],tourActiveId:null,mailSenders:[],mailSenderEditingId:null,emailSenderId:"default",calendarEvents:[],calendarYearMonth:new Date().toISOString().slice(0,7),calendarFilterCategory:"",calendarEdit:null,integrations:[],integrationEditingId:null,shopifyOrders:[],faxRecords:[],faxProcessing:!1,faxOcrText:null,userProfiles:[],userEditingId:null,myProfile:null,auditLogs:[],prospects:[],prospectActivities:[],prospectEditingId:null,prospectViewMode:"kanban",slackRules:[],slackLogs:[],materialEditing:null,materialEditingIsNew:!1,deliveryLocations:[],callLogs:[],mapFilters:{showCustomers:!0,showProspects:!0,showDelivery:!0,filterRegion:"",filterBusinessType:""},leadLists:[],leadItems:[],leadActiveListId:null,leadSearchQuery:"",leadSearchArea:"",leadSearchType:"",leadSearching:!1,leadSearchResults:[],printTemplate:"chain_store",printOptions:{...Il,overlayImageUrl:`${"/".replace(/\/$/,"")}/reference/chainstore_ref.png`},printCompany:{...Ll},printData:{documentNo:"D"+new Date().toISOString().slice(0,10).replaceAll("-",""),documentDate:new Date().toISOString().slice(0,10),orderDate:new Date().toISOString().slice(0,10),deliveryDate:new Date(Date.now()+2*864e5).toISOString().slice(0,10),customerName:"株式会社〇〇商事",customerHonorific:"御中",customerPostalCode:"100-0001",customerAddress:"東京都千代田区〇〇1-2-3",customerCode:"C0001",chainStoreCode:"0123",categoryCode:"21",slipTypeCode:"11",orderNo:"PO-"+new Date().toISOString().slice(5,10).replaceAll("-",""),vendorCode:"V0001",departmentCode:"101",settlementPrint:!1,title:"",remarks:"",lines:[{productCode:"P00012",productName:"純米吟醸 金井の雫",spec:"720ml",quantity:12,unit:"本",unitPrice:1500,amount:18e3,retailPrice:2200,janCode:"4901234567891",caseQty:6},{productCode:"P00008",productName:"本醸造 辛口",spec:"1.8L",quantity:6,unit:"本",unitPrice:1800,amount:10800,retailPrice:2400,janCode:"4901234567908",caseQty:6},{productCode:"P00021",productName:"梅酒 熟成",spec:"500ml",quantity:12,unit:"本",unitPrice:1200,amount:14400,retailPrice:1800,janCode:"4901234567915",caseQty:12}],taxRate:.1,previousBalance:0,paymentAmount:0},storeSalesDate:io,route:na,currentCategory:Et(na),sidebarOpen:!1,salesFilter:{startDate:"",endDate:""},invoiceFilter:{documentNo:"",startDate:"",endDate:"",customerCode:""},ledgerCustomerCode:lo,salesPeriod:"month",customRange:{start:"",end:""},quoteState:(()=>{const e={...Ka};try{const t=localStorage.getItem("quote-seal");t&&(e.sealSettings=JSON.parse(t))}catch{}return e})(),quoteCustomerQuery:"",quoteProductQuery:"",quotePricing:null,masterTab:"customers",masterFilter:{...xt},analyticsTab:"products",emailAudienceMode:se.mode,emailRegion:se.region,emailHistorySegment:se.historySegment,emailTemplateId:se.templateId,emailSubject:se.subject,emailBody:se.body,emailSaveMessage:se.saveMessage,emailSending:!1,demandForecast:{...Tn},globalSearchOpen:!1,globalQuery:"",authSkipped:!1,authSubmitting:!1,authError:null,user:null,loading:!0,actionLoading:!1,error:null};function ia(e){return e.slice(0,10)}function oo(e){return{...e}}function Je(){s.pickerMode=null,s.pickerQuery="",s.pickerTargetLine=null}function ls(){s.invoiceForm=At(),s.invoiceSavedDocNo=null,s.invoicePriceGroup="",s.invoiceErrors={},Je()}function os(e){const t={};return e.invoiceDate.trim()||(t.invoiceDate="伝票日付は必須です。"),e.customerCode.trim()||(t.customerCode="得意先コードは必須です。"),e.lines.length===0&&(t.lines="明細を1行以上入力してください。"),e.lines.forEach((n,l)=>{n.productCode.trim()||(t[`lines.${l}.productCode`]="商品コードは必須です。"),n.productName.trim()||(t[`lines.${l}.productName`]="商品名は必須です。"),n.quantity<=0&&(t[`lines.${l}.quantity`]="数量は1以上を入力してください。"),n.unitPrice<0&&(t[`lines.${l}.unitPrice`]="単価は0円以上で入力してください。")}),t}function ro(e){const t=s.invoiceForm.lines[e];t&&s.invoiceForm.lines.splice(e+1,0,oo(t))}function co(){const e=s.invoiceRecords[0],t=s.masterStats?.customers[0],n=s.masterStats?.products.slice(0,2)??[];s.invoiceForm={invoiceType:"sales",invoiceDate:new Date().toISOString().slice(0,10),customerCode:e?.customerCode??t?.code??"",customerName:e?.customerName??t?.name??"",staffCode:s.invoiceForm.staffCode||"S001",lines:n.map((l,r)=>{const c=r===0?1:2,a=1200*(r+1);return{productCode:l.code,productName:l.name,quantity:c,unitPrice:a,unit:"本",amount:c*a}}),note:e?`過去伝票 ${e.documentNo} をもとに複製`:"直近のサンプル伝票をもとに複製"},s.invoiceSavedDocNo=null,s.invoiceErrors={}}function uo(e){const t=s.masterStats?.customers.find(n=>n.code.toLowerCase()===e.trim().toLowerCase());return t?(s.invoiceForm.customerCode=t.code,s.invoiceForm.customerName=t.name,s.invoicePriceGroup=t.priceGroup||"",!0):!1}function po(e){const t=s.masterStats?.customers.find(n=>n.name===e.trim());return t?(s.invoiceForm.customerCode=t.code,s.invoiceForm.customerName=t.name,s.invoicePriceGroup=t.priceGroup||"",!0):!1}function rs(e){if(Q(e),s.invoiceErrors=os(s.invoiceForm),Object.keys(s.invoiceErrors).length>0){y();return}s.invoiceSaving=!0,y(),ga(s.invoiceForm).then(t=>{s.invoiceSavedDocNo=t.documentNo,s.invoiceSaving=!1,s.invoiceErrors={},s.invoiceForm=At(),y()}).catch(()=>{s.invoiceSaving=!1,y()})}function cs(e){const t=s.salesFilter.startDate?new Date(s.salesFilter.startDate):null,n=s.salesFilter.endDate?new Date(`${s.salesFilter.endDate}T23:59:59`):null;return[...e.salesRecords].sort((l,r)=>new Date(r.date).getTime()-new Date(l.date).getTime()).filter(l=>{const r=new Date(l.date);return!(t&&r<t||n&&r>n)})}function ds(){switch(s.emailAudienceMode){case"area":return s.emailRegion==="all"?ue:ue.filter(e=>e.area===s.emailRegion);case"history":return ue.filter(e=>e.historySegment===s.emailHistorySegment);default:return ue}}function mo(){const e=ds();return{audienceMode:s.emailAudienceMode,region:s.emailRegion,historySegment:s.emailHistorySegment,selectedTemplateId:s.emailTemplateId,subject:s.emailSubject,body:s.emailBody,recipientCount:e.length,previewRecipients:e.slice(0,5),saveMessage:s.emailSaveMessage,sending:s.emailSending,senderId:s.emailSenderId,senders:s.mailSenders}}function nt(e){const t=ds(),n=s.emailAudienceMode==="area"?s.emailRegion:s.emailAudienceMode==="history"?s.emailHistorySegment:"all";return{subject:s.emailSubject.trim(),body:s.emailBody.trim(),templateId:s.emailTemplateId,audienceMode:s.emailAudienceMode,audienceFilter:n,recipientCount:t.length,recipients:t.map(l=>l.email),status:e}}function Ct(){return s.user,!1}function Se(){s.globalSearchOpen=!1,s.globalQuery=""}function ho(){const e=s.globalQuery.trim().toLowerCase();return e?{customers:s.masterStats?.customers.filter(t=>t.code.toLowerCase().includes(e)||t.name.toLowerCase().includes(e))??[],products:s.masterStats?.products.filter(t=>t.code.toLowerCase().includes(e)||t.name.toLowerCase().includes(e))??[],documents:s.invoiceRecords.filter(t=>t.documentNo.toLowerCase().includes(e)||t.customerName.toLowerCase().includes(e)||t.date.toLowerCase().includes(e)),pages:sa.filter(t=>t.path.toLowerCase().includes(e)||t.title.toLowerCase().includes(e))}:{customers:[],products:[],documents:[],pages:sa}}function yo(){let e=[],t,n="export.csv";switch(s.route){case"/sales":e=(s.salesSummary?cs(s.salesSummary):[]).map(l=>({documentNo:l.documentNo,date:l.date,customerCode:l.customerCode,customerName:l.customerName,amount:l.amount})),t=[{key:"documentNo",label:"伝票番号"},{key:"date",label:"日付"},{key:"customerCode",label:"得意先コード"},{key:"customerName",label:"得意先名"},{key:"amount",label:"金額"}],n="sales.csv";break;case"/payment":e=[...s.paymentStatus?.records??[]].sort((l,r)=>r.balanceAmount-l.balanceAmount).map(l=>({...l})),t=[{key:"customerCode",label:"得意先コード"},{key:"customerName",label:"得意先名"},{key:"billedAmount",label:"請求額"},{key:"paymentAmount",label:"入金額"},{key:"balanceAmount",label:"請求残"},{key:"lastPaymentDate",label:"最終入金日"},{key:"status",label:"状態"}],n="payment-status.csv";break;case"/invoice":e=s.invoiceRecords.map(l=>({...l})),t=[{key:"documentNo",label:"伝票番号"},{key:"date",label:"日付"},{key:"customerCode",label:"得意先コード"},{key:"customerName",label:"得意先名"},{key:"itemCount",label:"明細数"},{key:"amount",label:"金額"}],n="invoices.csv";break;case"/purchase":e=s.purchaseList.map(l=>({...l})),t=[{key:"documentNo",label:"伝票番号"},{key:"purchaseDate",label:"仕入日"},{key:"supplierCode",label:"仕入先コード"},{key:"supplierName",label:"仕入先名"},{key:"itemName",label:"品目"},{key:"quantity",label:"数量"},{key:"unitPrice",label:"単価"},{key:"amount",label:"金額"},{key:"status",label:"状態"}],n="purchase.csv";break;case"/jikomi":e=s.jikomiList.map(l=>({...l})),t=[{key:"jikomiNo",label:"仕込番号"},{key:"productName",label:"銘柄"},{key:"riceType",label:"原料米"},{key:"plannedKg",label:"計画量"},{key:"actualKg",label:"実績量"},{key:"startDate",label:"開始日"},{key:"expectedDoneDate",label:"完了予定日"},{key:"tankNo",label:"タンク"},{key:"status",label:"状態"},{key:"note",label:"備考"}],n="jikomi.csv";break;case"/tanks":e=s.tankList.map(l=>({...l})),t=[{key:"tankNo",label:"タンクNo."},{key:"capacity",label:"容量"},{key:"currentVolume",label:"現在量"},{key:"productName",label:"銘柄"},{key:"jikomiNo",label:"仕込番号"},{key:"status",label:"状態"},{key:"lastUpdated",label:"更新日"}],n="tanks.csv";break;case"/kentei":e=s.kenteiList.map(l=>({...l})),t=[{key:"kenteiNo",label:"検定番号"},{key:"jikomiNo",label:"仕込番号"},{key:"productName",label:"銘柄"},{key:"kenteiDate",label:"検定日"},{key:"alcoholDegree",label:"アルコール度数"},{key:"extractDegree",label:"エキス分"},{key:"sakaMeterValue",label:"酒度"},{key:"volume",label:"容量"},{key:"taxCategory",label:"酒類区分"},{key:"status",label:"状態"}],n="kentei.csv";break;case"/materials":e=s.materialList.map(l=>({...l})),t=[{key:"code",label:"コード"},{key:"name",label:"品名"},{key:"unit",label:"単位"},{key:"currentStock",label:"現在庫"},{key:"minimumStock",label:"最低在庫"},{key:"unitCost",label:"単価"},{key:"lastUpdated",label:"更新日"}],n="materials.csv";break;case"/master":s.masterTab==="customers"?(e=s.masterStats?.customers.map(l=>({...l}))??[],t=[{key:"code",label:"得意先コード"},{key:"name",label:"得意先名"},{key:"closingDay",label:"締日"},{key:"paymentDay",label:"入金日"},{key:"isActive",label:"有効"}],n="master-customers.csv"):(e=s.masterStats?.products.map(l=>({...l}))??[],t=[{key:"code",label:"商品コード"},{key:"janCode",label:"JAN"},{key:"name",label:"商品名"},{key:"category",label:"カテゴリ"},{key:"isActive",label:"有効"}],n="master-products.csv");break;default:return}Kl(n,e,t)}function la(e){const t=`${"/".replace(/\/$/,"")}${e==="/"?"/":e}`;history.pushState(null,"",t),s.route=e,s.currentCategory=Et(e),s.sidebarOpen=!1,Se(),It(e)}async function It(e){s.actionLoading=!0,y();try{switch(e){case"/delivery":s.deliveryNote||(s.deliveryNote=await gt(s.deliverySearchDocNo||"D240122"));break;case"/billing":s.billingSummary||(s.billingSummary=await $t(s.billingYearMonth));break;case"/report":s.salesReport||(s.salesReport=await Xe());break;case"/customer-analysis":s.customerAnalysis||(s.customerAnalysis=await Aa());break;case"/product-abc":s.productABC||(s.productABC=await Ea());break;case"/demand-forecast":if(s.demandForecast.forecasts.length===0){const t=await xe();s.demandForecast.forecasts=jn(t.productTotals,t.monthlySales,s.salesSummary?.allDailySales??[]);try{const{supabaseQuery:n}=await b(async()=>{const{supabaseQuery:r}=await Promise.resolve().then(()=>q);return{supabaseQuery:r}},void 0),l=await n("delivery_schedule",{select:"delivery_date,customer_name,product_name,quantity,status",order:"delivery_date.asc"});s.demandForecast.deliveries=l.map(r=>({date:r.delivery_date,customerName:r.customer_name,productName:r.product_name,quantity:typeof r.quantity=="number"?r.quantity:parseInt(String(r.quantity))||0,status:r.status||"scheduled"}))}catch{}}break;case"/jikomi":s.jikomiList.length===0&&(s.jikomiList=await Ia());break;case"/tanks":s.tankList.length===0&&(s.tankList=await La());break;case"/kentei":s.kenteiList.length===0&&(s.kenteiList=await Da());break;case"/materials":s.materialList.length===0&&(s.materialList=await lt());break;case"/purchase":(s.purchaseList.length===0||s.payableList.length===0)&&([s.purchaseList,s.payableList]=await Promise.all([Pa(),qa()]));break;case"/raw-material":(s.billList.length===0||s.rawStockList.length===0)&&([s.billList,s.rawStockList]=await Promise.all([Ta(),Na()]));break;case"/tax":s.taxDeclaration||(s.taxDeclaration=await _t(s.taxYear,s.taxMonth));break;case"/store":(s.storeSales.length===0||s.storeOrders.length===0)&&([s.storeSales,s.storeOrders]=await Promise.all([wt(s.storeSalesDate),Oa()]));break;case"/mail-senders":case"/email":{const{fetchMailSenders:t}=await b(async()=>{const{fetchMailSenders:n}=await Promise.resolve().then(()=>w);return{fetchMailSenders:n}},void 0);if(s.mailSenders=await t(),!s.emailSenderId||!s.mailSenders.find(n=>n.id===s.emailSenderId)){const n=s.mailSenders.find(l=>l.isDefault)??s.mailSenders[0];n&&(s.emailSenderId=n.id)}}break;case"/calendar":{const{fetchCalendarEvents:t}=await b(async()=>{const{fetchCalendarEvents:n}=await Promise.resolve().then(()=>w);return{fetchCalendarEvents:n}},void 0);s.calendarEvents=await t(s.calendarYearMonth)}break;case"/integrations":{const{fetchIntegrationSettings:t}=await b(async()=>{const{fetchIntegrationSettings:n}=await Promise.resolve().then(()=>w);return{fetchIntegrationSettings:n}},void 0);s.integrations=await t()}break;case"/shopify":{const{fetchShopifyOrders:t,fetchIntegrationSettings:n}=await b(async()=>{const{fetchShopifyOrders:l,fetchIntegrationSettings:r}=await Promise.resolve().then(()=>w);return{fetchShopifyOrders:l,fetchIntegrationSettings:r}},void 0);s.shopifyOrders=await t(),s.integrations.length===0&&(s.integrations=await n())}break;case"/fax":{const{fetchFaxInbox:t,fetchIntegrationSettings:n}=await b(async()=>{const{fetchFaxInbox:l,fetchIntegrationSettings:r}=await Promise.resolve().then(()=>w);return{fetchFaxInbox:l,fetchIntegrationSettings:r}},void 0);s.faxRecords=await t(),s.integrations.length===0&&(s.integrations=await n())}break;case"/users":{const{fetchUserProfiles:t}=await b(async()=>{const{fetchUserProfiles:n}=await Promise.resolve().then(()=>w);return{fetchUserProfiles:n}},void 0);s.userProfiles=await t()}break;case"/profile":{const{fetchMyProfile:t,fetchAuditLogs:n,fetchMailSenders:l}=await b(async()=>{const{fetchMyProfile:c,fetchAuditLogs:a,fetchMailSenders:i}=await Promise.resolve().then(()=>w);return{fetchMyProfile:c,fetchAuditLogs:a,fetchMailSenders:i}},void 0),r=s.user?.email??s.myProfile?.email??"";r&&(s.myProfile=await t(r)),s.mailSenders.length===0&&(s.mailSenders=await l()),s.auditLogs=await n(50)}break;case"/audit":{const{fetchAuditLogs:t}=await b(async()=>{const{fetchAuditLogs:n}=await Promise.resolve().then(()=>w);return{fetchAuditLogs:n}},void 0);s.auditLogs=await t(200)}break;case"/prospects":{const{fetchProspects:t}=await b(async()=>{const{fetchProspects:n}=await Promise.resolve().then(()=>w);return{fetchProspects:n}},void 0);s.prospects=await t()}break;case"/map":{const{fetchProspects:t,fetchDeliveryLocations:n,fetchIntegrationSettings:l}=await b(async()=>{const{fetchProspects:r,fetchDeliveryLocations:c,fetchIntegrationSettings:a}=await Promise.resolve().then(()=>w);return{fetchProspects:r,fetchDeliveryLocations:c,fetchIntegrationSettings:a}},void 0);s.prospects=await t(),s.deliveryLocations=await n(),s.integrations.length===0&&(s.integrations=await l())}break;case"/calls":{const{fetchCallLogs:t,fetchIntegrationSettings:n}=await b(async()=>{const{fetchCallLogs:l,fetchIntegrationSettings:r}=await Promise.resolve().then(()=>w);return{fetchCallLogs:l,fetchIntegrationSettings:r}},void 0);s.callLogs=await t(100),s.integrations.length===0&&(s.integrations=await n())}break;case"/list-builder":{const{fetchLeadLists:t,fetchIntegrationSettings:n}=await b(async()=>{const{fetchLeadLists:l,fetchIntegrationSettings:r}=await Promise.resolve().then(()=>w);return{fetchLeadLists:l,fetchIntegrationSettings:r}},void 0);s.leadLists=await t(),s.integrations.length===0&&(s.integrations=await n())}break;case"/workflow":{const{fetchWorkflowOrdersFromDb:t}=await b(async()=>{const{fetchWorkflowOrdersFromDb:n}=await Promise.resolve().then(()=>w);return{fetchWorkflowOrdersFromDb:n}},void 0);s.workflowOrders=await t()}break;case"/tour":{const{fetchTourInquiriesFromDb:t}=await b(async()=>{const{fetchTourInquiriesFromDb:n}=await Promise.resolve().then(()=>w);return{fetchTourInquiriesFromDb:n}},void 0);s.tourInquiries=await t()}break;case"/slack":{const{fetchSlackRules:t,fetchSlackLogs:n,fetchIntegrationSettings:l}=await b(async()=>{const{fetchSlackRules:r,fetchSlackLogs:c,fetchIntegrationSettings:a}=await Promise.resolve().then(()=>w);return{fetchSlackRules:r,fetchSlackLogs:c,fetchIntegrationSettings:a}},void 0);s.slackRules=await t(),s.slackLogs=await n(50),s.integrations.length===0&&(s.integrations=await l())}break;case"/":{const{fetchProspects:t,fetchCalendarEvents:n,fetchWorkflowOrdersFromDb:l,fetchTourInquiriesFromDb:r}=await b(async()=>{const{fetchProspects:c,fetchCalendarEvents:a,fetchWorkflowOrdersFromDb:i,fetchTourInquiriesFromDb:o}=await Promise.resolve().then(()=>w);return{fetchProspects:c,fetchCalendarEvents:a,fetchWorkflowOrdersFromDb:i,fetchTourInquiriesFromDb:o}},void 0);s.prospects.length===0&&(s.prospects=await t()),s.calendarEvents.length===0&&(s.calendarEvents=await n(s.calendarYearMonth)),s.materialList.length===0&&(s.materialList=await lt()),s.workflowOrders.length===0&&(s.workflowOrders=await l()),s.tourInquiries.length===0&&(s.tourInquiries=await r())}break;default:break}}catch(t){console.warn("Route data load error",t)}finally{s.actionLoading=!1,y()}}function oa(){if(Ct())return fi(s.authError,s.authSubmitting);if(s.loading)return`
    ...
    `;switch(s.route){case"/cat/sales":return Ce("sales");case"/cat/brewery":return Ce("brewery");case"/cat/purchase":return Ce("purchase");case"/cat/more":return Ce("more");case"/invoice-entry":return ti(s.invoiceForm,s.invoiceSavedDocNo,s.invoiceSaving,s.invoiceErrors);case"/quote":return ii(s.quoteState,s.masterStats?.customers??[],s.masterStats?.products??[],s.quoteCustomerQuery,s.quoteProductQuery,s.quotePricing);case"/email":return Wn(mo());case"/delivery":return s.deliveryNote?Gn(s.deliveryNote,s.deliverySearchDocNo):'<section class="panel"><div class="loading-overlay"><div class="loading-spinner"></div><p class="loading-text">データを読み込んでいます…</p></div></section>';case"/billing":return s.billingSummary?Cn(s.billingSummary,s.billingYearMonth):'<section class="panel"><div class="loading-overlay"><div class="loading-spinner"></div><p class="loading-text">データを読み込んでいます…</p></div></section>';case"/report":return s.salesReport?Xi(s.salesReport):'<section class="panel"><div class="loading-overlay"><div class="loading-spinner"></div><p class="loading-text">データを読み込んでいます…</p></div></section>';case"/customer-analysis":return s.customerAnalysis?Mi(s.customerAnalysis):'<section class="panel"><div class="loading-overlay"><div class="loading-spinner"></div><p class="loading-text">データを読み込んでいます…</p></div></section>';case"/product-abc":return s.productABC?zi(s.productABC):'<section class="panel"><div class="loading-overlay"><div class="loading-spinner"></div><p class="loading-text">データを読み込んでいます…</p></div></section>';case"/demand-forecast":return Bn(s.demandForecast);case"/jikomi":return s.jikomiView==="calendar"?`${Ut(s.jikomiList,s.jikomiView)}${hi(s.jikomiList)}`:Ut(s.jikomiList,s.jikomiView);case"/tanks":return Vl(s.tankList);case"/kentei":return yi(s.kenteiList);case"/materials":return Ai(s.materialList)+xi(s.materialEditing,s.materialEditingIsNew);case"/purchase":return Di(s.purchaseList,s.payableList);case"/raw-material":return Pi(s.billList,s.rawStockList);case"/tax":return s.taxDeclaration?Jl(s.taxDeclaration,s.taxYear,s.taxMonth):'<section class="panel"><div class="loading-overlay"><div class="loading-spinner"></div><p class="loading-text">データを読み込んでいます…</p></div></section>';case"/store":return Ki(s.storeSales,s.storeOrders,s.storeTab,s.storeSalesDate);case"/setup":return s.pipelineMeta?Ni(s.pipelineMeta,J,O,s.syncDashboard):'<section class="panel"><div class="loading-overlay"><div class="loading-spinner"></div><p class="loading-text">データを読み込んでいます…</p></div></section>';case"/raw-browser":return zl(s.rawSelectedTable,s.rawRecords,s.rawTableList,s.rawPage,s.rawTotalCount);case"/import":return el(s.importEntity,s.importPreview,s.importing,s.importResult);case"/print":return Tl(s.printTemplate,s.printOptions,s.printCompany,s.printData);case"/form-designer":return al(s.printData,s.printCompany,s.printOptions,s.fdSavedPositions,s.fdDesignMode);case"/map":{const e=(s.masterStats?.customers??[]).slice(0,200).map((l,r)=>{const c=l;return{...l,lat:c.lat??35.37+r%12*.05+(Math.random()-.5)*.02,lng:c.lng??139.29+Math.floor(r/12)*.05+(Math.random()-.5)*.02,address1:c.address1??"",businessType:c.businessType??"",lastOrderAmount:0}}),n=!!s.integrations.find(l=>l.provider==="google_maps")?.config.api_key;return sl(e,s.prospects,s.deliveryLocations,s.mapFilters,n)}case"/workflow":return ll(s.workflowOrders);case"/mobile-order":return ol(s.mobileOrder,s.masterStats?.customers??[],s.masterStats?.products??[]);case"/tour":return cl(s.tourInquiries,s.tourActiveId);case"/mail-senders":return pl(s.mailSenders,s.mailSenderEditingId);case"/calendar":return ml(s.calendarEvents,s.calendarYearMonth,s.calendarFilterCategory,s.calendarEdit);case"/integrations":return yl(s.integrations,s.integrationEditingId);case"/shopify":{const e=s.integrations.find(t=>t.id==="shopify");return vl(s.shopifyOrders,e?.lastSyncAt??null)}case"/fax":return fl(s.faxRecords,s.faxProcessing,s.faxOcrText);case"/users":return bl(s.userProfiles,s.userEditingId,s.myProfile);case"/profile":return gl(s.myProfile,s.auditLogs.filter(e=>e.userEmail===s.myProfile?.email),s.mailSenders);case"/audit":return $l(s.auditLogs);case"/prospects":{const e={prospects:s.prospects,activities:s.prospectActivities,editingId:s.prospectEditingId,viewMode:s.prospectViewMode};return _l(e)}case"/slack":{const e=s.integrations.find(t=>t.provider==="slack")??null;return xl(e,s.slackRules,s.slackLogs)}case"/calls":{const e=s.integrations.find(t=>t.provider==="ivry");return Al(s.callLogs,s.masterStats?.customers??[],e?.lastSyncAt??null,e?.isEnabled??!1)}case"/list-builder":{const e={lists:s.leadLists,activeListId:s.leadActiveListId,items:s.leadItems,searchQuery:s.leadSearchQuery,searchArea:s.leadSearchArea,searchBusinessType:s.leadSearchType,searching:s.leadSearching,searchResults:s.leadSearchResults};return Cl(e)}}if(!s.salesSummary||!s.paymentStatus||!s.masterStats||!s.pipelineMeta||!s.customerLedger||!s.salesAnalytics)return"";switch(s.route){case"/sales":return Wi(cs(s.salesSummary),s.salesFilter.startDate,s.salesFilter.endDate);case"/payment":return Ii([...s.paymentStatus.records].sort((e,t)=>t.balanceAmount-e.balanceAmount));case"/master":return Si(s.masterStats,s.masterTab,s.masterFilter);case"/invoice":return ci(s.invoiceRecords,s.invoiceFilter);case"/ledger":return qn(s.customerLedger,s.ledgerCustomerCode);case"/analytics":return Hi(s.salesAnalytics,s.analyticsTab);default:return Yn(s.salesSummary,s.pipelineMeta,s.salesAnalytics,{prospects:s.prospects,upcomingEvents:s.calendarEvents,tourInquiries:s.tourInquiries,workflowOrdersCount:{new:s.workflowOrders.filter(e=>e.stage==="new").length,picking:s.workflowOrders.filter(e=>e.stage==="picking").length,packed:s.workflowOrders.filter(e=>e.stage==="packed").length,shipped:s.workflowOrders.filter(e=>e.stage==="shipped").length,total:s.workflowOrders.length},lowStockCount:s.materialList.filter(e=>e.currentStock<e.minimumStock*1.5).length,masterCounts:s.masterStats?{customers:s.masterStats.summary.customerCount,products:s.masterStats.summary.productCount,suppliers:s.syncDashboard?.tables.find(e=>e.tableName==="suppliers")?.rowCount??0,specialPrices:s.syncDashboard?.tables.find(e=>e.tableName==="customer_product_prices")?.rowCount??0}:void 0},s.salesPeriod,s.customRange)}}function vo(){if(Ct())return`
    ...
    `;const e={dashboard:[{label:"概要",items:[{path:"/",label:"ダッシュボード",kicker:"Home"},{path:"/sales",label:"売上一覧",kicker:"Sales"},{path:"/payment",label:"入金状況",kicker:"Payment"}]}],sales:[{label:"販売業務",items:[{path:"/invoice-entry",label:"伝票入力",kicker:"Entry"},{path:"/quote",label:"見積作成",kicker:"Quote"},{path:"/delivery",label:"納品書",kicker:"Delivery"},{path:"/billing",label:"月次請求",kicker:"Billing"},{path:"/invoice",label:"伝票照会",kicker:"Invoice"},{path:"/ledger",label:"得意先台帳",kicker:"Ledger"}]}],analytics:[{label:"分析",items:[{path:"/analytics",label:"売上分析",kicker:"Analytics"},{path:"/customer-analysis",label:"得意先分析",kicker:"CustABC"},{path:"/product-abc",label:"商品ABC",kicker:"ProdABC"},{path:"/demand-forecast",label:"需要予測",kicker:"Forecast"},{path:"/report",label:"集計帳票",kicker:"Report"}]}],crm:[{label:"営業・顧客",items:[{path:"/prospects",label:"新規営業",kicker:"Prospects"},{path:"/map",label:"取引先マップ",kicker:"Map"},{path:"/list-builder",label:"リスト取得",kicker:"ListBuild"},{path:"/calls",label:"通話履歴",kicker:"Calls"},{path:"/email",label:"メール配信",kicker:"Mail"}]},{label:"受注・出荷",items:[{path:"/workflow",label:"受注ワークフロー",kicker:"Workflow"},{path:"/mobile-order",label:"モバイル受注",kicker:"Mobile"},{path:"/shopify",label:"Shopify注文",kicker:"Shopify"},{path:"/fax",label:"FAX OCR",kicker:"FAX"}]}],orders:[{label:"仕入・調達",items:[{path:"/purchase",label:"仕入・買掛",kicker:"Purchase"},{path:"/raw-material",label:"手形・原料",kicker:"RawMat"}]}],brewery:[{label:"製造管理",items:[{path:"/jikomi",label:"仕込管理",kicker:"Jikomi"},{path:"/tanks",label:"タンク管理",kicker:"Tank"},{path:"/kentei",label:"検定管理",kicker:"Kentei"},{path:"/materials",label:"資材管理",kicker:"Material"},{path:"/tax",label:"酒税申告",kicker:"Tax"}]}],master:[{label:"マスタ・ツール",items:[{path:"/master",label:"マスタ管理",kicker:"Master"},{path:"/calendar",label:"カレンダー",kicker:"Calendar"},{path:"/store",label:"店舗・直売所",kicker:"Store"},{path:"/tour",label:"酒蔵見学",kicker:"Tour"},{path:"/print",label:"印刷",kicker:"Print"}]}],settings:[{label:"システム設定",items:[{path:"/setup",label:"連動設定",kicker:"Setup"},{path:"/integrations",label:"外部連携",kicker:"API"},{path:"/slack",label:"Slack通知",kicker:"Slack"},{path:"/import",label:"データ取込",kicker:"Import"},{path:"/raw-browser",label:"データブラウザ",kicker:"RawData"},{path:"/users",label:"ユーザー管理",kicker:"Users"},{path:"/profile",label:"プロフィール",kicker:"Profile"},{path:"/audit",label:"操作ログ",kicker:"Audit"}]}]},t=[{category:"dashboard",path:"/",label:"ダッシュボード"},{category:"sales",path:"/invoice-entry",label:"販売"},{category:"analytics",path:"/analytics",label:"分析"},{category:"crm",path:"/prospects",label:"営業"},{category:"orders",path:"/purchase",label:"仕入"},{category:"brewery",path:"/jikomi",label:"製造"},{category:"master",path:"/master",label:"マスタ"},{category:"settings",path:"/setup",label:"設定"}],n=e[s.currentCategory].map(i=>`
      ...
      `).join(""),r=s.pickerMode&&s.masterStats?s.pickerMode==="customer"?Zn(s.masterStats.customers,s.pickerQuery):Li(s.masterStats.products,s.pickerQuery):"",c=s.globalSearchOpen?Kn(s.globalQuery,ho()):"",a=s.user?`
