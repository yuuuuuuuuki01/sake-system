  ...
  `}const ya={chain_store:"チェーンストア伝票",quotation:"見積書",invoice_monthly:"請求書（月次）"},Pl={pageSize:"A4",orientation:"portrait",fontSize:"medium",showSeal:!0,showRegistrationNo:!0,showBankInfo:!0,showRemarks:!0,showJanCode:!0,showUnit:!0,colorMode:"color",copies:1,showReferenceOverlay:!1,overlayOpacity:.4,overlayImageUrl:"reference/chainstore_ref.png",calibrationOffsetX:0,calibrationOffsetY:0},Al={name:"金井酒造店",postalCode:"257-0014",address1:"神奈川県秦野市堀山下182",address2:"",tel:"0463-88-1511",fax:"0463-88-5885",email:"info@kaneishuzo.co.jp",registrationNo:"T1234567890123",bankName:"横浜銀行",bankBranch:"秦野支店",bankAccountType:"普通",bankAccountNo:"1234567",bankAccountHolder:"カ）カナイシュゾウテン",sealImageUrl:""};function Y(e){return"¥"+e.toLocaleString("ja-JP")}function Ce(e){const t=new Date(e);return`${t.getFullYear()}年${t.getMonth()+1}月${t.getDate()}日`}function ws(e,t){const n=e.reduce((l,d)=>l+d.amount,0),i=Math.floor(n*t),r=n+i;return{subtotal:n,taxAmount:i,total:r}}const O={currentDateY:{x:10,y:3.5,size:8},currentDateM:{x:17,y:3.5,size:8},currentDateD:{x:24,y:3.5,size:8},documentNo:{x:155,y:4,size:9,bold:!0},settlementCheck:{x:217,y:4,size:9},vendorName:{x:4,y:12,size:10,bold:!0},vendorAddress:{x:4,y:16.5,size:6.5},chainStoreCode:{x:63,y:13,size:9},categoryCode:{x:87,y:13,size:9},slipNumber:{x:117,y:13,size:9},vendorCode:{x:151,y:13,size:9},orderDateY:{x:180,y:13,size:7.5},orderDateM:{x:186,y:13,size:7.5},orderDateD:{x:192,y:13,size:7.5},deliveryDateY:{x:204,y:13,size:7.5},deliveryDateM:{x:211,y:13,size:7.5},deliveryDateD:{x:218,y:13,size:7.5},customerName:{x:4,y:20,size:10,bold:!0},partnerCode:{x:94,y:20,size:10,bold:!0},orderNo:{x:143,y:20,size:9},detailStartY:33,detailRowH:8.5,detailCols:{productName:{x:5,w:58,align:"left",size:7.5},productCode:{x:64,w:25,align:"left",size:7.5},color:{x:92,w:6,align:"center",size:7},size:{x:99,w:15,align:"center",size:7},unit:{x:115,w:8,align:"center",size:7},quantity:{x:124,w:10,align:"right",size:8},correctedQty:{x:137,w:14,align:"right",size:8},discount:{x:153,w:9,align:"right",size:7.5},unitPrice:{x:163,w:12,align:"right",size:8},costAmount:{x:176,w:16,align:"right",size:8,bold:!0},retailPrice:{x:193,w:12,align:"right",size:8},note:{x:206,w:18,align:"right",size:8}},totalQty:{x:125,y:89,size:9,bold:!0},receivedTotal:{x:150,y:89,size:9},returnTotal:{x:165,y:89,size:9},correctedCostTotal:{x:179,y:89,size:10,bold:!0},correctedRetailTotal:{x:207,y:89,size:10,bold:!0}};function j(e,t){const n=e.align??"left",i=e.size??8;return`<div class="bp-fld" style="${["position:absolute",`left:${e.x}mm`,`top:${e.y}mm`,e.w?`width:${e.w}mm`:"",`text-align:${n}`,`font-size:${i}pt`,e.bold?"font-weight:700":"","line-height:1","white-space:nowrap","overflow:hidden"].filter(Boolean).join(";")}">${t}</div>`}function ut(e){if(!e)return{y:"",m:"",d:""};const t=new Date(e),n=t.getFullYear(),i=n-2018;return{y:i>0?String(i).padStart(2,"0"):String(n).slice(-2),m:String(t.getMonth()+1).padStart(2,"0"),d:String(t.getDate()).padStart(2,"0")}}function Dl(e,t,n){const i=ut(e.documentDate),r=ut(e.orderDate??e.documentDate),l=ut(e.deliveryDate??e.documentDate),d=e.lines.slice(0,6).map((g,x)=>{const w=O.detailStartY+x*O.detailRowH,E=O.detailCols,C=[],$=(P,I)=>{I&&C.push(j({...P,y:w,x:P.x+0},I))};return $(E.productName,g.productName+(g.spec?` ${g.spec}`:"")),$(E.productCode,g.productCode),$(E.color,g.color??""),$(E.size,[g.size,g.caseQty?`×${g.caseQty}`:""].filter(Boolean).join(" ")),$(E.unit,g.unit),$(E.quantity,g.quantity>0?g.quantity.toLocaleString("ja-JP"):""),$(E.correctedQty,g.correctedQuantity?g.correctedQuantity.toLocaleString("ja-JP"):""),$(E.discount,g.discount?g.discount.toLocaleString("ja-JP"):""),$(E.unitPrice,g.unitPrice>0?g.unitPrice.toLocaleString("ja-JP"):""),$(E.costAmount,g.amount>0?g.amount.toLocaleString("ja-JP"):""),$(E.retailPrice,g.retailPrice?g.retailPrice.toLocaleString("ja-JP"):""),$(E.note,g.receivedAmount?g.receivedAmount.toLocaleString("ja-JP"):""),C.join("")}).join(""),a=e.lines.reduce((g,x)=>g+(x.amount||0),0),o=e.lines.reduce((g,x)=>g+(x.retailPrice||0)*(x.correctedQuantity??x.quantity),0),c=e.lines.reduce((g,x)=>g+(x.receivedAmount||0),0),u=e.lines.reduce((g,x)=>g+(x.returnAmount||0),0),p=e.lines.reduce((g,x)=>g+x.quantity,0),y=n.showReferenceOverlay?`background-image: url('${n.overlayImageUrl}'); background-size: 100% 100%; background-repeat: no-repeat; opacity: 1;`:"",h=n.calibrationOffsetX||0,f=n.calibrationOffsetY||0,k=`transform: translate(${h}mm, ${f}mm);`;return`
    <div class="print-page bp1701-overlay">
      <div class="bp-sheet" style="${y}">
        ...
        ${j(O.currentDateY,i.y)}
        ${j(O.currentDateM,i.m)}
        ${j(O.currentDateD,i.d)}
        ${j(O.documentNo,e.documentNo)}
        ${e.settlementPrint?j(O.settlementCheck,"✓"):""}

        ${j(O.vendorName,t.name)}
        ${j(O.vendorAddress,t.address1)}
        ${j(O.chainStoreCode,e.chainStoreCode??"")}
        ${j(O.categoryCode,e.categoryCode??"")}
        ${j(O.slipNumber,e.documentNo)}
        ${j(O.vendorCode,e.slipTypeCode??"")}

        ${j(O.customerName,`${e.customerName} ${e.customerHonorific}`)}
        ${j(O.orderDateY,r.y)}
        ${j(O.orderDateM,r.m)}
        ${j(O.orderDateD,r.d)}
        ${j(O.deliveryDateY,l.y)}
        ${j(O.deliveryDateM,l.m)}
        ${j(O.deliveryDateD,l.d)}
        ${j(O.orderNo,e.orderNo??"")}
        ${j(O.partnerCode,e.vendorCode??"")}

        ${d}

        ${j(O.totalQty,p.toLocaleString("ja-JP"))}
        ${j(O.receivedTotal,c.toLocaleString("ja-JP"))}
        ${j(O.returnTotal,u.toLocaleString("ja-JP"))}
        ${j(O.correctedCostTotal,a.toLocaleString("ja-JP"))}
        ${j(O.correctedRetailTotal,o.toLocaleString("ja-JP"))}
        </div>
      </div>
    </div>
...
`}async function Ol(e,t){const{supabaseInsert:n}=await _(async()=>{const{supabaseInsert:a}=await Promise.resolve().then(()=>M);return{supabaseInsert:a}},void 0);let i=0,r=0;const d={customers:"customers",products:"products",suppliers:"suppliers",staff:"staff"}[e];for(const a of t){if(!a._valid)continue;const{_valid:o,_error:c,...u}=a,p={...u};if(!p.id){const y=e==="customers"?"legacy_customer_code":e==="products"?"legacy_product_code":e==="suppliers"?"legacy_supplier_code":"legacy_staff_code";p.id=String(u[y]??`${e}-${Date.now()}-${i+r}`)}for(const y of["volume_ml","closing_day","payment_day","list_price","default_sale_price"])if(typeof p[y]=="string"&&p[y]!==""){const h=Number(p[y]);Number.isFinite(h)&&(p[y]=h)}try{await n(d,p)!==null?i++:r++}catch{r++}}return{inserted:i,failed:r}}function pt(e){return new Intl.DateTimeFormat("ja-JP",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"}).format(new Date(e))}function Ml(e,t=40){return!e||e.length<=t?e||"":e.slice(0,t)+"…"}function jl(e,t,n,i,r){const l=n.reduce((c,u)=>c+u.rowCount,0),d=n.map(c=>c.lastSyncAt).filter(c=>c!==null).sort().reverse()[0]??null,a=100,o=Math.max(1,Math.ceil(r/a));return`
  ...
  `}const Yl={monthly:"月次",annual:"年次",make_to_order:"受注生産",november:"11月生産"};function Ul(e,t,n){const i={draft:"下書き",confirmed:"確定",actual:"実績入力済"},r={draft:"neutral",confirmed:"info",actual:"success"},l=$=>Object.entries(Yl).map(([P,I])=>`<option value="${P}" ${P===$?"selected":""}>${I}</option>`).join(""),d=640,a=$=>$.map(P=>{const I=Math.max(0,P.demandForecast+P.safetyStockTarget-P.openingStock),T=P.plannedQty>0?P.plannedQty:Math.round(I),L=T>0?Math.ceil(T/d*10)/10:0,B=P.plannedQty>0?(P.actualQty-P.plannedQty)/P.plannedQty*100:null,G=B!==null?B>=0?"text-success":"text-danger":"";return`
    ...
    `}).join(""),o=n==="all"?e:e.filter($=>$.productionType===n),c=a(o),u=[{key:"all",label:"全て"},{key:"monthly",label:"月次"},{key:"annual",label:"年次"},{key:"november",label:"11月生産"},{key:"make_to_order",label:"受注生産"}],p=$=>{const I=($==="all"?e:e.filter(T=>T.productionType===$)).reduce((T,L)=>{const B=Math.max(0,L.demandForecast+L.safetyStockTarget-L.openingStock);return T+(L.plannedQty>0?L.plannedQty:Math.round(B))},0);return Math.ceil(I/d*10)/10},y=u.filter($=>$.key!=="all").map($=>{const P=p($.key),I=e.filter(L=>L.productionType===$.key).length,T=$.key==="make_to_order"?e.filter(L=>L.productionType==="make_to_order"&&L.plannedQty>0).length:null;return`
    ...
    `}).join(""),h=o.reduce(($,P)=>$+P.demandForecast,0),f=o.reduce(($,P)=>$+Math.max(0,P.demandForecast+P.safetyStockTarget-P.openingStock),0),k=o.reduce(($,P)=>$+P.plannedQty,0),g=o.reduce(($,P)=>$+P.actualQty,0),x=p(n),w=new Date,E=Array.from({length:24},($,P)=>{const I=new Date(w.getFullYear(),w.getMonth()-6+P,1),T=`${I.getFullYear()}-${String(I.getMonth()+1).padStart(2,"0")}`;return`<option value="${T}" ${T===t?"selected":""}>${T.replace("-","年")}月</option>`}).join(""),C=u.map($=>`<button class="button ${n===$.key?"primary":"secondary"}" type="button"
