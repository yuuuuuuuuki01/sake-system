...
`}function _s(e,t){const n=e.lines.reduce((P,o)=>P+o.amount,0),s=Math.round(n*e.taxRate/100),i=n+s,l=e.templateType==="sake",d=l?'<th style="width:90px;">JANコード</th><th style="width:40px;">入数</th><th style="width:70px;">希望小売価格</th>':"",p=l?9:6,u=e.lines.map((P,o)=>{const r=l?`<td style="font-size:9px;">${Y(P.janCode)}</td><td style="text-align:center;">${P.caseQty??""}</td><td style="text-align:right;">${P.retailPrice!=null?_e(P.retailPrice):""}</td>`:"";return`<tr>
  ...
  `:"",f=t.sealImageDataUrl?`<img src="${t.sealImageDataUrl}" style="width:${t.sealSize}px;height:${t.sealSize}px;opacity:0.9;flex-shrink:0;" />`:"",b=[];e.validUntil&&b.push(`<div class="q-cond-cell"><div class="q-cond-label">有効期限</div><div class="q-cond-value">${en(e.validUntil)}</div></div>`),e.paymentTerms&&b.push(`<div class="q-cond-cell"><div class="q-cond-label">支払条件</div><div class="q-cond-value">${Y(e.paymentTerms)}</div></div>`),e.deliveryDate&&b.push(`<div class="q-cond-cell"><div class="q-cond-label">納期</div><div class="q-cond-value">${Y(e.deliveryDate)}</div></div>`),e.deliveryPlace&&b.push(`<div class="q-cond-cell"><div class="q-cond-label">納品場所</div><div class="q-cond-value">${Y(e.deliveryPlace)}</div></div>`);const k=b.length>0?`<div class="q-cond-grid" style="grid-template-columns:repeat(${Math.min(b.length,4)},1fr);">${b.join("")}</div>`:"";return`
<div class="q-doc">
  <!-- タイトル行: 左=御見積書、右=見積番号・日付（右寄せ） -->
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:4px;align-items:center;">
          ${bs.map(P=>`
            <button type="button" data-action="set-accent-color" data-color="${Y(P.value)}" title="${Y(P.label)}"
              style="width:28px;height:28px;border-radius:4px;border:3px solid ${d.accentColor===P.value?"#333":"transparent"};background:${Y(P.value)};cursor:pointer;"></button>
          `).join("")}
        </div>
      </div>
      <div class="form-grid-2">
...
cd sake-system\\relay`,success:["エクスプローラーで sake-system\\\\relay フォルダが見える状態になっていれば成功です。","cd を実行したあと、コマンドプロンプトの行頭が relay フォルダを指していれば問題ありません。"],errors:["git コマンドが使えない場合は Git for Windows が未導入です。先に Git をインストールしてください。","アクセス拒否が出る場合は C:\\\\Program Files 配下ではなく C:\\\\sake-relay\\\\ などに配置してください。"]})}
  ...
  `}function Ls(e,t,n="all",s="",i=[],l=[],d="",p="",u=null,y="all",v="",f=[],b=[],k=[],P=null,o=[],r=[],c="amount",m="calendar"){const g=t==="products"?"商品別集計":t==="customers"?"得意先別集計":"担当別集計",$=t==="products"?e.productTotals:t==="customers"?e.customerTotals:e.staffTotals,_=n!=="all"&&i.length>0&&t!=="staff"?i:$,S=We(_,k,Ul),E={all:"月別",yearly:"月別推移",monthly:"日別推移",weekly:"日別推移",daily:"当日"},A=cn[c],L=V=>c==="quantity"?V.quantity:c==="volume"?V.volumeMl:V.amount,M=V=>c==="quantity"?`${V.toLocaleString()}本`:c==="volume"?Vt(V):Ze(V);let D,I=[],O,B,z;if(P&&P.monthlySales.length>0)D=P.monthlySales.slice(-24).map(V=>({month:V.month,amount:L(V)})),O=`${P.name} の月別${A}`,B=`${P.tab==="customers"?"得意先":"商品"}: ${P.code}`,z="#0968e5";else if(o.length>0&&n!=="all"){D=o.map(J=>({month:J.month,amount:L(J)})),I=r.map(J=>({month:J.month,amount:L(J)}));const V=D.reduce((J,X)=>J+X.amount,0),ee=I.reduce((J,X)=>J+X.amount,0),$e=ee>0?(V-ee)/ee*100:0,ge=$e>0?"+":"";O=`${E[n]} ${A}（${s}）`,B=`${M(V)}${ee>0?` / 前年比 ${ge}${$e.toFixed(1)}%`:""}`,z="#2f855a"}else{D=Hl(e.monthlySales,c,m).curr,I=[];const ee=D.reduce((ge,J)=>ge+J.amount,0);O=`${m==="fiscal"?"決算年度別":"暦年別"}${A}`,B=`累計 ${M(ee)}（${D.length}${m==="fiscal"?"期":"年"}）`,z="#0F5B8D"}const H=["amount","quantity","volume"].map(V=>`<button class="tab-button ${V===c?"active":""}" data-chart-metric="${V}">${cn[V]}</button>`).join(""),F=["all","yearly","monthly","weekly","daily"].map(V=>`<button class="button ${V===n?"primary":"secondary"} small" type="button" data-analytics-period="${V}">${dn[V]}</button>`).join(""),j=m==="fiscal"&&n==="yearly"?Kl(e.monthlySales):l,Q=m==="fiscal"&&n==="yearly"&&!j.includes(s)?j[0]??"":s,Z=n!=="all"&&j.length>0&&t!=="staff"?`<select id="analytics-period-select" style="padding:6px 10px;border:1px solid var(--border);border-radius:6px;font-size:13px;">
        ...
        </select>`:"",ge=(b.length>0?b:e.staffTotals).filter(X=>!d||X.name.includes(d)||X.code.includes(d)),J=y!=="all"&&v?` (${v})`:"";if(re=`
...
`}async function Ns(e,t){const{supabaseInsert:n}=await q(async()=>{const{supabaseInsert:p}=await Promise.resolve().then(()=>G);return{supabaseInsert:p}},void 0);let s=0,i=0;const d={customers:"customers",products:"products",suppliers:"suppliers",staff:"staff"}[e];for(const p of t){if(!p._valid)continue;const{_valid:u,_error:y,...v}=p,f={...v};if(!f.id){const b=e==="customers"?"legacy_customer_code":e==="products"?"legacy_product_code":e==="suppliers"?"legacy_supplier_code":"legacy_staff_code";f.id=String(v[b]??`${e}-${Date.now()}-${s+i}`)}for(const b of["volume_ml","closing_day","payment_day","list_price","default_sale_price"])if(typeof f[b]=="string"&&f[b]!==""){const k=Number(f[b]);Number.isFinite(k)&&(f[b]=k)}try{await n(d,f)!==null?s++:i++}catch{i++}}return{inserted:s,failed:i}}const Vc=Object.freeze(Object.defineProperty({__proto__:null,generateTemplateCSV:Ts,importToSupabase:Ns,parseCSV:qs,validateImport:Is},Symbol.toStringTag,{value:"Module"}));function Wt(e){return new Intl.DateTimeFormat("ja-JP",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"}).format(new Date(e))}function Yc(e,t=40){return!e||e.length<=t?e||"":e.slice(0,t)+"…"}function Jc(e,t,n,s,i){const l=n.reduce((y,v)=>y+v.rowCount,0),d=n.map(y=>y.lastSyncAt).filter(y=>y!==null).sort().reverse()[0]??null,p=100,u=Math.max(1,Math.ceil(i/p));return`
  ...
  `}function Ms(e){const[t,n]=e.split("-").map(Number),s=new Date(t,n,0).getDate();return Array.from({length:s},(i,l)=>{const d=l+1;return`${e}-${String(d).padStart(2,"0")}`})}function bn(e){const t=new Date(e).getDay();return["日","月","火","水","木","金","土"][t]}function $n(e){const t=new Date(e).getDay();return t===0||t===6}function Zc(e,t){return e.partTimers*t.partCapacity+e.employees*t.empCapacity}function Os(e){return e.partTimers+e.employees}function Ce(e,t,n={partCapacity:tt,empCapacity:at}){const s=e.filter(v=>v.partTimers>0||v.employees>0);if(s.length===0)return;const i=t.reduce((v,f)=>{const b=f.plannedQty>0?f.plannedQty:Math.max(0,f.demandForecast+f.safetyStockTarget-f.openingStock);return v+b},0);if(i<=0)return;const l=i/s.length;let d=0,p=0,u=1/0;const y=Math.ceil(l/n.partCapacity);for(let v=0;v<=y;v++){const f=l-v*n.partCapacity,b=f>0?Math.ceil(f/n.empCapacity):0,k=v+b;k<u&&(u=k,d=v,p=b)}for(const v of e)v.confirmed||(v.partTimers>0||v.employees>0)&&(v.partTimers=d,v.employees=p)}function ed(e,t,n={partCapacity:tt,empCapacity:at}){const s=t.filter(p=>Os(p)>0).map(p=>p.date).sort();if(s.length===0)return t.map(p=>({date:p.date,partTimers:p.partTimers,employees:p.employees,confirmed:p.confirmed,capacity:0,items:[],totalQty:0,utilization:0}));const i={monthly:0,november:1,annual:2,make_to_order:3},l=e.filter(p=>p.plannedQty>0||Math.max(0,p.demandForecast+p.safetyStockTarget-p.openingStock)>0).map(p=>({productCode:p.productCode,productName:p.productName,productionType:p.productionType,remaining:p.plannedQty>0?p.plannedQty:Math.max(0,p.demandForecast+p.safetyStockTarget-p.openingStock)})).filter(p=>p.remaining>0).sort((p,u)=>(i[p.productionType]??99)-(i[u.productionType]??99)||u.remaining-p.remaining),d=new Map;for(const p of t){const u=Zc(p,n);d.set(p.date,{date:p.date,partTimers:p.partTimers,employees:p.employees,confirmed:p.confirmed,capacity:u,items:[],totalQty:0,utilization:0})}for(const p of l){let u=p.remaining;if(u<=0)continue;if(s.reduce((v,f)=>{const b=d.get(f);return v+Math.max(0,b.capacity-b.totalQty)},0)<=0)break;for(const v of s){if(u<=0)break;const f=d.get(v),b=Math.max(0,f.capacity-f.totalQty);if(b<=0)continue;const k=Math.min(u,b);f.items.push({productCode:p.productCode,productName:p.productName,productionType:p.productionType,qty:k}),f.totalQty+=k,f.utilization=f.capacity>0?f.totalQty/f.capacity:0,u-=k}}return t.map(p=>d.get(p.date))}function qt(e,t=1,n=1){return Ms(e).map(s=>({date:s,partTimers:$n(s)?0:t,employees:$n(s)?0:n,confirmed:!1}))}function td(e,t,n,s=null,i=new Set,l={partCapacity:tt,empCapacity:at}){const d=Ms(t),p=e.filter(D=>!i.has(D.productCode)),u=ed(p,n,l),y=new Map(u.map(D=>[D.date,D])),v=p.reduce((D,I)=>D+(I.plannedQty>0?I.plannedQty:Math.max(0,I.demandForecast+I.safetyStockTarget-I.openingStock)),0),b=e.reduce((D,I)=>D+(I.plannedQty>0?I.plannedQty:Math.max(0,I.demandForecast+I.safetyStockTarget-I.openingStock)),0)-v,k=u.reduce((D,I)=>D+I.totalQty,0),P=n.filter(D=>Os(D)>0).length,o=u.reduce((D,I)=>D+I.capacity,0),r=n.reduce((D,I)=>D+I.partTimers,0),c=n.reduce((D,I)=>D+I.employees,0),m=P>0?Math.ceil(v/P):0,g=new Date,$=Array.from({length:24},(D,I)=>{const O=new Date(g.getFullYear(),g.getMonth()-6+I,1),B=`${O.getFullYear()}-${String(O.getMonth()+1).padStart(2,"0")}`;return`<option value="${B}" ${B===t?"selected":""}>${B.replace("-","年")}月</option>`}).join(""),w=new Date(d[0]).getDay(),_=[];for(let D=0;D<w;D++)_.push('<div style="min-height:44px;"></div>');for(const D of d){const I=y.get(D),O=new Date(D).getDay(),B=parseInt(D.split("-")[2]),z=I?.partTimers??0,H=I?.employees??0,F=z+H,j=I?.totalQty??0,Q=I?.utilization??0,Z=D===s,re=F===0?"var(--surface-alt)":Q>.95?"rgba(197,61,61,0.12)":Q>.7?"rgba(183,121,31,0.10)":Q>0?"rgba(47,133,90,0.08)":"var(--surface)",ie=F===0?"transparent":Q>.95?"#c53d3d":Q>.7?"#b7791f":Q>0?"#2f855a":"var(--border)",V=O===0?"#c53d3d":O===6?"#0F5B8D":"var(--text)",ee=F>0?`<span style="font-size:8px;color:var(--text-secondary);line-height:1;">${z>0?`パ${z}`:""}${H>0?`社${H}`:""}</span>`:"";_.push(`
    ...
    `)}const E=_.length%7;if(E>0)for(let D=0;D<7-E;D++)_.push('<div style="min-height:44px;"></div>');const A=s?y.get(s):null;s&&n.find(D=>D.date===s);const L=s&&A?(()=>{const D=A,I=parseInt(s.split("-")[2]),O=bn(s),B=Math.round(D.utilization*100),z=n.find(J=>J.date===s),H=s===new Date().toISOString().slice(0,10),F={monthly:"#0F5B8D",november:"#B7791F",annual:"#6B46C1",make_to_order:"#999"},j={monthly:"月次",november:"11月",annual:"年次",make_to_order:"受注"},Q=D.items.map(J=>`
    ...
    `).join(""),Z=`パ${D.partTimers}×${l.partCapacity} 社${D.employees}×${l.empCapacity} = ${W(D.capacity)}本`,re=D.totalQty>0?Math.ceil(D.totalQty/l.partCapacity):0,ie=[];if(D.totalQty>0)for(let J=0;J<=re;J++){const X=D.totalQty-J*l.partCapacity;if(X<=0){ie.push({p:J,e:0});break}const qe=Math.ceil(X/l.empCapacity);ie.push({p:J,e:qe})}const V=D.totalQty-D.capacity,ee=D.totalQty===0?"":V>0?`<span style="color:#c53d3d;font-weight:600;">⚠ ${W(V)}本 不足</span>`:'<span style="color:#2f855a;">✓ キャパ内</span>',$e=ie.filter(J=>J.p+J.e>0).sort((J,X)=>J.p+J.e-(X.p+X.e)).slice(0,3),ge=D.totalQty>0?`
      <div style="background:var(--surface-alt);border-radius:6px;padding:10px 12px;margin:0 4px 8px;">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
          ${$e.map((J,X)=>{const qe=J.p===D.partTimers&&J.e===D.employees;return`<button data-action="cal-apply-pattern" data-date="${s}" data-part="${J.p}" data-emp="${J.e}"
