...
`}function ks(e){const t=String(e);return/[,"\n]/.test(t)?`"${t.replaceAll('"','""')}"`:t}function _s(e){const n=["酒類コード","区分名","アルコール度数","製造数量","前月繰越","当月調整","輸出控除","見本等控除","課税数量","税率(円/L)","税額(円)"].join(","),i=e.rows.map(a=>[a.taxCategory,a.taxCategoryName,a.alcoholDegree,a.productionVolume,a.previousBalance,a.currentAdjustment,a.exportDeduction,a.sampleDeduction,a.taxableVolume,a.taxRate,a.taxAmount].map(ks).join(",")),c=`,合計,,${e.rows.reduce((a,l)=>a+l.productionVolume,0)},,,${e.rows.reduce((a,l)=>a+l.exportDeduction,0)},${e.rows.reduce((a,l)=>a+l.sampleDeduction,0)},${e.totalVolume},,${e.totalTax}`;return"\uFEFF"+[n,...i,c].join(`
`)+`
`}function Ss(e){const t=e.rows.map(c=>{const a=Math.max(0,c.productionVolume+c.previousBalance+c.currentAdjustment-c.exportDeduction-c.sampleDeduction),l=Math.round(a*c.taxRate);return{...c,taxableVolume:a,volume:a,taxAmount:l}}),n=t.reduce((c,a)=>c+a.taxableVolume,0),i=t.reduce((c,a)=>c+a.taxAmount,0);return{...e,rows:t,totalVolume:n,totalTax:i}}async function xs(e){const{supabaseInsert:t}=await f(async()=>{const{supabaseInsert:n}=await Promise.resolve().then(()=>E);return{supabaseInsert:n}},void 0);await t("tax_declarations",{target_year:e.targetYear,target_month:e.targetMonth,company_name:e.companyName,company_no:e.companyNo,company_address:e.companyAddress,company_representative:e.companyRepresentative,tax_office:e.taxOffice,total_taxable_volume:e.totalVolume,total_tax_amount:e.totalTax,status:e.status,xml_data:xa(e),submitted_at:e.submittedAt})}const As=Array.from({length:10},(e,t)=>({id:`ss${t+1}`,saleDate:"2026-04-15",saleTime:`${9+t}:${String(t*7%60).padStart(2,"0")}`,productCode:`P${String(t%4+1).padStart(5,"0")}`,productName:["純米吟醸 720ml","本醸造 1.8L","梅酒 500ml","特別純米 300ml"][t%4],quantity:1+t%3,unitPrice:[2200,1800,980,680][t%4],amount:(1+t%3)*[2200,1800,980,680][t%4],paymentMethod:["cash","card","paypay","cash"][t%4]})),Cs=[{id:"o1",orderNo:"ORD-2604001",orderDate:"2026-04-13",customerName:"鈴木 太郎",postalCode:"150-0001",address:"東京都渋谷区〇〇1-1",items:[{productName:"純米吟醸 720ml",quantity:2,amount:4400}],totalAmount:4400,status:"shipped",shippingDate:"2026-04-14"},{id:"o2",orderNo:"ORD-2604002",orderDate:"2026-04-14",customerName:"田中 花子",postalCode:"530-0001",address:"大阪府大阪市北区〇〇2-3",items:[{productName:"梅酒 500ml",quantity:3,amount:2940},{productName:"本醸造 1.8L",quantity:1,amount:1800}],totalAmount:4740,status:"processing",shippingDate:""},{id:"o3",orderNo:"ORD-2604003",orderDate:"2026-04-15",customerName:"佐藤 一郎",postalCode:"460-0001",address:"愛知県名古屋市中区〇〇3-5",items:[{productName:"特別純米 300ml ×6本セット",quantity:1,amount:3980}],totalAmount:3980,status:"new",shippingDate:""}];async function ft(e){return P(`data/api/latest/store-sales-${e}.json`,As)}async function Aa(){return P("data/api/latest/store-orders.json",Cs)}async function Ee(e){const t=await qe("email_campaigns",{subject:e.subject,body:e.body,template_id:e.templateId,audience_mode:e.audienceMode,audience_filter:e.audienceFilter,recipient_count:e.recipientCount,sent_count:e.status==="sent"?e.recipientCount:0,status:e.status,sent_at:e.status==="sent"?new Date().toISOString():null});return{id:t?.id??`local-email-${Date.now()}`,subject:t?.subject??e.subject,body:t?.body??e.body,templateId:t?.template_id??e.templateId,audienceMode:t?.audience_mode??e.audienceMode,audienceFilter:t?.audience_filter??e.audienceFilter,recipientCount:t?.recipient_count??e.recipientCount,status:t?.status??e.status,createdAt:t?.created_at??new Date().toISOString(),updatedAt:t?.updated_at??new Date().toISOString()}}async function Ca(e,t){throw new Error("VITE_RESEND_API_KEY is not configured")}async function Ls(e){const t={order:"updated_at.desc"};return e&&(t.template_key=`eq.${e}`),(await _("print_layouts",t)).map(i=>({id:m(i,["id"],""),name:m(i,["name"],""),templateKey:m(i,["template_key"],""),positions:i.positions??{},isDefault:j(i,["is_default"],!1),note:m(i,["note"],""),updatedAt:m(i,["updated_at"],"")}))}async function Ds(e){const{supabaseInsert:t}=await f(async()=>{const{supabaseInsert:c}=await Promise.resolve().then(()=>E);return{supabaseInsert:c}},void 0),n={id:e.id,name:e.name,template_key:e.templateKey,positions:e.positions,is_default:e.isDefault??!1,note:e.note??"",updated_at:new Date().toISOString()},i=await t("print_layouts",n);return i?{id:m(i,["id"],e.id),name:m(i,["name"],e.name),templateKey:m(i,["template_key"],e.templateKey),positions:i.positions??e.positions,isDefault:j(i,["is_default"],!1),note:m(i,["note"],""),updatedAt:m(i,["updated_at"],"")}:null}async function Es(e){const t=new URL("/rest/v1/print_layouts","https://loarwnuyvfxiscjjsmiz.supabase.co");t.searchParams.set("id",`eq.${e}`);const n="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxvYXJ3bnV5dmZ4aXNjampzbWl6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzYyMTM0MDYsImV4cCI6MjA5MTc4OTQwNn0.7XofVlIiTjlzA9Oz-3lTX1ghLL1qqCcDRadf_a__ys8 ";try{return(await fetch(t.toString(),{method:"DELETE",headers:{apikey:n,Authorization:`Bearer ${n}`}})).ok}catch{return!1}}async function Is(){return(await _("mail_senders",{order:"is_default.desc,name.asc"})).map(t=>({id:m(t,["id"],""),name:m(t,["name"],""),email:m(t,["email"],""),displayName:m(t,["display_name"],""),signature:m(t,["signature"],""),replyTo:m(t,["reply_to"],""),isDefault:j(t,["is_default"],!1),isVerified:j(t,["is_verified"],!1),note:m(t,["note"],"")}))}async function Ps(e){const{supabaseInsert:t}=await f(async()=>{const{supabaseInsert:i}=await Promise.resolve().then(()=>E);return{supabaseInsert:i}},void 0),n=await t("mail_senders",{id:e.id,name:e.name,email:e.email,display_name:e.displayName??"",signature:e.signature??"",reply_to:e.replyTo??"",is_default:e.isDefault??!1,is_verified:e.isVerified??!1,note:e.note??"",updated_at:new Date().toISOString()});return n?{id:m(n,["id"],e.id),name:m(n,["name"],e.name),email:m(n,["email"],e.email),displayName:m(n,["display_name"],""),signature:m(n,["signature"],""),replyTo:m(n,["reply_to"],""),isDefault:j(n,["is_default"],!1),isVerified:j(n,["is_verified"],!1)}:null}async function Ts(e){const t="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxvYXJ3bnV5dmZ4aXNjampzbWl6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzYyMTM0MDYsImV4cCI6MjA5MTc4OTQwNn0.7XofVlIiTjlzA9Oz-3lTX1ghLL1qqCcDRadf_a__ys8 ";try{const n=new URL("/rest/v1/mail_senders","https://loarwnuyvfxiscjjsmiz.supabase.co");return n.searchParams.set("id",`eq.${e}`),(await fetch(n.toString(),{method:"DELETE",headers:{apikey:t,Authorization:`Bearer ${t}`}})).ok}catch{return!1}}const bt={delivery:"🚚 納品",tour:"🏭 蔵見学",meeting:"📋 商談",brewing:"🍶 仕込",general:"📌 その他"},gt={delivery:"#9C27B0",tour:"#FF9800",meeting:"#2196F3",brewing:"#4CAF50",general:"#0F5B8D"};async function Ns(e){const t=`${e}-01T00:00:00Z`,[n,i]=e.split("-").map(o=>parseInt(o,10)),c=new Date(n,i,0).getDate(),a=`${e}-${String(c).padStart(2,"0")}T23:59:59Z`;return(await _("calendar_events",{starts_at:`gte.${t}`,and:`(starts_at.lte.${a})`,order:"starts_at.asc"})).map(o=>({id:m(o,["id"],""),title:m(o,["title"],""),description:m(o,["description"],""),category:m(o,["category"],"general")||"general",startsAt:m(o,["starts_at"],new Date().toISOString()),endsAt:m(o,["ends_at"],""),isAllDay:j(o,["is_all_day"],!1),location:m(o,["location"],""),attendees:o.attendees??[],relatedCustomerCode:m(o,["related_customer_code"],""),relatedOrderId:m(o,["related_order_id"],""),color:m(o,["color"],""),googleEventId:m(o,["google_event_id"],"")}))}async function js(e){const{supabaseInsert:t}=await f(async()=>{const{supabaseInsert:i}=await Promise.resolve().then(()=>E);return{supabaseInsert:i}},void 0);return await t("calendar_events",{id:e.id,title:e.title,description:e.description??"",category:e.category,starts_at:e.startsAt,ends_at:e.endsAt||null,is_all_day:e.isAllDay??!1,location:e.location??"",attendees:e.attendees??[],related_customer_code:e.relatedCustomerCode??null,related_order_id:e.relatedOrderId??null,color:e.color??gt[e.category],updated_at:new Date().toISOString()})?e:null}async function Rs(e){const t="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxvYXJ3bnV5dmZ4aXNjampzbWl6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzYyMTM0MDYsImV4cCI6MjA5MTc4OTQwNn0.7XofVlIiTjlzA9Oz-3lTX1ghLL1qqCcDRadf_a__ys8 ";try{const n=new URL("/rest/v1/calendar_events","https://loarwnuyvfxiscjjsmiz.supabase.co");return n.searchParams.set("id",`eq.${e}`),(await fetch(n.toString(),{method:"DELETE",headers:{apikey:t,Authorization:`Bearer ${t}`}})).ok}catch{return!1}}async function La(){return(await _("integration_settings",{order:"name.asc"})).map(t=>({id:m(t,["id"],""),name:m(t,["name"],""),provider:m(t,["provider"],""),config:t.config??{},isEnabled:j(t,["is_enabled"],!1),lastSyncAt:m(t,["last_sync_at"],""),lastStatus:m(t,["last_status"],"")}))}async function ze(e){const{supabaseInsert:t}=await f(async()=>{const{supabaseInsert:i}=await Promise.resolve().then(()=>E);return{supabaseInsert:i}},void 0);return await t("integration_settings",{id:e.id,name:e.name,provider:e.provider,config:e.config,is_enabled:e.isEnabled,last_sync_at:e.lastSyncAt||null,last_status:e.lastStatus||null,updated_at:new Date().toISOString()})?e:null}async function Os(e){const t=e.config.shop_domain,n=e.config.admin_token;if(!t||!n)return{count:0,error:"shop_domain と admin_token を設定してください"};try{const i=`https://${t}/admin/api/2024-01/orders.json?status=any&limit=50`,c=await fetch(i,{headers:{"X-Shopify-Access-Token":n,"Content-Type":"application/json"}});if(!c.ok)return{count:0,error:`HTTP ${c.status}`};const a=await c.json(),{supabaseInsert:l}=await f(async()=>{const{supabaseInsert:r}=await Promise.resolve().then(()=>E);return{supabaseInsert:r}},void 0);let o=0;for(const r of a.orders){const d=`shopify_${r.id}`;await l("shopify_orders",{id:d,shopify_order_id:String(r.id),order_number:String(r.order_number??""),order_date:String(r.created_at??new Date().toISOString()),customer_name:String(r.customer?.first_name??"")+" "+String(r.customer?.last_name??""),customer_email:String(r.customer?.email??""),total_amount:Math.round(parseFloat(String(r.total_price??"0"))),financial_status:String(r.financial_status??""),fulfillment_status:String(r.fulfillment_status??"unfulfilled"),line_items:r.line_items??[],shipping_address:r.shipping_address??null,raw_payload:r}),o++}return await ze({...e,lastSyncAt:new Date().toISOString(),lastStatus:`${o}件取得成功`}),{count:o}}catch(i){return{count:0,error:i instanceof Error?i.message:String(i)}}}async function qs(){return(await _("shopify_orders",{order:"order_date.desc",limit:"50"})).map(t=>({id:m(t,["id"],""),shopifyOrderId:m(t,["shopify_order_id"],""),orderNumber:m(t,["order_number"],""),orderDate:m(t,["order_date"],""),customerName:m(t,["customer_name"],""),customerEmail:m(t,["customer_email"],""),totalAmount:C(t.total_amount),financialStatus:m(t,["financial_status"],""),fulfillmentStatus:m(t,["fulfillment_status"],""),lineItems:t.line_items??[]}))}async function Ms(e){const t=e.config.oauth_token,n=e.config.calendar_id||"primary";if(!t)return{count:0,error:"oauth_token を設定してください (Google Cloud Console で取得)"};try{const i=new Date().toISOString(),c=new Date(Date.now()+30*86400*1e3).toISOString(),a=`https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(n)}/events?timeMin=${i}&timeMax=${c}&singleEvents=true&orderBy=startTime`,l=await fetch(a,{headers:{Authorization:`Bearer ${t}`}});if(!l.ok)return{count:0,error:`HTTP ${l.status}`};const o=await l.json(),{supabaseInsert:r}=await f(async()=>{const{supabaseInsert:u}=await Promise.resolve().then(()=>E);return{supabaseInsert:u}},void 0);let d=0;for(const u of o.items){const p=`gcal_${u.id}`,h=u.start?.dateTime??u.start?.date??"",v=u.end?.dateTime??u.end?.date??"";await r("calendar_events",{id:p,title:String(u.summary??"(無題)"),description:String(u.description??""),category:"general",starts_at:String(h),ends_at:String(v),location:String(u.location??""),google_event_id:String(u.id??""),updated_at:new Date().toISOString()}),d++}return await ze({...e,lastSyncAt:new Date().toISOString(),lastStatus:`${d}件取得`}),{count:d}}catch(i){return{count:0,error:i instanceof Error?i.message:String(i)}}}async function Fs(){return(await _("fax_inbox",{order:"received_at.desc",limit:"50"})).map(t=>({id:m(t,["id"],""),receivedAt:m(t,["received_at"],""),senderPhone:m(t,["sender_phone"],""),senderName:m(t,["sender_name"],""),imageUrl:m(t,["image_url"],""),ocrStatus:m(t,["ocr_status"],"pending")||"pending",ocrText:m(t,["ocr_text"],""),extractedData:t.extracted_data??{},linkedInvoiceId:m(t,["linked_invoice_id"],"")}))}async function Vs(e,t){const n=e.config.api_key;if(!n)return{text:"",error:"Cloud Vision API key 未設定"};try{const i=`https://vision.googleapis.com/v1/images:annotate?key=${n}`,c=await fetch(i,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({requests:[{image:{content:t.replace(/^data:image\/\w+;base64,/,"")},features:[{type:"DOCUMENT_TEXT_DETECTION"}],imageContext:{languageHints:["ja"]}}]})});return c.ok?{text:(await c.json()).responses?.[0]?.fullTextAnnotation?.text??""}:{text:"",error:`HTTP ${c.status}`}}catch(i){return{text:"",error:i instanceof Error?i.message:String(i)}}}async function zs(e){const{supabaseInsert:t}=await f(async()=>{const{supabaseInsert:i}=await Promise.resolve().then(()=>E);return{supabaseInsert:i}},void 0);return await t("fax_inbox",{id:e.id,received_at:e.receivedAt,sender_phone:e.senderPhone||null,sender_name:e.senderName||null,image_url:e.imageUrl||null,ocr_status:e.ocrStatus,ocr_text:e.ocrText||null,extracted_data:e.extractedData||null,linked_invoice_id:e.linkedInvoiceId||null})?e:null}const Ne={admin:"👑 管理者",manager:"📋 マネージャー",staff:"👤 スタッフ"},je={all:"全体",sales:"営業",brewery:"蔵人",management:"管理"};async function Js(){return(await _("user_profiles",{order:"display_name.asc"})).map(t=>({id:m(t,["id"],""),email:m(t,["email"],""),displayName:m(t,["display_name"],""),staffCode:m(t,["staff_code"],""),department:m(t,["department"],"all")||"all",role:m(t,["role"],"staff")||"staff",defaultMailSenderId:m(t,["default_mail_sender_id"],""),phone:m(t,["phone"],""),avatarUrl:m(t,["avatar_url"],""),isActive:j(t,["is_active"],!0),lastSignInAt:m(t,["last_sign_in_at"],""),createdAt:m(t,["created_at"],"")}))}async function Bs(e){if(!e)return null;const t=await _("user_profiles",{email:`eq.${e}`});if(t.length===0)return null;const n=t[0];return{id:m(n,["id"],""),email:m(n,["email"],""),displayName:m(n,["display_name"],""),staffCode:m(n,["staff_code"],""),department:m(n,["department"],"all")||"all",role:m(n,["role"],"staff")||"staff",defaultMailSenderId:m(n,["default_mail_sender_id"],""),phone:m(n,["phone"],""),avatarUrl:m(n,["avatar_url"],""),isActive:j(n,["is_active"],!0),lastSignInAt:m(n,["last_sign_in_at"],"")}}async function Us(e){const{supabaseInsert:t}=await f(async()=>{const{supabaseInsert:i}=await Promise.resolve().then(()=>E);return{supabaseInsert:i}},void 0);return await t("user_profiles",{id:e.id,email:e.email,display_name:e.displayName,staff_code:e.staffCode||null,department:e.department,role:e.role,default_mail_sender_id:e.defaultMailSenderId||null,phone:e.phone||null,avatar_url:e.avatarUrl||null,is_active:e.isActive,updated_at:new Date().toISOString()})?e:null}async function Ys(e){const t="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxvYXJ3bnV5dmZ4aXNjampzbWl6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzYyMTM0MDYsImV4cCI6MjA5MTc4OTQwNn0.7XofVlIiTjlzA9Oz-3lTX1ghLL1qqCcDRadf_a__ys8 ";try{const n=new URL("/rest/v1/user_profiles","https://loarwnuyvfxiscjjsmiz.supabase.co");return n.searchParams.set("id",`eq.${e}`),(await fetch(n.toString(),{method:"DELETE",headers:{apikey:t,Authorization:`Bearer ${t}`}})).ok}catch{return!1}}async function Hs(e){const{supabaseInsert:t}=await f(async()=>{const{supabaseInsert:n}=await Promise.resolve().then(()=>E);return{supabaseInsert:n}},void 0);await t("audit_logs",{action:e.action,entity_type:e.entityType??null,entity_id:e.entityId??null,user_email:e.userEmail??null,changes:e.changes??null})}async function Xs(e=100){return(await _("audit_logs",{order:"created_at.desc",limit:String(e)})).map(n=>({id:m(n,["id"],""),action:m(n,["action"],""),entityType:m(n,["entity_type"],""),entityId:m(n,["entity_id"],""),userEmail:m(n,["user_email"],""),changes:n.changes??{},createdAt:m(n,["created_at"],"")}))}const Re={new_order:"🛒 新規受注",payment_overdue:"⚠️ 入金遅延",low_stock:"📦 低在庫",fax_received:"📠 FAX受信",tour_inquiry:"🏭 見学問合せ",new_prospect:"🎯 新規見込客"};async function Da(){return(await _("slack_notifications",{order:"event_type.asc"})).map(t=>({id:m(t,["id"],""),eventType:m(t,["event_type"],"new_order"),enabled:j(t,["enabled"],!0),channel:m(t,["channel"],""),condition:t.condition??{},lastTriggeredAt:m(t,["last_triggered_at"],"")}))}async function Gs(e){const{supabaseInsert:t}=await f(async()=>{const{supabaseInsert:i}=await Promise.resolve().then(()=>E);return{supabaseInsert:i}},void 0);return await t("slack_notifications",{id:e.id,event_type:e.eventType,enabled:e.enabled,channel:e.channel,condition:e.condition})?e:null}async function Ks(e=50){return(await _("slack_notification_logs",{order:"sent_at.desc",limit:String(e)})).map(n=>({id:m(n,["id"],""),eventType:m(n,["event_type"],""),channel:m(n,["channel"],""),message:m(n,["message"],""),status:m(n,["status"],"sent"),error:m(n,["error"],""),sentAt:m(n,["sent_at"],"")}))}async function Qs(e,t,n){const c=(await La()).find(d=>d.provider==="slack");if(!c||!c.isEnabled)return{ok:!1,error:"Slack連携が無効です"};const a=c.config.webhook_url;if(!a)return{ok:!1,error:"Webhook URL未設定"};const o=(await Da()).find(d=>d.eventType===e&&d.enabled);if(!o)return{ok:!1,error:"通知ルールが無効"};const r=n??o.channel??c.config.default_channel??"#general";try{const d=await fetch(a,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:`${Re[e]} ${t}`,channel:r})}),u=d.ok,{supabaseInsert:p}=await f(async()=>{const{supabaseInsert:h}=await Promise.resolve().then(()=>E);return{supabaseInsert:h}},void 0);return await p("slack_notification_logs",{id:`slack_${Date.now()}`,event_type:e,channel:r,message:t,status:u?"sent":"failed",error:u?null:`HTTP ${d.status}`}),u?{ok:!0}:{ok:!1,error:`HTTP ${d.status}`}}catch(d){return{ok:!1,error:d instanceof Error?d.message:String(d)}}}const fe={cold:"❄️ 未接触",warm:"🌡️ 関心あり",hot:"🔥 見込み高",contacted:"📞 アプローチ中",negotiating:"💬 商談中",won:"🎉 受注",lost:"💔 失注"},Je={cold:"#90A4AE",warm:"#FFA726",hot:"#EF5350",contacted:"#42A5F5",negotiating:"#AB47BC",won:"#66BB6A",lost:"#757575"};async function Ws(){return(await _("prospects",{order:"updated_at.desc"})).map(t=>({id:m(t,["id"],""),companyName:m(t,["company_name"],""),contactName:m(t,["contact_name"],""),email:m(t,["email"],""),phone:m(t,["phone"],""),address:m(t,["address"],""),website:m(t,["website"],""),businessType:m(t,["business_type"],""),stage:m(t,["stage"],"cold"),source:m(t,["source"],""),expectedAmount:C(t.expected_amount),probability:C(t.probability),assignedStaffCode:m(t,["assigned_staff_code"],""),nextActionDate:m(t,["next_action_date"],""),nextAction:m(t,["next_action"],""),note:m(t,["note"],""),lastContactAt:m(t,["last_contact_at"],""),wonAt:m(t,["won_at"],""),lostAt:m(t,["lost_at"],""),lostReason:m(t,["lost_reason"],""),convertedCustomerCode:m(t,["converted_customer_code"],""),createdAt:m(t,["created_at"],"")}))}async function Ea(e){const{supabaseInsert:t}=await f(async()=>{const{supabaseInsert:i}=await Promise.resolve().then(()=>E);return{supabaseInsert:i}},void 0);return await t("prospects",{id:e.id,company_name:e.companyName,contact_name:e.contactName||null,email:e.email||null,phone:e.phone||null,address:e.address||null,website:e.website||null,business_type:e.businessType||null,stage:e.stage,source:e.source||null,expected_amount:e.expectedAmount,probability:e.probability,assigned_staff_code:e.assignedStaffCode||null,next_action_date:e.nextActionDate||null,next_action:e.nextAction||null,note:e.note||null,updated_at:new Date().toISOString()})?e:null}async function Zs(e){const t="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxvYXJ3bnV5dmZ4aXNjampzbWl6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzYyMTM0MDYsImV4cCI6MjA5MTc4OTQwNn0.7XofVlIiTjlzA9Oz-3lTX1ghLL1qqCcDRadf_a__ys8 ";try{const n=new URL("/rest/v1/prospects","https://loarwnuyvfxiscjjsmiz.supabase.co");return n.searchParams.set("id",`eq.${e}`),(await fetch(n.toString(),{method:"DELETE",headers:{apikey:t,Authorization:`Bearer ${t}`}})).ok}catch{return!1}}async function en(e){return(await _("prospect_activities",{prospect_id:`eq.${e}`,order:"activity_date.desc"})).map(n=>({id:m(n,["id"],""),prospectId:m(n,["prospect_id"],""),activityType:m(n,["activity_type"],"call"),title:m(n,["title"],""),description:m(n,["description"],""),activityDate:m(n,["activity_date"],""),result:m(n,["result"],""),staffCode:m(n,["staff_code"],"")}))}async function tn(e){const{supabaseInsert:t}=await f(async()=>{const{supabaseInsert:i}=await Promise.resolve().then(()=>E);return{supabaseInsert:i}},void 0);return await t("prospect_activities",{id:e.id,prospect_id:e.prospectId,activity_type:e.activityType,title:e.title||null,description:e.description||null,activity_date:e.activityDate,result:e.result||null,staff_code:e.staffCode||null})?e:null}const Ia=["瓶 (720ml)","瓶 (1.8L)","瓶 (300ml)","瓶 (500ml)","キャップ・栓","ラベル(表)","ラベル(裏)","首掛け","化粧箱","ダンボール","包装紙","熨斗・水引","和紙","リボン","その他"];async function an(e){const{supabaseInsert:t}=await f(async()=>{const{supabaseInsert:i}=await Promise.resolve().then(()=>E);return{supabaseInsert:i}},void 0);return await t("materials",{id:e.id,legacy_material_code:e.code,material_code:e.code,name:e.name,unit:e.unit,material_type:e.materialType||null,current_stock:e.currentStock,minimum_stock:e.minimumStock,unit_cost:e.unitCost,last_purchase_date:e.lastUpdated,is_active:!0,updated_at:new Date().toISOString()})?e:null}async function sn(e){const t="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxvYXJ3bnV5dmZ4aXNjampzbWl6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzYyMTM0MDYsImV4cCI6MjA5MTc4OTQwNn0.7XofVlIiTjlzA9Oz-3lTX1ghLL1qqCcDRadf_a__ys8 ";try{const n=new URL("/rest/v1/materials","https://loarwnuyvfxiscjjsmiz.supabase.co");return n.searchParams.set("id",`eq.${e}`),(await fetch(n.toString(),{method:"DELETE",headers:{apikey:t,Authorization:`Bearer ${t}`}})).ok}catch{return!1}}async function nn(){return(await _("delivery_locations",{order:"name.asc"})).map(t=>({id:m(t,["id"],""),customerCode:m(t,["customer_code"],""),name:m(t,["name"],""),postalCode:m(t,["postal_code"],""),address:m(t,["address"],""),lat:t.lat?Number(t.lat):void 0,lng:t.lng?Number(t.lng):void 0,contactName:m(t,["contact_name"],""),phone:m(t,["phone"],""),deliveryNote:m(t,["delivery_note"],""),isActive:j(t,["is_active"],!0)}))}async function ln(e){const{supabaseInsert:t}=await f(async()=>{const{supabaseInsert:i}=await Promise.resolve().then(()=>E);return{supabaseInsert:i}},void 0);return await t("delivery_locations",{id:e.id,customer_code:e.customerCode||null,name:e.name,postal_code:e.postalCode||null,address:e.address||null,lat:e.lat??null,lng:e.lng??null,contact_name:e.contactName||null,phone:e.phone||null,delivery_note:e.deliveryNote||null,is_active:e.isActive})?e:null}async function on(e=50){return(await _("call_logs",{order:"started_at.desc",limit:String(e)})).map(n=>({id:m(n,["id"],""),callDirection:m(n,["call_direction"],"inbound"),fromNumber:m(n,["from_number"],""),toNumber:m(n,["to_number"],""),matchedCustomerCode:m(n,["matched_customer_code"],""),matchedProspectId:m(n,["matched_prospect_id"],""),durationSeconds:C(n.duration_seconds),callStatus:m(n,["call_status"],"answered"),recordingUrl:m(n,["recording_url"],""),transcript:m(n,["transcript"],""),ivryCallId:m(n,["ivry_call_id"],""),startedAt:m(n,["started_at"],""),endedAt:m(n,["ended_at"],""),notes:m(n,["notes"],"")}))}async function Pa(e){const{supabaseInsert:t}=await f(async()=>{const{supabaseInsert:i}=await Promise.resolve().then(()=>E);return{supabaseInsert:i}},void 0);return await t("call_logs",{id:e.id,call_direction:e.callDirection,from_number:e.fromNumber||null,to_number:e.toNumber||null,matched_customer_code:e.matchedCustomerCode||null,matched_prospect_id:e.matchedProspectId||null,duration_seconds:e.durationSeconds??0,call_status:e.callStatus??"answered",started_at:e.startedAt||null,ended_at:e.endedAt||null,notes:e.notes||null,ivry_call_id:e.ivryCallId||null})?e:null}async function rn(e){const t=e.config.api_key,n=e.config.team_id;if(!t||!n)return{count:0,error:"IVRy API key または team_id 未設定"};try{const i=`https://api.ivry.jp/v1/teams/${n}/calls?limit=100`,c=await fetch(i,{headers:{Authorization:`Bearer ${t}`,"Content-Type":"application/json"}});if(!c.ok)return{count:0,error:`HTTP ${c.status}`};const l=(await c.json()).calls??[];let o=0;for(const r of l)await Pa({id:`ivry_${r.id}`,callDirection:String(r.direction??"inbound"),fromNumber:String(r.from??""),toNumber:String(r.to??""),durationSeconds:Number(r.duration??0),callStatus:String(r.status??"answered"),recordingUrl:String(r.recording_url??""),startedAt:String(r.started_at??""),endedAt:String(r.ended_at??""),ivryCallId:String(r.id??"")}),o++;return await ze({...e,lastSyncAt:new Date().toISOString(),lastStatus:`${o}件取得`}),{count:o}}catch(i){return{count:0,error:i instanceof Error?i.message:String(i)}}}async function cn(e,t){const n=e.config.api_key,i=e.config.team_id;if(!n||!i)return{synced:0,error:"IVRy API key または team_id 未設定"};try{let c=0;for(const a of t){if(!a.phone)continue;(await fetch(`https://api.ivry.jp/v1/teams/${i}/contacts`,{method:"POST",headers:{Authorization:`Bearer ${n}`,"Content-Type":"application/json"},body:JSON.stringify({name:a.name,phone_number:a.phone,external_id:a.customerCode??"",note:a.note??""})})).ok&&c++}return{synced:c}}catch(c){return{synced:0,error:c instanceof Error?c.message:String(c)}}}async function dn(){return(await _("lead_lists",{order:"created_at.desc"})).map(t=>({id:m(t,["id"],""),name:m(t,["name"],""),query:m(t,["query"],""),area:m(t,["area"],""),businessType:m(t,["business_type"],""),totalCount:C(t.total_count),source:m(t,["source"],"manual"),createdAt:m(t,["created_at"],"")}))}async function un(e){return(await _("lead_items",{list_id:`eq.${e}`,order:"rating.desc.nullslast"})).map(n=>({id:m(n,["id"],""),listId:m(n,["list_id"],""),companyName:m(n,["company_name"],""),address:m(n,["address"],""),phone:m(n,["phone"],""),website:m(n,["website"],""),email:m(n,["email"],""),businessType:m(n,["business_type"],""),rating:n.rating?Number(n.rating):void 0,reviewCount:C(n.review_count),lat:n.lat?Number(n.lat):void 0,lng:n.lng?Number(n.lng):void 0,placeId:m(n,["place_id"],""),status:m(n,["status"],"new"),convertedProspectId:m(n,["converted_prospect_id"],""),note:m(n,["note"],"")}))}async function pn(e){const{supabaseInsert:t}=await f(async()=>{const{supabaseInsert:i}=await Promise.resolve().then(()=>E);return{supabaseInsert:i}},void 0);return await t("lead_lists",{id:e.id,name:e.name,query:e.query||null,area:e.area||null,business_type:e.businessType||null,total_count:e.totalCount,source:e.source})?e:null}async function Ta(e){const{supabaseInsert:t}=await f(async()=>{const{supabaseInsert:i}=await Promise.resolve().then(()=>E);return{supabaseInsert:i}},void 0);return await t("lead_items",{id:e.id,list_id:e.listId,company_name:e.companyName,address:e.address||null,phone:e.phone||null,website:e.website||null,email:e.email||null,business_type:e.businessType||null,rating:e.rating??null,review_count:e.reviewCount??null,lat:e.lat??null,lng:e.lng??null,place_id:e.placeId||null,status:e.status,converted_prospect_id:e.convertedProspectId||null,note:e.note||null})?e:null}async function mn(e,t,n){const i=e.config.api_key;if(!i)return{results:[],error:"Google Maps API key 未設定"};const c=`${t} ${n}`.trim(),a=`https://maps.googleapis.com/maps/api/place/textsearch/json?query=${encodeURIComponent(c)}&language=ja&key=${i}`;try{const l=await fetch(a);if(!l.ok)return{results:[],error:`HTTP ${l.status}`};const o=await l.json();return o.status!=="OK"&&o.status!=="ZERO_RESULTS"?{results:[],error:`API status: ${o.status}`}:{results:o.results.map(d=>{const u=d.geometry?.location;return{id:`place_${d.place_id}`,listId:"",companyName:String(d.name??""),address:String(d.formatted_address??""),rating:d.rating?Number(d.rating):void 0,reviewCount:d.user_ratings_total?Number(d.user_ratings_total):void 0,lat:u?.lat,lng:u?.lng,placeId:String(d.place_id??""),status:"new"}})}}catch(l){return{results:[],error:l instanceof Error?l.message:String(l)}}}async function yn(e){const t={id:`p_${Date.now()}_${Math.random().toString(36).slice(2,8)}`,companyName:e.companyName,phone:e.phone,address:e.address,lat:e.lat,lng:e.lng,businessType:e.businessType,stage:"cold",source:"リスト",expectedAmount:1e5,probability:10,note:e.note??(e.rating?`Google評価: ⭐${e.rating} (${e.reviewCount}件)`:"")},n=await Ea(t);return n&&await Ta({...e,status:"imported",convertedProspectId:t.id}),n}async function hn(){return(await _("workflow_orders",{order:"order_date.desc"})).map(t=>({id:m(t,["id"],""),orderNo:m(t,["order_no"],""),customerName:m(t,["customer_name"],""),customerCode:m(t,["customer_code"],""),orderDate:m(t,["order_date"],""),deliveryDate:m(t,["delivery_date"],""),stage:m(t,["stage"],"new"),totalAmount:C(t.total_amount),itemCount:C(t.item_count),priority:m(t,["priority"],"normal"),staffName:m(t,["staff_name"],""),notes:m(t,["notes"],"")}))}async function vn(e){const{supabaseInsert:t}=await f(async()=>{const{supabaseInsert:i}=await Promise.resolve().then(()=>E);return{supabaseInsert:i}},void 0);return await t("workflow_orders",{id:e.id,order_no:e.orderNo,customer_name:e.customerName,customer_code:e.customerCode||null,order_date:e.orderDate,delivery_date:e.deliveryDate||null,stage:e.stage,total_amount:e.totalAmount,item_count:e.itemCount,priority:e.priority,staff_name:e.staffName||null,notes:e.notes||null,updated_at:new Date().toISOString()})?e:null}async function fn(){return(await _("tour_inquiries",{order:"created_at.desc"})).map(t=>({id:m(t,["id"],""),name:m(t,["name"],""),email:m(t,["email"],""),phone:m(t,["phone"],""),visitDate:m(t,["visit_date"],""),partySize:C(t.party_size)||1,language:m(t,["language"],"ja"),purpose:m(t,["purpose"],""),message:m(t,["message"],""),status:m(t,["status"],"new"),repliedAt:m(t,["replied_at"],""),confirmedTime:m(t,["confirmed_time"],""),createdAt:m(t,["created_at"],new Date().toISOString())}))}async function bn(e){const{supabaseInsert:t}=await f(async()=>{const{supabaseInsert:i}=await Promise.resolve().then(()=>E);return{supabaseInsert:i}},void 0);return await t("tour_inquiries",{id:e.id,name:e.name,email:e.email,phone:e.phone||null,visit_date:e.visitDate||null,party_size:e.partySize,language:e.language,purpose:e.purpose||null,message:e.message||null,status:e.status,replied_at:e.repliedAt||null,confirmed_time:e.confirmedTime||null})?e:null}const gn=[{table:"sake_sales_document_lines",display:"売上伝票明細"},{table:"sake_purchase_document_lines",display:"仕入伝票明細"},{table:"sake_sales_document_headers",display:"売上伝票ヘッダ"},{table:"sake_purchase_document_headers",display:"仕入伝票ヘッダ"},{table:"sake_inventory_movements_sk",display:"在庫移動(SK)"},{table:"sake_current_stock_sh",display:"在庫(SH)"},{table:"sake_inventory_movements_k5",display:"在庫移動(K5)"},{table:"sake_current_stock_h5",display:"在庫(H5)"},{table:"sake_special_prices_sh",display:"特価(SH)"},{table:"sake_products_sh",display:"商品(SH)"},{table:"sake_special_prices_h5",display:"特価(H5)"},{table:"sake_products_sk",display:"商品(SK)"},{table:"sake_products_k5",display:"商品(K5)"},{table:"sake_products_h5",display:"商品(H5)"},{table:"sake_customers",display:"得意先"},{table:"sake_suppliers",display:"仕入先"},{table:"sake_delivery_destinations",display:"納品先"},{table:"sake_trading_partners",display:"取引先"},{table:"sake_current_stock_sk",display:"在庫(SK)"}];async function Na(){return(await Promise.all(gn.map(async t=>{const[n,i]=await Promise.all([rt(t.table),_(t.table,{select:"_synced_at",order:"_synced_at.desc",limit:"1"})]);return{tableName:t.table,displayName:t.display,rowCount:n,lastSyncAt:i[0]?._synced_at??null}}))).sort((t,n)=>n.rowCount-t.rowCount)}async function Ie(e,t,n=100){const i=(t-1)*n,[c,a]=await Promise.all([_(e,{select:"_source_file,_record_index,_record_size,_raw_b64,_source_path,_source_file_mtime,_synced_at",order:"_record_index.asc",limit:String(n),offset:String(i)}),rt(e)]);return{records:c,total:a}}async function nt(e){const t=await _("customers",{select:"memo",or:`legacy_customer_code.eq.${e},customer_code.eq.${e}`,limit:"1"});if(t.length===0)return"";const n=t[0].memo;if(typeof n=="string"&&n)try{const i=JSON.parse(n);return String(i.price_group??"")}catch{return""}return""}async function ja(e,t){if(e){const i=await _("customer_product_prices",{select:"special_price",price_group:`eq.${e}`,legacy_product_code:`eq.${t}`,limit:"1"});if(i.length>0&&i[0].special_price)return C(i[0].special_price)}const n=await _("products",{select:"default_sale_price",or:`legacy_product_code.eq.${t},product_code.eq.${t}`,limit:"1"});return n.length>0&&n[0].default_sale_price?C(n[0].default_sale_price):0}const g=Object.freeze(Object.defineProperty({__proto__:null,CALENDAR_CATEGORY_COLORS:gt,CALENDAR_CATEGORY_LABELS:bt,DEPT_LABELS:je,INVOICE_TYPE_LABELS:tt,JIKOMI_STATUS_LABELS:va,MATERIAL_CATEGORIES:Ia,PROSPECT_STAGE_COLORS:Je,PROSPECT_STAGE_LABELS:fe,ROLE_LABELS:Ne,SEASONAL_TEMPLATES:dt,SLACK_EVENT_LABELS:Re,TAX_DEDUCTION_LABELS:st,TAX_RATE_CATEGORIES:Sa,convertLeadToProspect:yn,deleteCalendarEvent:Rs,deleteMailSender:Ts,deleteMaterial:sn,deletePrintLayout:Es,deleteProspect:Zs,deleteUserProfile:Ys,fetchAuditLogs:Xs,fetchBillList:ka,fetchBillingSummary:ht,fetchCalendarEvents:Ns,fetchCallLogs:on,fetchCustomerAnalysis:ya,fetchCustomerLedger:mt,fetchCustomerPriceGroup:nt,fetchDeliveryLocations:nn,fetchDeliveryNote:yt,fetchFaxInbox:Fs,fetchIntegrationSettings:La,fetchInvoices:Te,fetchJikomiList:fa,fetchKenteiList:ga,fetchLeadItems:un,fetchLeadLists:dn,fetchMailSenders:Is,fetchMasterStats:ra,fetchMaterialList:at,fetchMyProfile:Bs,fetchPayableList:wa,fetchPaymentStatus:oa,fetchPipelineMeta:ca,fetchPrintLayouts:Ls,fetchProductABC:ha,fetchProductPrice:ja,fetchProspectActivities:en,fetchProspects:Ws,fetchPurchaseList:$a,fetchRawMaterialStock:_a,fetchRawRecords:Ie,fetchRawTableList:Na,fetchSalesAnalytics:Fe,fetchSalesReport:Ve,fetchSalesSummary:la,fetchShopifyOrders:qs,fetchSlackLogs:Ks,fetchSlackRules:Da,fetchStoreOrders:Aa,fetchStoreSales:ft,fetchSyncDashboard:da,fetchTankList:ba,fetchTaxDeclaration:vt,fetchTourInquiriesFromDb:fn,fetchUserProfiles:Js,fetchWorkflowOrdersFromDb:hn,generateTaxCSV:_s,generateTaxXML:xa,ocrFaxImage:Vs,recalculateTaxDeclaration:Ss,recordAudit:Hs,saveCalendarEvent:js,saveCallLog:Pa,saveDeliveryLocation:ln,saveEmailCampaign:Ee,saveFaxRecord:zs,saveIntegrationSetting:ze,saveInvoice:ua,saveLeadItem:Ta,saveLeadList:pn,saveMailSender:Ps,saveMaterial:an,savePrintLayout:Ds,saveProspect:Ea,saveProspectActivity:tn,saveSlackRule:Gs,saveTaxDeclaration:xs,saveTourInquiry:bn,saveUserProfile:Us,saveWorkflowOrder:vn,searchPlaces:mn,sendEmailCampaign:Ca,sendSlackNotification:Qs,submitFeatureRequest:pa,syncGoogleCalendar:Ms,syncIvryCallLogs:rn,syncPhoneBookToIvry:cn,syncShopifyOrders:Os},Symbol.toStringTag,{value:"Module"}));function te(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}const $n={open:"未締め",closed:"締め済"};function wn(e,t){const n=e.customers.map(i=>`
      <tr>
        <td>
        ...
        <td>
          <span class="status-pill ${i.status==="closed"?"success":"warning"}">${$n[i.status]}</span>
        </td>
        <td>
  ...
  `}function Ra(e){return new Intl.DateTimeFormat("ja-JP",{year:"numeric",month:"2-digit",day:"2-digit"}).format(new Date(e))}function me(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function Sn(e){return e.salesHistory.length===0?'<tr><td colspan="3" class="empty-row">データなし</td></tr>':e.salesHistory.map(t=>`
        <tr>
          <td>${Ra(t.date)}</td>
          <td class="mono">${t.documentNo}</td>
          <td class="numeric">${me(t.amount)}</td>
        </tr>
      `).join("")}function xn(e){return e.paymentHistory.length===0?'<tr><td colspan="3" class="empty-row">データなし</td></tr>':e.paymentHistory.map(t=>`
        <tr>
          <td>${Ra(t.date)}</td>
          <td>${t.method}</td>
          <td class="numeric">${me(t.amount)}</td>
        </tr>
      `).join("")}function An(e,t){return`
    <section class="page-head">
      <div>
  ...
  `}function En(e,t,n,i,c="month"){const a={success:"正常",warning:"注意",error:"異常",running:"実行中"},l=Cn(e.allDailySales,c),o=l.reduce((p,h)=>p+h.amount,0),r=l.length,d=e.salesRecords.slice(0,10).map(p=>`
            <tr>
          ...
          `).join(""),u=["today","month","90days","year","all"].map(p=>`<button class="button ${p===c?"primary":"secondary"} small" type="button" data-period="${p}">${Ye[p]}</button>`).join("");return`
    <section class="page-head">
      <div>
    ...
    <section class="content-grid">
      <article class="panel">
        <div class="panel-header">
          <div>
            <h2>日次売上</h2>
            <p class="panel-caption">${Ye[c]} (${l.length}日分)</p>
          </div>
        </div>
        ${Dn(l.length>0?l:e.dailySales)}
      </article>
  ...
  `}function In(e){const t=e.prospects.reduce((l,o)=>l+o.expectedAmount*o.probability/100,0),n=e.prospects.filter(l=>l.stage==="hot"||l.stage==="negotiating").length,i=new Date().toISOString().slice(0,10),c=e.upcomingEvents.filter(l=>l.startsAt.slice(0,10)>=i).slice(0,5),a=e.tourInquiries.filter(l=>l.status==="new").length;return`
    <section class="kpi-grid compact">
      <article class="panel kpi-card">
      ...
      <article class="panel">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(100px, 1fr));gap:8px;">
          ${["cold","warm","hot","contacted","negotiating","won"].map(l=>{const o=e.prospects.filter(d=>d.stage===l).length,r=e.prospects.filter(d=>d.stage===l).reduce((d,u)=>d+u.expectedAmount,0);return`
              <div style="background:${Je[l]};color:white;padding:12px;border-radius:6px;text-align:center;">
  ...
  `}function Pn(e){return e?new Intl.DateTimeFormat("ja-JP",{year:"numeric",month:"long",day:"numeric"}).format(new Date(e)):""}function ae(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function Tn(e,t){const n=e.lines.length?e.lines.map((c,a)=>`
          <tr>
  ...
  `}function F(e){return e.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}function Nn(e){return F(e).replaceAll(`
`,"<br />")}function jn(e){const n=[...Object.values(dt),{id:"custom",season:"カスタム",subject:"",body:""}].map(c=>`
        <button
  ...
  `}function Rn(e,t){const n=[we("得意先",t.customers.map(c=>`
          <button
  ...
  `}function re(e){return e.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}function Oa(e){const t=e.resultsHtml.trim()?e.resultsHtml:`<p class="empty-note">${re(e.emptyMessage??"該当データがありません。")}</p>`;return`
  ...
  `}function ke(e){return e.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}function Pt(e){return e.trim().toLowerCase()}function On(e,t){const n=Pt(t),i=e.filter(a=>n?[a.code,a.name,a.name].map(Pt).some(l=>l.includes(n)):!0).slice(0,50),c=i.length?`
        <div class="table-wrap">
          <table>
            <thead>
              <tr>
      ...
      `:"";return Oa({title:"得意先検索",searchQuery:t,placeholder:"コード・名前で検索",resultsHtml:c,emptyMessage:"該当する得意先が見つかりません。"})}function qn(e){return e.toISOString().slice(0,10)}function Z(e){return e.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}function X(e,t){return e[t]?`<div class="field-error">${Z(e[t])}</div>`:""}function se(e,t,n=""){return[n,e[t]?"has-error":""].filter(Boolean).join(" ")}function Mn(e,t,n,i){const c=Object.keys(tt).map(r=>`<option value="${r}" ${e.invoiceType===r?"selected":""}>${tt[r]}</option>`).join(""),a=e.lines.map((r,d)=>`
      <tr>
        <td>
    ...
    <section class="panel">
      ...
      <div class="filter-grid filter-grid--wide">
        ...
        <label class="field">
          <span>伝票日付</span>
          <input class="${se(i,"invoiceDate")}" id="inv-date" type="date" value="${e.invoiceDate||qn(new Date)}" />
          ${X(i,"invoiceDate")}
        </label>
        <label class="field">
          <span>得意先コード</span>
  ...
  `}function Fn(e){return new Intl.DateTimeFormat("ja-JP",{year:"numeric",month:"2-digit",day:"2-digit"}).format(new Date(e))}function Vn(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function zn(e,t){const n=e.length?e.map(i=>`
            <tr>
              ...
              <td class="numeric">${Vn(i.amount)}</td>
            </tr>
  ...
  `}function Jn(e){return new Date(e.getFullYear(),e.getMonth(),1)}function Bn(e,t){return new Date(e.getFullYear(),e.getMonth()+t,1)}function qa(e,t){const n=new Date(e);return n.setDate(n.getDate()+t),n}function Ma(e){const t=new Date(e),n=t.getDay();return t.setDate(t.getDate()-n),t.setHours(0,0,0,0),t}function Tt(e){const t=qa(Ma(e),6);return t.setHours(23,59,59,999),t}function Nt(e){return new Date(`${e}T00:00:00`)}function jt(e){return`${e.getMonth()+1}/${e.getDate()}`}function Un(e){return e.replaceAll("&","&amp;").replaceAll('"',"&quot;").replaceAll("<","&lt;")}function Yn(){const e=new Date,t=Ma(Bn(Jn(e),-3)),n=Tt(new Date(e.getFullYear(),e.getMonth()+4,0)),i=[];let c=new Date(t);for(;c<=n;){const a=Tt(c);i.push({start:new Date(c),end:a,label:`${jt(c)} - ${jt(a)}`}),c=qa(c,7)}return i}function Hn(e){const t=Yn(),n=`160px repeat(${t.length}, minmax(56px, 1fr))`,i=t.map(a=>`
        <div class="gantt-week">
          <span>${a.label}</span>
        </div>
      `).join(""),c=e.length?e.map(a=>{const l=Nt(a.startDate),o=Nt(a.expectedDoneDate),r=Math.max(0,t.findIndex(p=>p.end>=l)),d=Math.max(r,t.reduce((p,h,v)=>h.start<=o?v:p,r)),u=[`仕込番号: ${a.jikomiNo}`,`銘柄: ${a.productName}`,`期間: ${a.startDate} - ${a.expectedDoneDate}`,`タンク: ${a.tankNo}`,`備考: ${a.note||"なし"}`].join(`
`);return`
  ...
  `}function Rt(e,t){const n={planned:"neutral",active:"warning",done:"success"},i=e.map(o=>`
      <tr>
        ...
        <td>
          <span class="status-pill ${n[o.status]}">${va[o.status]}</span>
        </td>
        <td>${o.note||"―"}</td>
      </tr>
  ...
  `}function Xn(e){const t={pending:"未実施",submitted:"申請中",approved:"承認済"},n={pending:"neutral",submitted:"warning",approved:"success"},i=e.map(r=>`
      <tr>
  ...
  `}function Gn(e){return e.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}function Kn(e,t){return`
  ...
  `}const $t={query:"",businessType:"",areaCode:"",activeOnly:"",page:1},ye=50;function Qn(e,t){let n=e;if(t.query){const o=t.query.toLowerCase();n=n.filter(r=>r.code.toLowerCase().includes(o)||r.name.toLowerCase().includes(o)||r.kanaName&&r.kanaName.toLowerCase().includes(o)||r.address1&&r.address1.toLowerCase().includes(o)||r.phone&&r.phone.toLowerCase().includes(o))}t.businessType&&(n=n.filter(o=>o.businessType===t.businessType)),t.areaCode&&(n=n.filter(o=>o.areaCode===t.areaCode)),t.activeOnly==="active"?n=n.filter(o=>o.isActive):t.activeOnly==="inactive"&&(n=n.filter(o=>!o.isActive));const i=Math.max(1,Math.ceil(n.length/ye)),a=(Math.min(t.page,i)-1)*ye,l=n.slice(a,a+ye);return{filtered:n,paged:l,totalPages:i}}function Ot(e,t,n){if(n<=1)return`<div class="master-pagination"><span>${e}件</span></div>`;const i=(t-1)*ye+1,c=Math.min(t*ye,e),a=[];for(let l=1;l<=n;l++)l===1||l===n||l>=t-2&&l<=t+2?a.push(`<button class="button ${l===t?"primary":"secondary"}" type="button" data-action="master-page" data-page="${l}" style="min-width:36px;padding:4px 8px;">${l}</button>`):(l===t-3||l===t+3)&&a.push('<span style="padding:0 4px;color:var(--text-secondary);">…</span>');return`
  ...
  `}function Wn(e,t){const n=[...new Set(e.map(c=>c.businessType).filter(Boolean))].sort(),i=[...new Set(e.map(c=>c.areaCode).filter(Boolean))].sort();return`
  ...
  `}function Fa(e,t){return!e||e.length<=t?e||"":e.slice(0,t)+"…"}function Zn(e){return e.map(t=>`
        <tr>
          ...
          <td title="${t.address1||""}">${Fa(t.address1||"",20)}</td>
          <td>${t.areaCode||""}</td>
          <td>${t.staffCode||""}</td>
      ...
      `).join("")}function ei(e){return e.map(t=>`
        <tr>
          <td class="mono">${t.code}</td>
          <td>${Fa(t.name,30)}</td>
          <td>${t.category}</td>
      ...
      `).join("")}function ti(e,t,n=$t){const{filtered:i,paged:c,totalPages:a}=Qn(e.customers,n);return`
    <section class="page-head">
      <div>
      ...
      `:`
        <div class="table-wrap">
          <table>
            ...
            <tbody>${ei(e.products)}</tbody>
          </table>
        </div>
      `}
    </section>
  `}function He(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function ai(e,t){if(!e&&!t)return"";const n=e;return`
          ...
          <div class="filter-grid filter-grid--wide">
            ...
            <label class="field" style="flex:1 1 160px;">
              ...
              <select id="mat-type">
                ...
                ${Ia.map(i=>`<option ${n?.materialType===i?"selected":""}>${i}</option>`).join("")}
              </select>
            </label>
  ...
  `}function si(e){const t=e.map(c=>{const l=(c.minimumStock>0?c.currentStock/c.minimumStock:1/0)<1.5;return`
        <tr>
  ...
  `}function ni(e){return e?new Intl.DateTimeFormat("ja-JP",{year:"numeric",month:"2-digit",day:"2-digit"}).format(new Date(e)):"-"}function Xe(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}const ii={unpaid:"未入金",partial:"一部入金",paid:"入金済"};function li(e){return`
    <section class="page-head">
      <div>
    ...
    <section class="panel">
      ...
      <div class="table-wrap">
        ...
        <tr>
          ...
          <td>${ni(n.lastPaymentDate)}</td>
          <td><span class="status-pill ${n.status==="paid"?"success":n.status==="partial"?"warning":"danger"}">${ii[n.status]}</span></td>
        </tr>
      `).join("")}</tbody>
        </table>
      </div>
    </section>
  `}function ne(e){return e.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}function qt(e){return e.trim().toLowerCase()}function oi(e,t){const n=qt(t),i=e.filter(a=>n?[a.code,a.name,a.janCode].map(qt).some(l=>l.includes(n)):!0),c=i.length?`
        <div class="table-wrap">
          <table>
            <thead>
              <tr>
      ...
      `:"";return Oa({title:"商品検索",searchQuery:t,placeholder:"コード・名前・JANで検索",resultsHtml:c,emptyMessage:"該当する商品が見つかりません。"})}function G(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function ri(e,t){const n={pending:"未確定",confirmed:"確定",paid:"支払済"},i={pending:"warning",confirmed:"neutral",paid:"success"},c={unpaid:"未払い",partial:"一部支払",paid:"支払済"},a={unpaid:"warning",partial:"neutral",paid:"success"},l=e.map(p=>`
      <tr>
        <td class="mono">${p.documentNo}</td>
  ...
  `}function ce(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function ci(e,t){const n={holding:"保有中",due:"期日到来",cleared:"決済済"},i={holding:"neutral",due:"warning",cleared:"success"},c=e.map(u=>`
      <tr>
  ...
  `}function xe(e){if(!e)return"error";const n=(Date.now()-new Date(e).getTime())/(1e3*60*60);return n<1?"success":n<24?"warning":"error"}function Mt(e){if(!e)return"未同期";const n=(Date.now()-new Date(e).getTime())/(1e3*60*60);return n<1?"正常":n<24?"注意":"要確認"}function ui(e){return`
    <section class="kpi-grid compact">
      <article class="panel kpi-card">
  ...
  `}function pi(e,t,n,i){const c={success:"正常",warning:"注意",error:"異常",running:"実行中"};return`
    <section class="page-head">
      <div>
...
cd sake-system\\relay`,success:["エクスプローラーで sake-system\\\\relay フォルダが見える状態になっていれば成功です。","cd を実行したあと、コマンドプロンプトの行頭が relay フォルダを指していれば問題ありません。"],errors:["git コマンドが使えない場合は Git for Windows が未導入です。先に Git をインストールしてください。","アクセス拒否が出る場合は C:\\\\Program Files 配下ではなく C:\\\\sake-relay\\\\ などに配置してください。"]})}
  ...
  `}function vi(e){e.ranking.reduce((r,d)=>r+d.amount,0);const t=e.ranking.filter(r=>r.abcRank==="A").length,n=e.ranking.filter(r=>r.abcRank==="B").length,i=e.ranking.filter(r=>r.abcRank==="C").length,c=e.ranking.filter(r=>r.abcRank==="A").reduce((r,d)=>r+d.amount,0),a=e.ranking.filter(r=>r.abcRank==="B").reduce((r,d)=>r+d.amount,0),l=e.ranking.filter(r=>r.abcRank==="C").reduce((r,d)=>r+d.amount,0),o=e.ranking.map(r=>`
        <tr>
          ...
          <td><span class="status-pill ${mi(r.abcRank)}">${r.abcRank}</span></td>
        </tr>
      `).join("");return`
    <section class="page-head">
      <div>
  ...
  `}function Ft(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function fi(e){switch(e){case"A":return"success";case"B":return"warning";case"C":return"neutral"}}function bi(e){if(e.length===0)return'<div class="chart-empty">データなし</div>';const t=e.slice(0,20),n=760,i=320,c={top:24,right:56,bottom:60,left:72},a=n-c.left-c.right,l=i-c.top-c.bottom,o=Math.max(...t.map($=>$.amount),1),r=a/t.length,d=[0,.25,.5,.75,1].map($=>{const k=c.top+l-l*$,w=`${Math.round(o*$/1e4)}万`;return`
        <g>
      ...
      `}).join(""),h=t.map(($,k)=>{const w=c.left+k*r+r/2,x=c.top+l-l*$.cumRatio/100;return`${w},${x}`}).join(" "),v=t.map(($,k)=>{const w=c.left+k*r+r/2,x=c.top+l-l*$.cumRatio/100;return`<circle cx="${w}" cy="${x}" r="3.5" fill="#C53D3D" />`}).join("");return`
  ...
  `}function gi(e){const t=e.ranking.filter(o=>o.abcRank==="A").length,n=e.ranking.filter(o=>o.abcRank==="B").length,i=e.ranking.filter(o=>o.abcRank==="C").length,c=e.ranking.filter(o=>o.abcRank==="A").reduce((o,r)=>o+r.amount,0),a=e.ranking.map(o=>`
        <tr>
          ...
          <td><span class="status-pill ${fi(o.abcRank)}">${o.abcRank}</span></td>
        </tr>
      `).join(""),l=e.monthlyByProduct.length>0?(()=>{const o=e.months.map(d=>`<th class="numeric">${d}</th>`).join(""),r=e.monthlyByProduct.map(d=>{const u=d.values.reduce((h,v)=>h+v,0),p=d.values.map(h=>`<td class="numeric">${h>0?(h/1e4).toFixed(0)+"万":"—"}</td>`).join("");return`<tr><td>${d.label}</td>${p}<td class="numeric"><strong>${Ft(u)}</strong></td></tr>`}).join("");return`
          <div class="table-wrap">
            <table>
  ...
  `}function $i(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function wi(e){return e.replace("-","/")}function ki(e){if(e.length===0)return'<div class="chart-empty">データなし</div>';const t=760,n=280,i={top:16,right:24,bottom:36,left:64},c=t-i.left-i.right,a=n-i.top-i.bottom,l=Math.max(...e.map(u=>u.amount),1),o=c/e.length,r=[0,.25,.5,.75,1].map(u=>{const p=i.top+a-a*u,h=`${Math.round(l*u/1e4).toLocaleString("ja-JP")}万円`;return`
        <g>
          <line x1="${i.left}" y1="${p}" x2="${t-i.right}" y2="${p}" class="chart-grid" />
          <text x="8" y="${p+4}" class="chart-axis">${h}</text>
        </g>
      `}).join(""),d=e.map((u,p)=>{const h=u.amount/l*a,v=Math.max(o-18,24),$=i.left+p*o+(o-v)/2,k=i.top+a-h;return`
        <g>
          <rect x="${$}" y="${k}" width="${v}" height="${h}" rx="6" class="analytics-bar" />
          <text x="${$+v/2}" y="${n-10}" class="chart-axis centered-axis">${wi(u.month)}</text>
        </g>
      `}).join("");return`
  ...
  `}function _i(e){return e.length===0?'<tr><td colspan="5" class="empty-row">データなし</td></tr>':e.map(t=>`
        <tr>
          <td class="mono">${t.code}</td>
          <td>${t.name}</td>
          <td class="numeric">${$i(t.amount)}</td>
          <td class="numeric">${t.quantity.toLocaleString("ja-JP")}</td>
          <td class="numeric">${t.documents.toLocaleString("ja-JP")}</td>
        </tr>
      `).join("")}function Si(e,t){const n=t==="products"?"商品別集計":"得意先別集計",i=t==="products"?e.productTotals:e.customerTotals;return`
    <section class="page-head">
      <div>
  ...
  `}function de(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function xi(e){const t=Math.max(...e.salesByProduct.flatMap(a=>a.values),1),n=e.salesByProduct.map(a=>{const l=a.values.map((o,r)=>`
          <div class="bar-col">
  ...
  `}function Ai(e){return new Intl.DateTimeFormat("ja-JP",{year:"numeric",month:"2-digit",day:"2-digit"}).format(new Date(e))}function Ci(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function Vt(e){return e.toISOString().slice(0,10)}function Li(e,t,n){const i=e.length?e.map(c=>`
            <tr>
  ...
  `}function Ae(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function Di(e,t,n,i){const c={cash:"現金",card:"カード",paypay:"PayPay",other:"その他"},a={new:"新規",processing:"処理中",shipped:"発送済",delivered:"配達済"},l={new:"warning",processing:"neutral",shipped:"success",delivered:"success"},o=e.map(p=>`
      <tr>
        <td>${p.saleTime}</td>
  ...
  `}const Ge={customers:"得意先マスタ",products:"商品マスタ",suppliers:"仕入先マスタ",staff:"担当者マスタ"},Ei={customers:{required:["legacy_customer_code","name"],optional:["kana_name","phone","postal_code","address1","address2","closing_day","payment_day","email"]},products:{required:["legacy_product_code","name"],optional:["kana_name","jan_code","category_code","volume_ml","alcohol_degree","list_price","default_sale_price"]},suppliers:{required:["legacy_supplier_code","name"],optional:["kana_name","phone","postal_code","address1","closing_day","payment_day","email"]},staff:{required:["legacy_staff_code","name"],optional:["kana_name","department"]}};function Ii(e,t,n,i){const c=Ei[e],a=Object.keys(Ge).map(o=>`
  ...
  `}function Ke(e){const t={};return e.querySelectorAll("[data-fd-id]").forEach(n=>{const i=n.dataset.fdId??"",c=parseFloat(n.style.left)||0,a=parseFloat(n.style.top)||0;t[i]={x:c,y:a}}),t}function Ni(e,t,n,i,c){const a=Array.from(new Set([...e.map(d=>d.businessType??""),...t.map(d=>d.businessType??"")].filter(Boolean))),l=e.filter(d=>d.lat&&d.lng).length,o=t.filter(d=>d.lat&&d.lng).length,r=n.filter(d=>d.lat&&d.lng).length;return`
    <section class="page-head">
      <div>
  ...
  `}const ji={new:{label:"新規受注",color:"#2196F3",icon:"📥"},picking:{label:"ピッキング中",color:"#FF9800",icon:"🔍"},packed:{label:"梱包完了",color:"#4CAF50",icon:"📦"},shipped:{label:"発送済",color:"#9C27B0",icon:"🚚"},delivered:{label:"配達完了",color:"#4CAF50",icon:"✅"}},Ri=["new","picking","packed","shipped","delivered"];function Oi(e){const t={new:[],picking:[],packed:[],shipped:[],delivered:[]};e.forEach(a=>t[a.stage].push(a));const n=Ri.map(a=>{const l=ji[a],o=t[a];return`
  ...
  `}function qi(e,t,n){const i=e.cart.reduce((a,l)=>a+l.amount,0);return`
  ...
  `}function Mi(e,t,n){if(e.step==="customer"){const i=e.customerQuery.toLowerCase(),c=i?t.filter(a=>a.name.toLowerCase().includes(i)||a.code.toLowerCase().includes(i)):t.slice(0,20);return`
      <section class="panel">
...
金井酒造店`;function Ji(e,t){const n=t?e.find(c=>c.id===t):null,i=t==="__new__";return`
    <section class="page-head">
      <div>
    ...
    ${n||i?`
      ...
      `:""}
  `}function Bi(e,t,n,i){const[c,a]=t.split("-").map(S=>parseInt(S,10)),l=new Date(c,a-1,1),o=new Date(c,a,0),r=l.getDay(),d=o.getDate(),u=[];for(let S=0;S<r;S++)u.push({isOutside:!0});for(let S=1;S<=d;S++)u.push({date:new Date(c,a-1,S)});for(;u.length%7!==0;)u.push({isOutside:!0});const p=n?e.filter(S=>S.category===n):e,h={};p.forEach(S=>{const R=S.startsAt.slice(0,10);h[R]??=[],h[R].push(S)});const v=new Date().toISOString().slice(0,10),$=u.map(S=>{if(S.isOutside)return'<div class="cal-cell cal-outside"></div>';const R=S.date,U=`${R.getFullYear()}-${String(R.getMonth()+1).padStart(2,"0")}-${String(R.getDate()).padStart(2,"0")}`,Ue=h[U]??[],Ka=U===v,xt=R.getDay();return`
        <div class="cal-cell ${Ka?"cal-today":""} ${xt===0?"cal-sun":xt===6?"cal-sat":""}"
             data-cal-date="${U}">
      ...
      `}).join(""),k=i?.isOpen?Ui(i):"",w=new Date(c,a-2,1),x=new Date(c,a,1),J=`${w.getFullYear()}-${String(w.getMonth()+1).padStart(2,"0")}`,T=`${x.getFullYear()}-${String(x.getMonth()+1).padStart(2,"0")}`,ee=(()=>{const S=new Date;return`${S.getFullYear()}-${String(S.getMonth()+1).padStart(2,"0")}`})();return`
    <section class="page-head">
      <div>
    ...
    ${k}
  `}function Ui(e){const t=e.event;return`
  ...
  `}function Hi(e,t){const n=e.reduce((a,l)=>a+l.totalAmount,0),i=e.filter(a=>a.financialStatus==="paid").length,c=e.filter(a=>a.fulfillmentStatus!=="fulfilled"&&a.fulfillmentStatus!=="shipped").length;return`
    <section class="page-head">
      <div>
  ...
  `}function Gi(e,t,n){const i=t==="__new__"?null:e.find(l=>l.id===t),c=t==="__new__";return n?.role==="admin"?`
    <section class="page-head">
      <div>
  ...
  `}function Wi(e){const t=e.prospects.reduce((a,l)=>a+l.expectedAmount,0),n=e.prospects.reduce((a,l)=>a+l.expectedAmount*l.probability/100,0),i=e.prospects.filter(a=>a.stage==="won").length,c=e.prospects.filter(a=>a.stage==="hot"||a.stage==="negotiating").length;return`
    <section class="page-head">
      <div>
    ...
    ${e.viewMode==="kanban"?Zi(e.prospects):el(e.prospects)}
    ...
    <section class="panel" style="padding:12px;">
      ...
      <div class="pk-board">${["cold","warm","hot","contacted","negotiating","won","lost"].map(i=>{const c=e.filter(l=>l.stage===i),a=c.reduce((l,o)=>l+o.expectedAmount,0);return`
        <div class="pk-col" data-prospect-stage="${i}">
          <div class="pk-col-header" style="--pk-color:${Je[i]};">
          ...
          <tbody>
            ${e.map(t=>`
              <tr>
                ...
                <td><span class="status-pill" style="background:${Je[t.stage]};color:white;">${fe[t.stage]}</span></td>
  ...
  `}function al(e,t,n){const i=e?.config.webhook_url??"",c=e?.config.default_channel??"#general";return`
    <section class="page-head">
      <div>
  ...
  `}const nl=["飲食店","居酒屋","寿司屋","和食","焼肉","フレンチ","イタリアン","バー","酒販店","ワインショップ","百貨店","スーパー","ホテル","旅館","ブライダル","セレクトショップ"];function il(e){const t=e.activeListId?e.lists.find(a=>a.id===e.activeListId):null,n=e.items.filter(a=>a.status==="new").length,i=e.items.filter(a=>a.status==="imported").length,c=e.items.filter(a=>a.status==="excluded").length;return`
    <section class="page-head">
      <div>
        <p class="eyebrow">新規営業</p>
        <h1>リスト取得ツール</h1>
    ...
    <section class="panel">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:8px;align-items:flex-end;">
        <label class="field" style="flex:1 1 180px;">
          ...
          <select id="lb-type">
            <option value="">選択</option>
            ${nl.map(a=>`<option value="${a}" ${e.searchBusinessType===a?"selected":""}>${a}</option>`).join("")}
          </select>
        </label>
        <label class="field" style="flex:1 1 180px;">
          <span>エリア</span>
  ...
  `}function cl(e,t,n){const{subtotal:i,taxAmount:c,total:a}=Va(e.lines,e.taxRate),l=e.previousBalance??0,o=e.paymentAmount??0,r=l-o+a,d=e.lines.map(p=>`
      <tr>
  ...
  `}function dl(e,t,n){const{subtotal:i,taxAmount:c,total:a}=Va(e.lines,e.taxRate),l=e.lines.map(r=>`
      <tr>
  ...
  `}function ul(e,t,n,i){let c="";switch(e){case"chain_store":c=rl(i,n,t);break;case"quotation":c=dl(i,n,t);break;case"invoice_monthly":c=cl(i,n,t);break}const a=Object.keys(Yt).map(r=>`<button class="tab-button ${e===r?"active":""}" data-print-template="${r}">${Yt[r]}</button>`).join(""),l=i.lines.map((r,d)=>`
      <tr>
...
`}async function fl(e,t){const{supabaseInsert:n}=await f(async()=>{const{supabaseInsert:o}=await Promise.resolve().then(()=>E);return{supabaseInsert:o}},void 0);let i=0,c=0;const l={customers:"customers",products:"products",suppliers:"suppliers",staff:"staff"}[e];for(const o of t){if(!o._valid)continue;const{_valid:r,_error:d,...u}=o,p={...u};if(!p.id){const h=e==="customers"?"legacy_customer_code":e==="products"?"legacy_product_code":e==="suppliers"?"legacy_supplier_code":"legacy_staff_code";p.id=String(u[h]??`${e}-${Date.now()}-${i+c}`)}for(const h of["volume_ml","closing_day","payment_day","list_price","default_sale_price"])if(typeof p[h]=="string"&&p[h]!==""){const v=Number(p[h]);Number.isFinite(v)&&(p[h]=v)}try{await n(l,p)!==null?i++:c++}catch{c++}}return{inserted:i,failed:c}}function We(e){return new Intl.DateTimeFormat("ja-JP",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"}).format(new Date(e))}function bl(e,t=40){return!e||e.length<=t?e||"":e.slice(0,t)+"…"}function gl(e,t,n,i,c){const a=n.reduce((d,u)=>d+u.rowCount,0),l=n.map(d=>d.lastSyncAt).filter(d=>d!==null).sort().reverse()[0]??null,o=100,r=Math.max(1,Math.ceil(c/o));return`
    ...
    `}
  `}function $l(e){const t={empty:"空",in_use:"使用中",aging:"熟成中"},n={empty:"neutral",in_use:"warning",aging:"success"},i=e.map(d=>{const u=d.capacity>0?Math.round(d.currentVolume/d.capacity*100):0;return`
        <tr>
  ...
  `}function Ze(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function wl(e,t,n){const i=e.rows.map((d,u)=>`
      <tr>
      ...
      <tr>
        ...
        <td>
          <select class="input-cell" data-ded-row="${u}" data-ded-field="categoryCode">
            ${Sa.map(p=>`<option value="${p.code}" ${p.code===d.categoryCode?"selected":""}>${p.code}:${p.name}</option>`).join("")}
          </select>
        </td>
        <td class="numeric">
  ...
  `;const o={id:c,message:e,type:t,el:l};ot.push(o),i.appendChild(l),requestAnimationFrame(()=>{l.classList.add("toast-enter")});const r=()=>xl(o);l.querySelector(".toast-dismiss").addEventListener("click",r),setTimeout(()=>{l.classList.add("toast-exit"),l.addEventListener("animationend",r,{once:!0})},a)}function xl(e){const t=ot.indexOf(e);t!==-1&&(ot.splice(t,1),e.el.remove())}function Al(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function B(e,t={}){const{title:n="確認",confirmLabel:i="OK",cancelLabel:c="キャンセル",variant:a="primary"}=t;return new Promise(l=>{const o=document.createElement("div");o.className="modal-backdrop confirm-backdrop",o.setAttribute("role","dialog"),o.setAttribute("aria-modal","true"),o.innerHTML=`
    ...
    `;const r=u=>{o.classList.add("confirm-exit"),o.addEventListener("animationend",()=>{o.remove()},{once:!0}),l(u)};o.querySelector(".confirm-cancel").addEventListener("click",()=>r(!1)),o.querySelector(".confirm-ok").addEventListener("click",()=>r(!0)),o.addEventListener("click",u=>{u.target===o&&r(!1)});const d=u=>{u.key==="Escape"&&(document.removeEventListener("keydown",d),r(!1))};document.addEventListener("keydown",d),document.body.appendChild(o),requestAnimationFrame(()=>{o.querySelector(".confirm-ok")?.focus()})})}const Cl=`<svg width="28" height="28" viewBox="0 0 28 28" fill="none">
...
</svg>`,Ll=`<svg width="28" height="28" viewBox="0 0 28 28" fill="none">
  <circle cx="14" cy="14" r="13" stroke="currentColor" stroke-width="1.5"/>
...
`)}`,l=new Blob([a],{type:"text/csv;charset=utf-8;"}),o=URL.createObjectURL(l),r=document.createElement("a");r.href=o,r.download=e,document.body.append(r),r.click(),r.remove(),window.setTimeout(()=>URL.revokeObjectURL(o),0)}const El=["/","/cat/sales","/cat/brewery","/cat/purchase","/cat/more","/sales","/payment","/master","/invoice","/ledger","/analytics","/customer-analysis","/product-abc","/invoice-entry","/delivery","/billing","/report","/jikomi","/tanks","/kentei","/materials","/purchase","/raw-material","/tax","/store","/setup","/email","/import","/print","/form-designer","/map","/workflow","/mobile-order","/tour","/mail-senders","/calendar","/integrations","/shopify","/fax","/users","/profile","/audit","/prospects","/slack","/calls","/list-builder","/raw-browser"],Le=[{name:"青葉商事",email:"aoba@example.jp",area:"関東",historySegment:"seasonal"},{name:"北斗酒販",email:"hokuto@example.jp",area:"北海道",historySegment:"premium"},{name:"中央フーズ",email:"chuo@example.jp",area:"関東",historySegment:"seasonal"},{name:"東海酒店",email:"tokai@example.jp",area:"中部",historySegment:"premium"},{name:"三和物産",email:"sanwa@example.jp",area:"関西",historySegment:"liqueur"},{name:"南星リカー",email:"nansei@example.jp",area:"九州",historySegment:"seasonal"},{name:"山川酒店",email:"yamakawa@example.jp",area:"関西",historySegment:"premium"},{name:"瑞穂商店",email:"mizuho@example.jp",area:"中部",historySegment:"seasonal"}],Xt=[{path:"/sales",title:"売上一覧"},{path:"/payment",title:"入金状況"},{path:"/master",title:"マスタ"},{path:"/invoice",title:"伝票照会"},{path:"/ledger",title:"得意先台帳"},{path:"/analytics",title:"売上分析"},{path:"/invoice-entry",title:"伝票入力"},{path:"/delivery",title:"納品書"},{path:"/billing",title:"月次請求"},{path:"/report",title:"集計帳票"},{path:"/customer-analysis",title:"得意先分析"},{path:"/product-abc",title:"商品ABC分析"},{path:"/jikomi",title:"仕込管理"},{path:"/tanks",title:"タンク管理"},{path:"/kentei",title:"検定管理"},{path:"/materials",title:"資材管理"},{path:"/purchase",title:"仕入・買掛"},{path:"/raw-material",title:"手形・原料"},{path:"/tax",title:"酒税申告"},{path:"/store",title:"店舗・直売所"},{path:"/setup",title:"連動設定"},{path:"/import",title:"CSV/Excelインポート"},{path:"/print",title:"印刷センター"},{path:"/form-designer",title:"帳票デザイナー"},{path:"/map",title:"取引先マップ"},{path:"/workflow",title:"受注ワークフロー"},{path:"/mobile-order",title:"モバイル受注"},{path:"/tour",title:"酒蔵見学"},{path:"/mail-senders",title:"メール送信元管理"},{path:"/calendar",title:"カレンダー"},{path:"/integrations",title:"外部連携設定"},{path:"/shopify",title:"Shopify注文"},{path:"/fax",title:"FAX OCR"},{path:"/users",title:"ユーザー管理"},{path:"/profile",title:"プロフィール"},{path:"/audit",title:"操作ログ"},{path:"/prospects",title:"新規営業"},{path:"/slack",title:"Slack通知"},{path:"/calls",title:"通話履歴(IVRy)"},{path:"/list-builder",title:"リスト取得ツール"},{path:"/raw-browser",title:"データブラウザ"}];function za(e){const t=dt[e];return t?{subject:t.subject,body:t.body}:{subject:"",body:""}}function wt(){return{invoiceType:"sales",invoiceDate:new Date().toISOString().slice(0,10),customerCode:"",customerName:"",staffCode:"",lines:[],note:""}}function Il(){const e=za("spring");return{mode:"all",region:"all",historySegment:"seasonal",templateId:"spring",subject:e.subject,body:e.body,saveMessage:null}}const Be=new Date,Pl=Be.toISOString().slice(0,7),Tl=Be.getFullYear(),Nl=Be.getMonth()+1,jl=Be.toISOString().slice(0,10),Rl="C0011",Q=Il();function Ja(e){const t="/".endsWith("/")?"/".slice(0,-1):"/",n=e.startsWith(t)?e.slice(t.length)||"/":e;return El.includes(n)?n:"/"}function kt(e){switch(e){case"/cat/sales":case"/invoice":case"/ledger":case"/invoice-entry":case"/delivery":case"/billing":case"/report":case"/customer-analysis":case"/product-abc":return"sales";case"/cat/brewery":case"/jikomi":case"/tanks":case"/kentei":case"/materials":return"brewery";case"/cat/purchase":case"/purchase":case"/raw-material":return"purchase";case"/cat/more":case"/master":case"/analytics":case"/tax":case"/store":case"/setup":case"/raw-browser":return"more";case"/email":return"email";default:return"dashboard"}}const Gt=Ja(location.pathname),s={salesSummary:null,paymentStatus:null,masterStats:null,pipelineMeta:null,syncDashboard:null,rawTableList:[],rawRecords:[],rawSelectedTable:null,rawPage:1,rawTotalCount:0,invoiceRecords:[],customerLedger:null,salesAnalytics:null,customerAnalysis:null,productABC:null,invoiceForm:wt(),invoiceSaving:!1,invoiceSavedDocNo:null,invoicePriceGroup:"",pickerMode:null,pickerQuery:"",pickerTargetLine:null,invoiceErrors:{},deliveryNote:null,deliverySearchDocNo:"",billingSummary:null,billingYearMonth:Pl,salesReport:null,jikomiList:[],jikomiView:"list",tankList:[],kenteiList:[],materialList:[],purchaseList:[],payableList:[],billList:[],rawStockList:[],taxDeclaration:null,taxYear:Tl,taxMonth:Nl,storeSales:[],storeOrders:[],storeTab:"pos",importEntity:"customers",importPreview:null,importing:!1,importResult:null,fdDesignMode:!0,fdSavedPositions:null,fdActiveFieldId:null,mapRegionFilter:"",workflowOrders:[],mobileOrder:{step:"customer",selectedCustomer:null,cart:[],customerQuery:"",productQuery:"",memo:"",submittedDocNo:null},tourInquiries:[],tourActiveId:null,mailSenders:[],mailSenderEditingId:null,emailSenderId:"default",calendarEvents:[],calendarYearMonth:new Date().toISOString().slice(0,7),calendarFilterCategory:"",calendarEdit:null,integrations:[],integrationEditingId:null,shopifyOrders:[],faxRecords:[],faxProcessing:!1,faxOcrText:null,userProfiles:[],userEditingId:null,myProfile:null,auditLogs:[],prospects:[],prospectActivities:[],prospectEditingId:null,prospectViewMode:"kanban",slackRules:[],slackLogs:[],materialEditing:null,materialEditingIsNew:!1,deliveryLocations:[],callLogs:[],mapFilters:{showCustomers:!0,showProspects:!0,showDelivery:!0,filterRegion:"",filterBusinessType:""},leadLists:[],leadItems:[],leadActiveListId:null,leadSearchQuery:"",leadSearchArea:"",leadSearchType:"",leadSearching:!1,leadSearchResults:[],printTemplate:"chain_store",printOptions:{...ll,overlayImageUrl:`${"/".replace(/\/$/,"")}/reference/chainstore_ref.png`},printCompany:{...ol},printData:{documentNo:"D"+new Date().toISOString().slice(0,10).replaceAll("-",""),documentDate:new Date().toISOString().slice(0,10),orderDate:new Date().toISOString().slice(0,10),deliveryDate:new Date(Date.now()+2*864e5).toISOString().slice(0,10),customerName:"株式会社〇〇商事",customerHonorific:"御中",customerPostalCode:"100-0001",customerAddress:"東京都千代田区〇〇1-2-3",customerCode:"C0001",chainStoreCode:"0123",categoryCode:"21",slipTypeCode:"11",orderNo:"PO-"+new Date().toISOString().slice(5,10).replaceAll("-",""),vendorCode:"V0001",departmentCode:"101",settlementPrint:!1,title:"",remarks:"",lines:[{productCode:"P00012",productName:"純米吟醸 金井の雫",spec:"720ml",quantity:12,unit:"本",unitPrice:1500,amount:18e3,retailPrice:2200,janCode:"4901234567891",caseQty:6},{productCode:"P00008",productName:"本醸造 辛口",spec:"1.8L",quantity:6,unit:"本",unitPrice:1800,amount:10800,retailPrice:2400,janCode:"4901234567908",caseQty:6},{productCode:"P00021",productName:"梅酒 熟成",spec:"500ml",quantity:12,unit:"本",unitPrice:1200,amount:14400,retailPrice:1800,janCode:"4901234567915",caseQty:12}],taxRate:.1,previousBalance:0,paymentAmount:0},storeSalesDate:jl,route:Gt,currentCategory:kt(Gt),sidebarOpen:!1,salesFilter:{startDate:"",endDate:""},invoiceFilter:{documentNo:"",startDate:"",endDate:"",customerCode:""},ledgerCustomerCode:Rl,salesPeriod:"month",masterTab:"customers",masterFilter:{...$t},analyticsTab:"products",emailAudienceMode:Q.mode,emailRegion:Q.region,emailHistorySegment:Q.historySegment,emailTemplateId:Q.templateId,emailSubject:Q.subject,emailBody:Q.body,emailSaveMessage:Q.saveMessage,emailSending:!1,globalSearchOpen:!1,globalQuery:"",authSkipped:!1,authSubmitting:!1,authError:null,user:null,loading:!0,actionLoading:!1,error:null};function Kt(e){return e.slice(0,10)}function Ol(e){return{...e}}function Oe(){s.pickerMode=null,s.pickerQuery="",s.pickerTargetLine=null}function Ba(){s.invoiceForm=wt(),s.invoiceSavedDocNo=null,s.invoicePriceGroup="",s.invoiceErrors={},Oe()}function Ua(e){const t={};return e.invoiceDate.trim()||(t.invoiceDate="伝票日付は必須です。"),e.customerCode.trim()||(t.customerCode="得意先コードは必須です。"),e.lines.length===0&&(t.lines="明細を1行以上入力してください。"),e.lines.forEach((n,i)=>{n.productCode.trim()||(t[`lines.${i}.productCode`]="商品コードは必須です。"),n.productName.trim()||(t[`lines.${i}.productName`]="商品名は必須です。"),n.quantity<=0&&(t[`lines.${i}.quantity`]="数量は1以上を入力してください。"),n.unitPrice<0&&(t[`lines.${i}.unitPrice`]="単価は0円以上で入力してください。")}),t}function ql(e){const t=s.invoiceForm.lines[e];t&&s.invoiceForm.lines.splice(e+1,0,Ol(t))}function Ml(){const e=s.invoiceRecords[0],t=s.masterStats?.customers[0],n=s.masterStats?.products.slice(0,2)??[];s.invoiceForm={invoiceType:"sales",invoiceDate:new Date().toISOString().slice(0,10),customerCode:e?.customerCode??t?.code??"",customerName:e?.customerName??t?.name??"",staffCode:s.invoiceForm.staffCode||"S001",lines:n.map((i,c)=>{const a=c===0?1:2,l=1200*(c+1);return{productCode:i.code,productName:i.name,quantity:a,unitPrice:l,unit:"本",amount:a*l}}),note:e?`過去伝票 ${e.documentNo} をもとに複製`:"直近のサンプル伝票をもとに複製"},s.invoiceSavedDocNo=null,s.invoiceErrors={}}function Fl(e){const t=s.masterStats?.customers.find(n=>n.code.toLowerCase()===e.trim().toLowerCase());return t?(s.invoiceForm.customerCode=t.code,s.invoiceForm.customerName=t.name,s.invoicePriceGroup=t.priceGroup||"",!0):!1}function Vl(e){const t=s.masterStats?.customers.find(n=>n.name===e.trim());return t?(s.invoiceForm.customerCode=t.code,s.invoiceForm.customerName=t.name,s.invoicePriceGroup=t.priceGroup||"",!0):!1}function Ya(e){if(V(e),s.invoiceErrors=Ua(s.invoiceForm),Object.keys(s.invoiceErrors).length>0){y();return}s.invoiceSaving=!0,y(),ua(s.invoiceForm).then(t=>{s.invoiceSavedDocNo=t.documentNo,s.invoiceSaving=!1,s.invoiceErrors={},s.invoiceForm=wt(),y()}).catch(()=>{s.invoiceSaving=!1,y()})}function Ha(e){const t=s.salesFilter.startDate?new Date(s.salesFilter.startDate):null,n=s.salesFilter.endDate?new Date(`${s.salesFilter.endDate}T23:59:59`):null;return[...e.salesRecords].sort((i,c)=>new Date(c.date).getTime()-new Date(i.date).getTime()).filter(i=>{const c=new Date(i.date);return!(t&&c<t||n&&c>n)})}function Xa(){switch(s.emailAudienceMode){case"area":return s.emailRegion==="all"?Le:Le.filter(e=>e.area===s.emailRegion);case"history":return Le.filter(e=>e.historySegment===s.emailHistorySegment);default:return Le}}function zl(){const e=Xa();return{audienceMode:s.emailAudienceMode,region:s.emailRegion,historySegment:s.emailHistorySegment,selectedTemplateId:s.emailTemplateId,subject:s.emailSubject,body:s.emailBody,recipientCount:e.length,previewRecipients:e.slice(0,5),saveMessage:s.emailSaveMessage,sending:s.emailSending,senderId:s.emailSenderId,senders:s.mailSenders}}function et(e){const t=Xa(),n=s.emailAudienceMode==="area"?s.emailRegion:s.emailAudienceMode==="history"?s.emailHistorySegment:"all";return{subject:s.emailSubject.trim(),body:s.emailBody.trim(),templateId:s.emailTemplateId,audienceMode:s.emailAudienceMode,audienceFilter:n,recipientCount:t.length,recipients:t.map(i=>i.email),status:e}}function _t(){return s.user,!1}function ve(){s.globalSearchOpen=!1,s.globalQuery=""}function Jl(){const e=s.globalQuery.trim().toLowerCase();return e?{customers:s.masterStats?.customers.filter(t=>t.code.toLowerCase().includes(e)||t.name.toLowerCase().includes(e))??[],products:s.masterStats?.products.filter(t=>t.code.toLowerCase().includes(e)||t.name.toLowerCase().includes(e))??[],documents:s.invoiceRecords.filter(t=>t.documentNo.toLowerCase().includes(e)||t.customerName.toLowerCase().includes(e)||t.date.toLowerCase().includes(e)),pages:Xt.filter(t=>t.path.toLowerCase().includes(e)||t.title.toLowerCase().includes(e))}:{customers:[],products:[],documents:[],pages:Xt}}function Bl(){let e=[],t,n="export.csv";switch(s.route){case"/sales":e=(s.salesSummary?Ha(s.salesSummary):[]).map(i=>({documentNo:i.documentNo,date:i.date,customerCode:i.customerCode,customerName:i.customerName,amount:i.amount})),t=[{key:"documentNo",label:"伝票番号"},{key:"date",label:"日付"},{key:"customerCode",label:"得意先コード"},{key:"customerName",label:"得意先名"},{key:"amount",label:"金額"}],n="sales.csv";break;case"/payment":e=[...s.paymentStatus?.records??[]].sort((i,c)=>c.balanceAmount-i.balanceAmount).map(i=>({...i})),t=[{key:"customerCode",label:"得意先コード"},{key:"customerName",label:"得意先名"},{key:"billedAmount",label:"請求額"},{key:"paymentAmount",label:"入金額"},{key:"balanceAmount",label:"請求残"},{key:"lastPaymentDate",label:"最終入金日"},{key:"status",label:"状態"}],n="payment-status.csv";break;case"/invoice":e=s.invoiceRecords.map(i=>({...i})),t=[{key:"documentNo",label:"伝票番号"},{key:"date",label:"日付"},{key:"customerCode",label:"得意先コード"},{key:"customerName",label:"得意先名"},{key:"itemCount",label:"明細数"},{key:"amount",label:"金額"}],n="invoices.csv";break;case"/purchase":e=s.purchaseList.map(i=>({...i})),t=[{key:"documentNo",label:"伝票番号"},{key:"purchaseDate",label:"仕入日"},{key:"supplierCode",label:"仕入先コード"},{key:"supplierName",label:"仕入先名"},{key:"itemName",label:"品目"},{key:"quantity",label:"数量"},{key:"unitPrice",label:"単価"},{key:"amount",label:"金額"},{key:"status",label:"状態"}],n="purchase.csv";break;case"/jikomi":e=s.jikomiList.map(i=>({...i})),t=[{key:"jikomiNo",label:"仕込番号"},{key:"productName",label:"銘柄"},{key:"riceType",label:"原料米"},{key:"plannedKg",label:"計画量"},{key:"actualKg",label:"実績量"},{key:"startDate",label:"開始日"},{key:"expectedDoneDate",label:"完了予定日"},{key:"tankNo",label:"タンク"},{key:"status",label:"状態"},{key:"note",label:"備考"}],n="jikomi.csv";break;case"/tanks":e=s.tankList.map(i=>({...i})),t=[{key:"tankNo",label:"タンクNo."},{key:"capacity",label:"容量"},{key:"currentVolume",label:"現在量"},{key:"productName",label:"銘柄"},{key:"jikomiNo",label:"仕込番号"},{key:"status",label:"状態"},{key:"lastUpdated",label:"更新日"}],n="tanks.csv";break;case"/kentei":e=s.kenteiList.map(i=>({...i})),t=[{key:"kenteiNo",label:"検定番号"},{key:"jikomiNo",label:"仕込番号"},{key:"productName",label:"銘柄"},{key:"kenteiDate",label:"検定日"},{key:"alcoholDegree",label:"アルコール度数"},{key:"extractDegree",label:"エキス分"},{key:"sakaMeterValue",label:"酒度"},{key:"volume",label:"容量"},{key:"taxCategory",label:"酒類区分"},{key:"status",label:"状態"}],n="kentei.csv";break;case"/materials":e=s.materialList.map(i=>({...i})),t=[{key:"code",label:"コード"},{key:"name",label:"品名"},{key:"unit",label:"単位"},{key:"currentStock",label:"現在庫"},{key:"minimumStock",label:"最低在庫"},{key:"unitCost",label:"単価"},{key:"lastUpdated",label:"更新日"}],n="materials.csv";break;case"/master":s.masterTab==="customers"?(e=s.masterStats?.customers.map(i=>({...i}))??[],t=[{key:"code",label:"得意先コード"},{key:"name",label:"得意先名"},{key:"closingDay",label:"締日"},{key:"paymentDay",label:"入金日"},{key:"isActive",label:"有効"}],n="master-customers.csv"):(e=s.masterStats?.products.map(i=>({...i}))??[],t=[{key:"code",label:"商品コード"},{key:"janCode",label:"JAN"},{key:"name",label:"商品名"},{key:"category",label:"カテゴリ"},{key:"isActive",label:"有効"}],n="master-products.csv");break;default:return}Dl(n,e,t)}function Qt(e){const t=`${"/".replace(/\/$/,"")}${e==="/"?"/":e}`;history.pushState(null,"",t),s.route=e,s.currentCategory=kt(e),s.sidebarOpen=!1,ve(),St(e)}async function St(e){s.actionLoading=!0,y();try{switch(e){case"/delivery":s.deliveryNote||(s.deliveryNote=await yt(s.deliverySearchDocNo||"D240122"));break;case"/billing":s.billingSummary||(s.billingSummary=await ht(s.billingYearMonth));break;case"/report":s.salesReport||(s.salesReport=await Ve());break;case"/customer-analysis":s.customerAnalysis||(s.customerAnalysis=await ya());break;case"/product-abc":s.productABC||(s.productABC=await ha());break;case"/jikomi":s.jikomiList.length===0&&(s.jikomiList=await fa());break;case"/tanks":s.tankList.length===0&&(s.tankList=await ba());break;case"/kentei":s.kenteiList.length===0&&(s.kenteiList=await ga());break;case"/materials":s.materialList.length===0&&(s.materialList=await at());break;case"/purchase":(s.purchaseList.length===0||s.payableList.length===0)&&([s.purchaseList,s.payableList]=await Promise.all([$a(),wa()]));break;case"/raw-material":(s.billList.length===0||s.rawStockList.length===0)&&([s.billList,s.rawStockList]=await Promise.all([ka(),_a()]));break;case"/tax":s.taxDeclaration||(s.taxDeclaration=await vt(s.taxYear,s.taxMonth));break;case"/store":(s.storeSales.length===0||s.storeOrders.length===0)&&([s.storeSales,s.storeOrders]=await Promise.all([ft(s.storeSalesDate),Aa()]));break;case"/mail-senders":case"/email":{const{fetchMailSenders:t}=await f(async()=>{const{fetchMailSenders:n}=await Promise.resolve().then(()=>g);return{fetchMailSenders:n}},void 0);if(s.mailSenders=await t(),!s.emailSenderId||!s.mailSenders.find(n=>n.id===s.emailSenderId)){const n=s.mailSenders.find(i=>i.isDefault)??s.mailSenders[0];n&&(s.emailSenderId=n.id)}}break;case"/calendar":{const{fetchCalendarEvents:t}=await f(async()=>{const{fetchCalendarEvents:n}=await Promise.resolve().then(()=>g);return{fetchCalendarEvents:n}},void 0);s.calendarEvents=await t(s.calendarYearMonth)}break;case"/integrations":{const{fetchIntegrationSettings:t}=await f(async()=>{const{fetchIntegrationSettings:n}=await Promise.resolve().then(()=>g);return{fetchIntegrationSettings:n}},void 0);s.integrations=await t()}break;case"/shopify":{const{fetchShopifyOrders:t,fetchIntegrationSettings:n}=await f(async()=>{const{fetchShopifyOrders:i,fetchIntegrationSettings:c}=await Promise.resolve().then(()=>g);return{fetchShopifyOrders:i,fetchIntegrationSettings:c}},void 0);s.shopifyOrders=await t(),s.integrations.length===0&&(s.integrations=await n())}break;case"/fax":{const{fetchFaxInbox:t,fetchIntegrationSettings:n}=await f(async()=>{const{fetchFaxInbox:i,fetchIntegrationSettings:c}=await Promise.resolve().then(()=>g);return{fetchFaxInbox:i,fetchIntegrationSettings:c}},void 0);s.faxRecords=await t(),s.integrations.length===0&&(s.integrations=await n())}break;case"/users":{const{fetchUserProfiles:t}=await f(async()=>{const{fetchUserProfiles:n}=await Promise.resolve().then(()=>g);return{fetchUserProfiles:n}},void 0);s.userProfiles=await t()}break;case"/profile":{const{fetchMyProfile:t,fetchAuditLogs:n,fetchMailSenders:i}=await f(async()=>{const{fetchMyProfile:a,fetchAuditLogs:l,fetchMailSenders:o}=await Promise.resolve().then(()=>g);return{fetchMyProfile:a,fetchAuditLogs:l,fetchMailSenders:o}},void 0),c=s.user?.email??s.myProfile?.email??"";c&&(s.myProfile=await t(c)),s.mailSenders.length===0&&(s.mailSenders=await i()),s.auditLogs=await n(50)}break;case"/audit":{const{fetchAuditLogs:t}=await f(async()=>{const{fetchAuditLogs:n}=await Promise.resolve().then(()=>g);return{fetchAuditLogs:n}},void 0);s.auditLogs=await t(200)}break;case"/prospects":{const{fetchProspects:t}=await f(async()=>{const{fetchProspects:n}=await Promise.resolve().then(()=>g);return{fetchProspects:n}},void 0);s.prospects=await t()}break;case"/map":{const{fetchProspects:t,fetchDeliveryLocations:n,fetchIntegrationSettings:i}=await f(async()=>{const{fetchProspects:c,fetchDeliveryLocations:a,fetchIntegrationSettings:l}=await Promise.resolve().then(()=>g);return{fetchProspects:c,fetchDeliveryLocations:a,fetchIntegrationSettings:l}},void 0);s.prospects=await t(),s.deliveryLocations=await n(),s.integrations.length===0&&(s.integrations=await i())}break;case"/calls":{const{fetchCallLogs:t,fetchIntegrationSettings:n}=await f(async()=>{const{fetchCallLogs:i,fetchIntegrationSettings:c}=await Promise.resolve().then(()=>g);return{fetchCallLogs:i,fetchIntegrationSettings:c}},void 0);s.callLogs=await t(100),s.integrations.length===0&&(s.integrations=await n())}break;case"/list-builder":{const{fetchLeadLists:t,fetchIntegrationSettings:n}=await f(async()=>{const{fetchLeadLists:i,fetchIntegrationSettings:c}=await Promise.resolve().then(()=>g);return{fetchLeadLists:i,fetchIntegrationSettings:c}},void 0);s.leadLists=await t(),s.integrations.length===0&&(s.integrations=await n())}break;case"/workflow":{const{fetchWorkflowOrdersFromDb:t}=await f(async()=>{const{fetchWorkflowOrdersFromDb:n}=await Promise.resolve().then(()=>g);return{fetchWorkflowOrdersFromDb:n}},void 0);s.workflowOrders=await t()}break;case"/tour":{const{fetchTourInquiriesFromDb:t}=await f(async()=>{const{fetchTourInquiriesFromDb:n}=await Promise.resolve().then(()=>g);return{fetchTourInquiriesFromDb:n}},void 0);s.tourInquiries=await t()}break;case"/slack":{const{fetchSlackRules:t,fetchSlackLogs:n,fetchIntegrationSettings:i}=await f(async()=>{const{fetchSlackRules:c,fetchSlackLogs:a,fetchIntegrationSettings:l}=await Promise.resolve().then(()=>g);return{fetchSlackRules:c,fetchSlackLogs:a,fetchIntegrationSettings:l}},void 0);s.slackRules=await t(),s.slackLogs=await n(50),s.integrations.length===0&&(s.integrations=await i())}break;case"/":{const{fetchProspects:t,fetchCalendarEvents:n,fetchWorkflowOrdersFromDb:i,fetchTourInquiriesFromDb:c}=await f(async()=>{const{fetchProspects:a,fetchCalendarEvents:l,fetchWorkflowOrdersFromDb:o,fetchTourInquiriesFromDb:r}=await Promise.resolve().then(()=>g);return{fetchProspects:a,fetchCalendarEvents:l,fetchWorkflowOrdersFromDb:o,fetchTourInquiriesFromDb:r}},void 0);s.prospects.length===0&&(s.prospects=await t()),s.calendarEvents.length===0&&(s.calendarEvents=await n(s.calendarYearMonth)),s.materialList.length===0&&(s.materialList=await at()),s.workflowOrders.length===0&&(s.workflowOrders=await i()),s.tourInquiries.length===0&&(s.tourInquiries=await c())}break;default:break}}catch(t){console.warn("Route data load error",t)}finally{s.actionLoading=!1,y()}}function Wt(){if(_t())return Kn(s.authError,s.authSubmitting);if(s.loading)return`
    ...
    `;switch(s.route){case"/cat/sales":return ge("sales");case"/cat/brewery":return ge("brewery");case"/cat/purchase":return ge("purchase");case"/cat/more":return ge("more");case"/invoice-entry":return Mn(s.invoiceForm,s.invoiceSavedDocNo,s.invoiceSaving,s.invoiceErrors);case"/email":return jn(zl());case"/delivery":return s.deliveryNote?Tn(s.deliveryNote,s.deliverySearchDocNo):'<section class="panel"><div class="loading-overlay"><div class="loading-spinner"></div><p class="loading-text">データを読み込んでいます…</p></div></section>';case"/billing":return s.billingSummary?wn(s.billingSummary,s.billingYearMonth):'<section class="panel"><div class="loading-overlay"><div class="loading-spinner"></div><p class="loading-text">データを読み込んでいます…</p></div></section>';case"/report":return s.salesReport?xi(s.salesReport):'<section class="panel"><div class="loading-overlay"><div class="loading-spinner"></div><p class="loading-text">データを読み込んでいます…</p></div></section>';case"/customer-analysis":return s.customerAnalysis?vi(s.customerAnalysis):'<section class="panel"><div class="loading-overlay"><div class="loading-spinner"></div><p class="loading-text">データを読み込んでいます…</p></div></section>';case"/product-abc":return s.productABC?gi(s.productABC):'<section class="panel"><div class="loading-overlay"><div class="loading-spinner"></div><p class="loading-text">データを読み込んでいます…</p></div></section>';case"/jikomi":return s.jikomiView==="calendar"?`${Rt(s.jikomiList,s.jikomiView)}${Hn(s.jikomiList)}`:Rt(s.jikomiList,s.jikomiView);case"/tanks":return $l(s.tankList);case"/kentei":return Xn(s.kenteiList);case"/materials":return si(s.materialList)+ai(s.materialEditing,s.materialEditingIsNew);case"/purchase":return ri(s.purchaseList,s.payableList);case"/raw-material":return ci(s.billList,s.rawStockList);case"/tax":return s.taxDeclaration?wl(s.taxDeclaration,s.taxYear,s.taxMonth):'<section class="panel"><div class="loading-overlay"><div class="loading-spinner"></div><p class="loading-text">データを読み込んでいます…</p></div></section>';case"/store":return Di(s.storeSales,s.storeOrders,s.storeTab,s.storeSalesDate);case"/setup":return s.pipelineMeta?pi(s.pipelineMeta,z,O,s.syncDashboard):'<section class="panel"><div class="loading-overlay"><div class="loading-spinner"></div><p class="loading-text">データを読み込んでいます…</p></div></section>';case"/raw-browser":return gl(s.rawSelectedTable,s.rawRecords,s.rawTableList,s.rawPage,s.rawTotalCount);case"/import":return Ii(s.importEntity,s.importPreview,s.importing,s.importResult);case"/print":return ul(s.printTemplate,s.printOptions,s.printCompany,s.printData);case"/form-designer":return Ti(s.printData,s.printCompany,s.printOptions,s.fdSavedPositions,s.fdDesignMode);case"/map":{const e=(s.masterStats?.customers??[]).slice(0,200).map((i,c)=>{const a=i;return{...i,lat:a.lat??35.37+c%12*.05+(Math.random()-.5)*.02,lng:a.lng??139.29+Math.floor(c/12)*.05+(Math.random()-.5)*.02,address1:a.address1??"",businessType:a.businessType??"",lastOrderAmount:0}}),n=!!s.integrations.find(i=>i.provider==="google_maps")?.config.api_key;return Ni(e,s.prospects,s.deliveryLocations,s.mapFilters,n)}case"/workflow":return Oi(s.workflowOrders);case"/mobile-order":return qi(s.mobileOrder,s.masterStats?.customers??[],s.masterStats?.products??[]);case"/tour":return Fi(s.tourInquiries,s.tourActiveId);case"/mail-senders":return Ji(s.mailSenders,s.mailSenderEditingId);case"/calendar":return Bi(s.calendarEvents,s.calendarYearMonth,s.calendarFilterCategory,s.calendarEdit);case"/integrations":return Yi(s.integrations,s.integrationEditingId);case"/shopify":{const e=s.integrations.find(t=>t.id==="shopify");return Hi(s.shopifyOrders,e?.lastSyncAt??null)}case"/fax":return Xi(s.faxRecords,s.faxProcessing,s.faxOcrText);case"/users":return Gi(s.userProfiles,s.userEditingId,s.myProfile);case"/profile":return Ki(s.myProfile,s.auditLogs.filter(e=>e.userEmail===s.myProfile?.email),s.mailSenders);case"/audit":return Qi(s.auditLogs);case"/prospects":{const e={prospects:s.prospects,activities:s.prospectActivities,editingId:s.prospectEditingId,viewMode:s.prospectViewMode};return Wi(e)}case"/slack":{const e=s.integrations.find(t=>t.provider==="slack")??null;return al(e,s.slackRules,s.slackLogs)}case"/calls":{const e=s.integrations.find(t=>t.provider==="ivry");return sl(s.callLogs,s.masterStats?.customers??[],e?.lastSyncAt??null,e?.isEnabled??!1)}case"/list-builder":{const e={lists:s.leadLists,activeListId:s.leadActiveListId,items:s.leadItems,searchQuery:s.leadSearchQuery,searchArea:s.leadSearchArea,searchBusinessType:s.leadSearchType,searching:s.leadSearching,searchResults:s.leadSearchResults};return il(e)}}if(!s.salesSummary||!s.paymentStatus||!s.masterStats||!s.pipelineMeta||!s.customerLedger||!s.salesAnalytics)return"";switch(s.route){case"/sales":return Li(Ha(s.salesSummary),s.salesFilter.startDate,s.salesFilter.endDate);case"/payment":return li([...s.paymentStatus.records].sort((e,t)=>t.balanceAmount-e.balanceAmount));case"/master":return ti(s.masterStats,s.masterTab,s.masterFilter);case"/invoice":return zn(s.invoiceRecords,s.invoiceFilter);case"/ledger":return An(s.customerLedger,s.ledgerCustomerCode);case"/analytics":return Si(s.salesAnalytics,s.analyticsTab);default:return En(s.salesSummary,s.pipelineMeta,s.salesAnalytics,{prospects:s.prospects,upcomingEvents:s.calendarEvents,tourInquiries:s.tourInquiries,workflowOrdersCount:{new:s.workflowOrders.filter(e=>e.stage==="new").length,picking:s.workflowOrders.filter(e=>e.stage==="picking").length,packed:s.workflowOrders.filter(e=>e.stage==="packed").length,shipped:s.workflowOrders.filter(e=>e.stage==="shipped").length,total:s.workflowOrders.length},lowStockCount:s.materialList.filter(e=>e.currentStock<e.minimumStock*1.5).length,masterCounts:s.masterStats?{customers:s.masterStats.summary.customerCount,products:s.masterStats.summary.productCount,suppliers:s.syncDashboard?.tables.find(e=>e.tableName==="suppliers")?.rowCount??0,specialPrices:s.syncDashboard?.tables.find(e=>e.tableName==="customer_product_prices")?.rowCount??0}:void 0},s.salesPeriod)}}function Ul(){if(_t())return`
    ...
    `;const e={dashboard:[{label:"概要",items:[{path:"/",label:"ダッシュボード",kicker:"Home"},{path:"/sales",label:"売上一覧",kicker:"Sales"},{path:"/payment",label:"入金状況",kicker:"Payment"}]}],sales:[{label:"販売管理",items:[{path:"/cat/sales",label:"販売管理トップ",kicker:"Category"},{path:"/invoice-entry",label:"伝票入力",kicker:"Entry"},{path:"/delivery",label:"納品書",kicker:"Delivery"},{path:"/billing",label:"月次請求",kicker:"Billing"},{path:"/report",label:"集計帳票",kicker:"Report"},{path:"/customer-analysis",label:"得意先分析",kicker:"CustABC"},{path:"/product-abc",label:"商品ABC",kicker:"ProdABC"},{path:"/invoice",label:"伝票照会",kicker:"Invoice"},{path:"/ledger",label:"得意先台帳",kicker:"Ledger"}]}],brewery:[{label:"蔵内管理",items:[{path:"/cat/brewery",label:"蔵内管理トップ",kicker:"Category"},{path:"/jikomi",label:"仕込管理",kicker:"Jikomi"},{path:"/tanks",label:"タンク管理",kicker:"Tank"},{path:"/kentei",label:"検定管理",kicker:"Kentei"},{path:"/materials",label:"資材管理",kicker:"Material"}]}],purchase:[{label:"仕入管理",items:[{path:"/cat/purchase",label:"仕入管理トップ",kicker:"Category"},{path:"/purchase",label:"仕入・買掛",kicker:"Purchase"},{path:"/raw-material",label:"手形・原料",kicker:"RawMat"}]}],more:[{label:"その他",items:[{path:"/cat/more",label:"その他トップ",kicker:"Category"},{path:"/tax",label:"酒税申告",kicker:"Tax"},{path:"/store",label:"店舗・直売所",kicker:"Store"},{path:"/analytics",label:"売上分析",kicker:"Analytics"},{path:"/master",label:"マスタ",kicker:"Master"},{path:"/email",label:"メール配信",kicker:"Mail"},{path:"/setup",label:"連動設定",kicker:"Setup"},{path:"/raw-browser",label:"データブラウザ",kicker:"RawData"}]}],email:[{label:"メール配信",items:[{path:"/email",label:"季節商品案内",kicker:"Mail"}]}]},t=[{category:"dashboard",path:"/",label:"ダッシュボード"},{category:"sales",path:"/cat/sales",label:"販売管理"},{category:"brewery",path:"/cat/brewery",label:"蔵内管理"},{category:"purchase",path:"/cat/purchase",label:"仕入管理"},{category:"more",path:"/cat/more",label:"その他"},{category:"email",path:"/email",label:"メール配信"}],n=e[s.currentCategory].map(o=>`
      ...
      `).join(""),c=s.pickerMode&&s.masterStats?s.pickerMode==="customer"?On(s.masterStats.customers,s.pickerQuery):oi(s.masterStats.products,s.pickerQuery):"",a=s.globalSearchOpen?Rn(s.globalQuery,Jl()):"",l=s.user?`
