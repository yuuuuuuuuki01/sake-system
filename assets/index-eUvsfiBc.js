...
`}function ol(e){const t=xs(e,.15),n=Mt(e,.88),s=Mt(e,.96);return`
...
`);return`
  ...
  `}function pa(e,t){return!e||e.length<=t?e||"":e.slice(0,t)+"…"}function El(e){switch(e){case"000":return"生産者";case"001":return"小売";case"002":return"卸";default:return e||"―"}}function Cl(e){return e.map(t=>`
      ...
      `).join("")}function Ll(e,t,n=Ma,s=[]){const{filtered:i,paged:l,totalPages:d}=kl(e.customers,n);return`
    <section class="page-head">
      <div>
  ...
  `}function Qe(e){return e.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}function rn(e){return e.trim().toLowerCase()}function Ol(e,t){const n=rn(t),s=e.filter(l=>n?[l.code,l.name,l.janCode].map(rn).some(d=>d.includes(n)):!0),i=s.length?`
        <div class="table-wrap">
          <table>
            <thead>
              <tr>
...
cd sake-system\\relay`,success:["エクスプローラーで sake-system\\\\relay フォルダが見える状態になっていれば成功です。","cd を実行したあと、コマンドプロンプトの行頭が relay フォルダを指していれば問題ありません。"],errors:["git コマンドが使えない場合は Git for Windows が未導入です。先に Git をインストールしてください。","アクセス拒否が出る場合は C:\\\\Program Files 配下ではなく C:\\\\sake-relay\\\\ などに配置してください。"]})}
  ...
  `}const Ul={code:"code",name:"name",amount:"amount",quantity:"quantity",documents:"documents",volumeMl:"volumeMl"},cn={amount:"売上額",quantity:"出荷本数",volume:"移出量"},ya=10;function Ra(e){const[t,n]=e.split("-").map(Number);return n>=ya?t:t-1}function Ql(e){const t=ya-1,n=new Date(e+1,t,0).getDate();return{from:`${e}-${String(ya).padStart(2,"0")}-01`,to:`${e+1}-${String(t).padStart(2,"0")}-${String(n).padStart(2,"0")}`}}function Hl(e,t,n){const s=d=>t==="quantity"?d.quantity:t==="volume"?d.volumeMl:d.amount,i=new Map;for(const d of e){const p=n==="fiscal"?`${Ra(d.month)}年度`:d.month.slice(0,4);i.set(p,(i.get(p)??0)+s(d))}return{curr:[...i.entries()].sort((d,p)=>d[0].localeCompare(p[0])).map(([d,p])=>({month:d,amount:p}))}}function Kl(e){const t=new Set;for(const n of e)t.add(Ra(n.month));return[...t].sort((n,s)=>s-n).map(String)}function Ze(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function Gl(e){return e.replace("-","/")}const dn={all:"全期間",yearly:"年次",monthly:"月次",weekly:"週次",daily:"日次"};function Xl(e,t="#0F5B8D",n=[],s="amount"){if(e.length===0)return'<div class="chart-empty">データなし</div>';const i=n.length>0&&n.some(m=>m.amount>0),l=760,d=280,p={top:16,right:24,bottom:36,left:s==="amount"?64:56},u=l-p.left-p.right,y=d-p.top-p.bottom,v=[...e.map(m=>m.amount),...n.map(m=>m.amount)],f=Math.max(...v,1),b=u/e.length;function k(m){if(s==="quantity")return m>=1e4?`${(m/1e4).toFixed(1)}万本`:`${Math.round(m).toLocaleString()}本`;if(s==="volume"){const g=m/1e3;return g>=1e4?`${(g/1e3).toFixed(0)}kL`:`${Math.round(g).toLocaleString()} L`}return`${Math.round(m/1e4).toLocaleString("ja-JP")}万円`}function P(m){return s==="quantity"?`${m.toLocaleString()}本`:s==="volume"?Vt(m):Ze(m)}const o=[0,.25,.5,.75,1].map(m=>{const g=p.top+y-y*m,$=k(f*m);return`<g>
      ...
      </g>`}).join(""),r=e.map((m,g)=>{const $=i?Math.max((b-18)/2,10):Math.max(b-18,24),w=i?2:0,_=p.left+g*b+(b-(i?$*2+w:$))/2,S=m.amount/f*y,E=p.top+y-S,A=n[g]?.amount??0,L=A/f*y,O=p.top+y-L,D=i?`<rect x="${_}" y="${O}" width="${$}" height="${L}" rx="4" fill="#ccc" opacity="0.6"><title>前年 ${P(A)}</title></rect>`:"",I=i?_+$+w:_;return`<g>
      ${D}
  ...
  `}function Ls(e,t,n="all",s="",i=[],l=[],d="",p="",u=null,y="all",v="",f=[],b=[],k=[],P=null,o=[],r=[],c="amount",m="calendar"){const g=t==="products"?"商品別集計":t==="customers"?"得意先別集計":"担当別集計",$=t==="products"?e.productTotals:t==="customers"?e.customerTotals:e.staffTotals,_=n!=="all"&&i.length>0&&t!=="staff"?i:$,S=We(_,k,Ul),E={all:"月別",yearly:"月別推移",monthly:"日別推移",weekly:"日別推移",daily:"当日"},A=cn[c],L=V=>c==="quantity"?V.quantity:c==="volume"?V.volumeMl:V.amount,O=V=>c==="quantity"?`${V.toLocaleString()}本`:c==="volume"?Vt(V):Ze(V);let D,I=[],M,B,z;if(P&&P.monthlySales.length>0)D=P.monthlySales.slice(-24).map(V=>({month:V.month,amount:L(V)})),M=`${P.name} の月別${A}`,B=`${P.tab==="customers"?"得意先":"商品"}: ${P.code}`,z="#0968e5";else if(o.length>0&&n!=="all"){D=o.map(Y=>({month:Y.month,amount:L(Y)})),I=r.map(Y=>({month:Y.month,amount:L(Y)}));const V=D.reduce((Y,X)=>Y+X.amount,0),ee=I.reduce((Y,X)=>Y+X.amount,0),$e=ee>0?(V-ee)/ee*100:0,ge=$e>0?"+":"";M=`${E[n]} ${A}（${s}）`,B=`${O(V)}${ee>0?` / 前年比 ${ge}${$e.toFixed(1)}%`:""}`,z="#2f855a"}else{D=Hl(e.monthlySales,c,m).curr,I=[];const ee=D.reduce((ge,Y)=>ge+Y.amount,0);M=`${m==="fiscal"?"決算年度別":"暦年別"}${A}`,B=`累計 ${O(ee)}（${D.length}${m==="fiscal"?"期":"年"}）`,z="#0F5B8D"}const H=["amount","quantity","volume"].map(V=>`<button class="tab-button ${V===c?"active":""}" data-chart-metric="${V}">${cn[V]}</button>`).join(""),F=["all","yearly","monthly","weekly","daily"].map(V=>`<button class="button ${V===n?"primary":"secondary"} small" type="button" data-analytics-period="${V}">${dn[V]}</button>`).join(""),j=m==="fiscal"&&n==="yearly"?Kl(e.monthlySales):l,Q=m==="fiscal"&&n==="yearly"&&!j.includes(s)?j[0]??"":s,Z=n!=="all"&&j.length>0&&t!=="staff"?`<select id="analytics-period-select" style="padding:6px 10px;border:1px solid var(--border);border-radius:6px;font-size:13px;">
...
金井酒造店`;function vc(e,t){const n=t?e.find(i=>i.id===t):null,s=t==="__new__";return`
          ...
          <div class="cal-events">
            ${E.slice(0,3).map(O=>`
              <button class="cal-event" data-cal-event-id="${O.id}"
                      style="background:${O.color||Ta[O.category]||"#0F5B8D"};"
                      title="${O.title}">
  ...
  `}function zc(e,t,n,s){let i="";switch(e){case"chain_store":i=Mc(s,n,t);break;case"quotation":i=jc(s,n,t);break;case"invoice_monthly":i=Rc(s,n,t);break}const l=Object.keys(vn).map(u=>`<button class="tab-button ${e===u?"active":""}" data-print-template="${u}">${vn[u]}</button>`).join(""),d=s.lines.map((u,y)=>`
      <tr>
...
`}async function Ns(e,t){const{supabaseInsert:n}=await q(async()=>{const{supabaseInsert:p}=await Promise.resolve().then(()=>G);return{supabaseInsert:p}},void 0);let s=0,i=0;const d={customers:"customers",products:"products",suppliers:"suppliers",staff:"staff"}[e];for(const p of t){if(!p._valid)continue;const{_valid:u,_error:y,...v}=p,f={...v};if(!f.id){const b=e==="customers"?"legacy_customer_code":e==="products"?"legacy_product_code":e==="suppliers"?"legacy_supplier_code":"legacy_staff_code";f.id=String(v[b]??`${e}-${Date.now()}-${s+i}`)}for(const b of["volume_ml","closing_day","payment_day","list_price","default_sale_price"])if(typeof f[b]=="string"&&f[b]!==""){const k=Number(f[b]);Number.isFinite(k)&&(f[b]=k)}try{await n(d,f)!==null?s++:i++}catch{i++}}return{inserted:s,failed:i}}const Vc=Object.freeze(Object.defineProperty({__proto__:null,generateTemplateCSV:Ts,importToSupabase:Ns,parseCSV:qs,validateImport:Is},Symbol.toStringTag,{value:"Module"}));function Wt(e){return new Intl.DateTimeFormat("ja-JP",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"}).format(new Date(e))}function Yc(e,t=40){return!e||e.length<=t?e||"":e.slice(0,t)+"…"}function Jc(e,t,n,s,i){const l=n.reduce((y,v)=>y+v.rowCount,0),d=n.map(y=>y.lastSyncAt).filter(y=>y!==null).sort().reverse()[0]??null,p=100,u=Math.max(1,Math.ceil(i/p));return`
  ...
  `}const tt=400,at=240;function W(e){return e.toLocaleString("ja-JP")}function Zt(e){const[t,n]=e.split("-");return`${t.slice(2)}/${n}`}function Uc(e,t){return!e||e.column!==t?'<span style="opacity:0.3;margin-left:2px;">⇅</span>':e.dir==="asc"?'<span style="margin-left:2px;">↑</span>':'<span style="margin-left:2px;">↓</span>'}function Ee(e,t,n,s=""){return`<th class="${s}" style="cursor:pointer;user-select:none;white-space:nowrap;"
    ...
    `}).join(""),o=t.map((c,m)=>{let g=p.top+y;const $=p.left+m*b+(b-k)/2,w=s.map((O,D)=>{const M=(n[O.code]?.[c]??0)/f*y;return g-=M,`<rect x="${$}" y="${g}" width="${k}" height="${M}" fill="${i[D%i.length]}" opacity="0.85" rx="${D===s.length-1?3:0}" />`}).join(""),[_,S]=c.split("-"),E=parseInt(S),A=E===1||m%3===0,L=E===1?`${_.slice(2)}年`:`${E}月`;return`<g>${w}${A?`<text x="${$+k/2}" y="${d-10}" class="chart-axis centered-axis">${L}</text>`:""}</g>`}).join(""),r=s.map((c,m)=>`<span style="display:inline-flex;align-items:center;gap:4px;margin-right:12px;font-size:11px;">
  ...
  `}const Xc={monthly:"月次",annual:"年次",make_to_order:"受注生産",november:"11月生産"};function Wc(e,t,n,s){const i={draft:"下書き",confirmed:"確定",actual:"実績入力済"},l={draft:"neutral",confirmed:"info",actual:"success"},d=S=>Object.entries(Xc).map(([E,A])=>`<option value="${E}" ${E===S?"selected":""}>${A}</option>`).join(""),p=640,u=S=>S.map(E=>{const A=Math.max(0,E.demandForecast+E.safetyStockTarget-E.openingStock),L=E.plannedQty>0?E.plannedQty:Math.round(A),O=L>0?Math.ceil(L/p*10)/10:0,D=E.plannedQty>0?(E.actualQty-E.plannedQty)/E.plannedQty*100:null,I=D!==null?D>=0?"text-success":"text-danger":"";return`
    ...
    `}).join(""),v=(n==="all"?e:e.filter(S=>S.productionType===n)).slice().sort((S,E)=>{if(!s)return 0;const A=s.dir==="asc"?1:-1,L=Math.max(0,S.demandForecast+S.safetyStockTarget-S.openingStock),O=Math.max(0,E.demandForecast+E.safetyStockTarget-E.openingStock);switch(s.column){case"plan-name":return A*S.productName.localeCompare(E.productName,"ja");case"plan-forecast":return A*(S.demandForecast-E.demandForecast);case"plan-required":return A*(L-O);case"plan-planned":return A*(S.plannedQty-E.plannedQty);case"plan-actual":return A*(S.actualQty-E.actualQty);case"plan-label":{const D=S.plannedQty>0?S.plannedQty:Math.round(L),I=E.plannedQty>0?E.plannedQty:Math.round(O);return A*(D-I)}default:return 0}}),f=u(v),b=[{key:"all",label:"全て"},{key:"monthly",label:"月次"},{key:"annual",label:"年次"},{key:"november",label:"11月生産"},{key:"make_to_order",label:"受注生産"}],k=S=>{const A=(S==="all"?e:e.filter(L=>L.productionType===S)).reduce((L,O)=>{const D=Math.max(0,O.demandForecast+O.safetyStockTarget-O.openingStock);return L+(O.plannedQty>0?O.plannedQty:Math.round(D))},0);return Math.ceil(A/p*10)/10},P=b.filter(S=>S.key!=="all").map(S=>{const E=k(S.key),A=e.filter(O=>O.productionType===S.key).length,L=S.key==="make_to_order"?e.filter(O=>O.productionType==="make_to_order"&&O.plannedQty>0).length:null;return`
  ...
  `}function Os(e){const[t,n]=e.split("-").map(Number),s=new Date(t,n,0).getDate();return Array.from({length:s},(i,l)=>{const d=l+1;return`${e}-${String(d).padStart(2,"0")}`})}function bn(e){const t=new Date(e).getDay();return["日","月","火","水","木","金","土"][t]}function $n(e){const t=new Date(e).getDay();return t===0||t===6}function Zc(e,t){return e.partTimers*t.partCapacity+e.employees*t.empCapacity}function Ms(e){return e.partTimers+e.employees}function Ce(e,t,n={partCapacity:tt,empCapacity:at}){const s=e.filter(v=>v.partTimers>0||v.employees>0);if(s.length===0)return;const i=t.reduce((v,f)=>{const b=f.plannedQty>0?f.plannedQty:Math.max(0,f.demandForecast+f.safetyStockTarget-f.openingStock);return v+b},0);if(i<=0)return;const l=i/s.length;let d=0,p=0,u=1/0;const y=Math.ceil(l/n.partCapacity);for(let v=0;v<=y;v++){const f=l-v*n.partCapacity,b=f>0?Math.ceil(f/n.empCapacity):0,k=v+b;k<u&&(u=k,d=v,p=b)}for(const v of e)v.confirmed||(v.partTimers>0||v.employees>0)&&(v.partTimers=d,v.employees=p)}function ed(e,t,n={partCapacity:tt,empCapacity:at}){const s=t.filter(p=>Ms(p)>0).map(p=>p.date).sort();if(s.length===0)return t.map(p=>({date:p.date,partTimers:p.partTimers,employees:p.employees,confirmed:p.confirmed,capacity:0,items:[],totalQty:0,utilization:0}));const i={monthly:0,november:1,annual:2,make_to_order:3},l=e.filter(p=>p.plannedQty>0||Math.max(0,p.demandForecast+p.safetyStockTarget-p.openingStock)>0).map(p=>({productCode:p.productCode,productName:p.productName,productionType:p.productionType,remaining:p.plannedQty>0?p.plannedQty:Math.max(0,p.demandForecast+p.safetyStockTarget-p.openingStock)})).filter(p=>p.remaining>0).sort((p,u)=>(i[p.productionType]??99)-(i[u.productionType]??99)||u.remaining-p.remaining),d=new Map;for(const p of t){const u=Zc(p,n);d.set(p.date,{date:p.date,partTimers:p.partTimers,employees:p.employees,confirmed:p.confirmed,capacity:u,items:[],totalQty:0,utilization:0})}for(const p of l){let u=p.remaining;if(u<=0)continue;if(s.reduce((v,f)=>{const b=d.get(f);return v+Math.max(0,b.capacity-b.totalQty)},0)<=0)break;for(const v of s){if(u<=0)break;const f=d.get(v),b=Math.max(0,f.capacity-f.totalQty);if(b<=0)continue;const k=Math.min(u,b);f.items.push({productCode:p.productCode,productName:p.productName,productionType:p.productionType,qty:k}),f.totalQty+=k,f.utilization=f.capacity>0?f.totalQty/f.capacity:0,u-=k}}return t.map(p=>d.get(p.date))}function qt(e,t=1,n=1){return Os(e).map(s=>({date:s,partTimers:$n(s)?0:t,employees:$n(s)?0:n,confirmed:!1}))}function td(e,t,n,s=null,i=new Set,l={partCapacity:tt,empCapacity:at}){const d=Os(t),p=e.filter(D=>!i.has(D.productCode)),u=ed(p,n,l),y=new Map(u.map(D=>[D.date,D])),v=p.reduce((D,I)=>D+(I.plannedQty>0?I.plannedQty:Math.max(0,I.demandForecast+I.safetyStockTarget-I.openingStock)),0),b=e.reduce((D,I)=>D+(I.plannedQty>0?I.plannedQty:Math.max(0,I.demandForecast+I.safetyStockTarget-I.openingStock)),0)-v,k=u.reduce((D,I)=>D+I.totalQty,0),P=n.filter(D=>Ms(D)>0).length,o=u.reduce((D,I)=>D+I.capacity,0),r=n.reduce((D,I)=>D+I.partTimers,0),c=n.reduce((D,I)=>D+I.employees,0),m=P>0?Math.ceil(v/P):0,g=new Date,$=Array.from({length:24},(D,I)=>{const M=new Date(g.getFullYear(),g.getMonth()-6+I,1),B=`${M.getFullYear()}-${String(M.getMonth()+1).padStart(2,"0")}`;return`<option value="${B}" ${B===t?"selected":""}>${B.replace("-","年")}月</option>`}).join(""),w=new Date(d[0]).getDay(),_=[];for(let D=0;D<w;D++)_.push('<div style="min-height:44px;"></div>');for(const D of d){const I=y.get(D),M=new Date(D).getDay(),B=parseInt(D.split("-")[2]),z=I?.partTimers??0,H=I?.employees??0,F=z+H,j=I?.totalQty??0,Q=I?.utilization??0,Z=D===s,re=F===0?"var(--surface-alt)":Q>.95?"rgba(197,61,61,0.12)":Q>.7?"rgba(183,121,31,0.10)":Q>0?"rgba(47,133,90,0.08)":"var(--surface)",ie=F===0?"transparent":Q>.95?"#c53d3d":Q>.7?"#b7791f":Q>0?"#2f855a":"var(--border)",V=M===0?"#c53d3d":M===6?"#0F5B8D":"var(--text)",ee=F>0?`<span style="font-size:8px;color:var(--text-secondary);line-height:1;">${z>0?`パ${z}`:""}${H>0?`社${H}`:""}</span>`:"";_.push(`
    ...
    `)}const E=_.length%7;if(E>0)for(let D=0;D<7-E;D++)_.push('<div style="min-height:44px;"></div>');const A=s?y.get(s):null;s&&n.find(D=>D.date===s);const L=s&&A?(()=>{const D=A,I=parseInt(s.split("-")[2]),M=bn(s),B=Math.round(D.utilization*100),z=n.find(Y=>Y.date===s),H=s===new Date().toISOString().slice(0,10),F={monthly:"#0F5B8D",november:"#B7791F",annual:"#6B46C1",make_to_order:"#999"},j={monthly:"月次",november:"11月",annual:"年次",make_to_order:"受注"},Q=D.items.map(Y=>`
      ...
      <section class="panel" style="margin-top:12px;border:2px solid ${H?"#2f855a":"#0F5B8D"};">
        <div style="padding:12px 16px 8px;${H?"background:rgba(47,133,90,0.06);":""}">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
            ...
            <h2 style="margin:0;font-size:16px;">${I}日（${M}）${H?"":"の生産内訳"}</h2>
          </div>
  ...
  `:"",O=[{color:"#0F5B8D",label:"月次"},{color:"#B7791F",label:"11月"},{color:"#6B46C1",label:"年次"},{color:"#999",label:"受注"}].map(D=>`<span style="display:inline-flex;align-items:center;gap:2px;font-size:10px;">
    ...
    <section class="panel" style="margin-top:12px;" id="cal-label-section">
      ...
      <div id="cal-label-list" style="max-height:500px;overflow-y:auto;">
        ${(()=>{const D=[{key:"monthly",label:"月次",color:"#0F5B8D"},{key:"november",label:"11月生産",color:"#B7791F"},{key:"annual",label:"年次",color:"#6B46C1"},{key:"make_to_order",label:"受注生産",color:"#999"}],I=new Map;for(const M of e){if((M.plannedQty>0?M.plannedQty:Math.max(0,M.demandForecast+M.safetyStockTarget-M.openingStock))<=0)continue;const z=M.productionType||"monthly";I.has(z)||I.set(z,[]),I.get(z).push(M)}return D.filter(M=>I.has(M.key)).map(M=>{const B=I.get(M.key),z=B.reduce((Z,re)=>Z+(re.plannedQty>0?re.plannedQty:Math.max(0,re.demandForecast+re.safetyStockTarget-re.openingStock)),0),H=B.filter(Z=>i.has(Z.productCode)).length,F=H===B.length,j=H===0,Q=B.map(Z=>{const re=Z.plannedQty>0?Z.plannedQty:Math.max(0,Z.demandForecast+Z.safetyStockTarget-Z.openingStock),ie=i.has(Z.productCode);return`
              ...
              <div style="border-bottom:2px solid var(--border);">
                <div style="display:flex;align-items:center;gap:8px;padding:10px 4px;background:var(--surface-alt);position:sticky;top:0;z-index:1;">
                  ...
                  <div style="flex:1;font-size:13px;font-weight:600;">${M.label}<span style="font-weight:400;color:var(--text-secondary);margin-left:6px;">${B.length}品 ${W(Math.round(z))}本</span></div>
...
${o?.stack??""}</div></section>`}return`
  ...
  `}const Ne={純米大吟醸:"#7c3aed",大吟醸:"#a855f7",純米吟醸:"#2563eb",純米:"#059669",本醸造:"#d97706",普通酒:"#6b7280",リキュール:"#e11d48",その他:"#9ca3af"},De=["純米大吟醸","大吟醸","純米吟醸","純米","本醸造","普通酒","リキュール","その他"];function U(e){return e.toLocaleString("ja-JP")}function le(e){return(e/1e3).toLocaleString("ja-JP",{maximumFractionDigits:1})}function ja(e){return e<2?"#ef4444":e<=4?"#eab308":"#22c55e"}function nd(e){return e<2?"要醸造":e<=4?"注意":"余裕あり"}function za(e){return"bc-"+encodeURIComponent(e).replace(/%/g,"-")}function sd(e){if(e.length===0)return'<div class="chart-empty">出荷データなし</div>';const t=[...new Set(e.map(r=>r.month))].sort(),n=De.filter(r=>e.some(c=>c.brewCategory===r)),s={};for(const r of e)s[r.month]||(s[r.month]={}),s[r.month][r.brewCategory]=r.shipmentMl;const i=820,l=300,d={top:20,right:20,bottom:50,left:70},p=i-d.left-d.right,u=l-d.top-d.bottom,y=t.map(r=>n.reduce((c,m)=>c+(s[r]?.[m]??0),0)),v=Math.max(...y,1),f=p/t.length,b=Math.max(f-8,14),k=[0,.25,.5,.75,1].map(r=>{const c=d.top+u-u*r,m=v*r/1e3;return`
    ...
    `}).join(""),P=t.map((r,c)=>{let m=d.top+u;const g=d.left+c*f+(f-b)/2,$=n.map(L=>{const O=s[r]?.[L]??0,D=O/v*u;return m-=D,D>0?`<rect x="${g}" y="${m}" width="${b}" height="${D}" fill="${Ne[L]??"#9ca3af"}" opacity="0.85" rx="1"><title>${L}: ${le(O)}L</title></rect>`:""}).join(""),[w,_]=r.split("-"),S=parseInt(_),E=S===10||c%2===0,A=S===10?`${w}年度`:`${S}月`;return`<g>${$}${E?`<text x="${g+b/2}" y="${l-12}" class="chart-axis centered-axis" style="font-size:10px;">${A}</text>`:""}</g>`}).join(""),o=n.map(r=>`<span style="display:inline-flex;align-items:center;gap:4px;margin-right:12px;font-size:11px;">
  ...
  `}function od(e,t,n,s){const i=new Map;for(const p of e){const u=p.brewCategory;i.has(u)||i.set(u,{rows:[],totalMl:0,avgMl:0,stockL:0});const y=i.get(u);y.rows.push(p),y.totalMl+=p.totalShipmentMl,y.avgMl+=p.monthlyAvgMl,y.stockL=p.currentStockL}const l=new Map;for(const p of t)l.has(p.brewCategory)||l.set(p.brewCategory,[]),l.get(p.brewCategory).push(p);return`<div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:16px;">${De.filter(p=>i.has(p)).map(p=>{const u=i.get(p),y=Ne[p]??"#9ca3af",v=za(p);l.get(p);const f=n[p]??{rawAlcoholPct:18,targetAlcoholPct:15},b=f.targetAlcoholPct>0?f.rawAlcoholPct/f.targetAlcoholPct:1;u.stockL*1e3;const k=u.totalMl,P=u.avgMl,o=k/1e3,r=Math.round(u.stockL*b*10)/10,c=r*1e3,m=P>0?Math.round(c/P*10)/10:0,g=r-o,$=P>0?Math.round(P*2/1e3*10)/10:0,w=r<$,_=ja(m),S=nd(m),E=Math.min(m/12*100,100),A=g>=0?"#22c55e":"#ef4444",L=g>=0?`+${U(Math.round(g))}L 余裕`:`${U(Math.round(g))}L 不足`,O=b>1.001;return`
        <div class="card" style="border-top:3px solid ${y};min-width:260px;flex:1;">
          ...
          <div id="stock-edit-${v}" style="display:none;margin-bottom:8px;">
            ${(()=>{const D=s.filter(z=>z.parentCategory===p),I=[{name:p,label:p},...D.map(z=>({name:z.name,label:z.name}))],M=I.flatMap(z=>t.filter(F=>F.brewCategory===z.name).map(F=>({...F,catLabel:z.label}))),B=I.map(z=>`<option value="${z.name}">${z.label}</option>`).join("");return`
  ...
  `}function rd(e,t,n,s,i,l={}){const d={html:"",needByCategory:{}};if(e.length===0)return d;const p={},u=new Date,y=u.getMonth()+1,v=y>=10?u.getFullYear():u.getFullYear()-1,f=v+1,b=new Map;for(const L of e)b.has(L.brewCategory)||b.set(L.brewCategory,new Map),b.get(L.brewCategory).set(L.fy,{shipL:L.shipmentL,annualL:L.annualizedL});const k=new Map;for(const L of i)k.has(L.brewCategory)||k.set(L.brewCategory,new Map),k.get(L.brewCategory).set(L.monthNum,L.avgMonthlyL);const P=[...new Set(e.map(L=>L.fy))].sort(),o=[...b.keys()].sort((L,O)=>{const D=[...De,...s.map(I=>I.name)];return(D.indexOf(L)===-1?99:D.indexOf(L))-(D.indexOf(O)===-1?99:D.indexOf(O))}),r=[];for(let L=y;L<=9;L++)r.push(L);if(y>=10)for(let L=1;L<=9;L++)r.push(L);const c=P.filter(L=>L<v),m=P.includes(v),g=o.map(L=>{const O=b.get(L);P.filter(pe=>O.has(pe));const D=Ne[L]??"#6366f1",I=k.get(L)??new Map,M=c.filter(pe=>O.has(pe)).map(pe=>O.get(pe).shipL);let B=0;if(M.length>=2){const pe=[];for(let xe=1;xe<M.length;xe++)M[xe-1]>0&&pe.push((M[xe]-M[xe-1])/M[xe-1]);B=pe.length>0?pe.reduce((xe,Ks)=>xe+Ks,0)/pe.length:0}const z=M.length>0?M[M.length-1]:O.get(v)?.annualL??0,H=r.reduce((pe,xe)=>pe+(I.get(xe)??0),0),F=t.filter(pe=>pe.brewCategory===L).reduce((pe,xe)=>pe+xe.volumeL,0),j=n[L],Q=j&&j.targetAlcoholPct>0?j.rawAlcoholPct/j.targetAlcoholPct:1,Z=Math.round(F*Q),re=Math.max(0,Z-Math.round(H)),ie=L in l,V=ie?l[L]:B,ee=Math.round(V*100),$e=Math.round(z*(1+V)),ge=Math.max(0,$e-re);p[L]=ge;const Y=ee>0?"#22c55e":ee<0?"#ef4444":"#6b7280",X=Math.round(B*100),qe=O.get(v)?.annualL??0;return`
      <tr>
        <td style="color:${D};font-weight:600;white-space:nowrap;">${L}</td>
        ${c.map(pe=>`<td style="text-align:right;">${O.has(pe)?U(Math.round(O.get(pe).shipL)):"—"}</td>`).join("")}
        ...
        <td style="text-align:right;">
          <input type="number" step="1" value="${ee}"
            ...
            title="${ie?`手動設定（自動: ${M.length>=2?X+"%":"—"}）`:"自動算出"}" />%
        </td>
    ...
    `}).join("");let $=0,w=0,_=0,S=0,E=0;for(const L of o){const O=b.get(L),D=k.get(L)??new Map,I=c.filter(V=>O.has(V)).map(V=>O.get(V).shipL);let M=0;if(I.length>=2){const V=[];for(let ee=1;ee<I.length;ee++)I[ee-1]>0&&V.push((I[ee]-I[ee-1])/I[ee-1]);M=V.length>0?V.reduce((ee,$e)=>ee+$e,0)/V.length:0}const B=I.length>0?I[I.length-1]:O.get(v)?.annualL??0,z=t.filter(V=>V.brewCategory===L).reduce((V,ee)=>V+ee.volumeL,0),H=n[L],F=H&&H.targetAlcoholPct>0?H.rawAlcoholPct/H.targetAlcoholPct:1,j=Math.round(z*F),Q=r.reduce((V,ee)=>V+(D.get(ee)??0),0),Z=Math.max(0,j-Math.round(Q)),re=L in l?l[L]:M,ie=Math.round(B*(1+re));$+=j,w+=Math.round(Q),_+=Z,S+=ie,E+=Math.max(0,ie-Z)}const A=y<=9?`${y}月〜9月`:`${y}月〜翌9月`;return{needByCategory:p,html:`
  ...
  `}function dd(e,t,n,s,i){if(e.length===0)return"";const l=n.map(f=>f.name);[...De,...l];const d=new Map;for(const f of n)d.has(f.parentCategory)||d.set(f.parentCategory,[]),d.get(f.parentCategory).push(f);const p=new Map;for(const f of e)p.has(f.brewCategory)||p.set(f.brewCategory,[]),p.get(f.brewCategory).push(f);for(const f of l)p.has(f)||p.set(f,[]);const u=new Set;for(const f of n)for(const b of p.get(f.name)??[])u.add(b.productCode);const y=new Map;for(const f of De)y.set(f,p.get(f)??[]);const v=De.filter(f=>p.has(f)).map(f=>{const b=p.get(f)??[],k=Ne[f]??"#9ca3af",P=d.get(f)??[],o=P.length>0,r=b.reduce((A,L)=>A+L.annualMl,0),c=b.reduce((A,L)=>A+L.monthlyAvgMl,0),m=b.filter(A=>!u.has(A.productCode)),g=m.filter(A=>!t.has(A.productCode)),$=g.reduce((A,L)=>A+L.annualMl,0),w=g.reduce((A,L)=>A+L.monthlyAvgMl,0),_=m.filter(A=>t.has(A.productCode)),S=m.map(A=>{const L=t.has(A.productCode);return`
        ...
        `}).join(""),E=P.map(A=>{const L=p.get(A.name)??[],O=L.reduce((F,j)=>F+j.annualMl,0),D=L.reduce((F,j)=>F+j.monthlyAvgMl,0),I=i.filter(F=>F.brewCategory===A.name),M=I.reduce((F,j)=>F+j.volumeL,0),B=za(A.name),z=L.map(F=>`
          ...
          <tr><td colspan="5" style="padding:0;">
            <div style="border-left:3px solid #6366f1;margin:8px 0 8px 16px;padding:6px 0 6px 12px;">
              <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px;flex-wrap:wrap;">
                ...
                <span style="font-size:11px;color:var(--text-secondary);">${L.length}品 ・ ${le(O)}L/年${M>0?` ・ 在庫${U(M)}L`:""}</span>
              ...
              ${z.length>0||H.length>0?`
                <table class="data-table" style="font-size:11px;margin:0;">
                  ...
                  ${L.length>0?`<tfoot><tr style="font-weight:600;"><td></td><td>確定分計</td><td></td>
                    <td style="text-align:right;">${le(O)}</td><td style="text-align:right;">${le(D)}</td>
                  </tr></tfoot>`:""}
                </table>
...
<div class="panel">
  ...
  `}function Od(e){const t={empty:"空",in_use:"使用中",aging:"熟成中"},n={empty:"neutral",in_use:"warning",aging:"success"},s=e.map(y=>{const v=y.capacity>0?Math.round(y.currentVolume/y.capacity*100):0;return`
        <tr>
  ...
  `}function aa(e){return new Intl.NumberFormat("ja-JP",{style:"currency",currency:"JPY",maximumFractionDigits:0}).format(e)}function Md(e,t,n){const s=e.rows.map((y,v)=>`
      <tr>
